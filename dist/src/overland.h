...
struct entrance_data
{
   ...
   const char *description; /* Description shown when you're in the room with the exit */
...
DECLARE_DO_FUN( do_showexit ); /* MAP OLC support to show exits */
...
typedef enum
{
   ACON_C1, ACON_C2, ACON_C3, ACON_C4, ACON_MAX
} acon_types;
...
#define REMOVE_EXIT_FLAG(var, bit)     xREMOVE_BIT((var)->exit_flags, (bit))
#define GET_MOVE(ch)            ((ch)->move)

/* Function protos for needed support functions */
const char *bamf_print( const char *fmt, CHAR_DATA * ch );
