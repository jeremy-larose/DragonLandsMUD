...
/*int spell_slots[MAX_SLOTS][MAX_PER_SLOT];  Spell Slots JL */
...
const char *skill_tname[] = { "unknown", "Spell", "Skill", "Weapon", "Tongue", "Herb", "Racial", "Disease", "Proficiency" };
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'C':
            ...
            KEY( "CasterType", Class->caster_type, fread_number( fp ) );
            break;
         ...
         case 'S':
            KEY( "SpellModStat", Class->spell_mod_stat, fread_number( fp ) );
            KEY( "SavingStatOne", Class->saving_throw_one, fread_number( fp ) );
            KEY( "SavingStatTwo", Class->saving_throw_two, fread_number( fp ) );
            if( !str_cmp( word, "Skill" ) )
            {
               ...
               else
               {
                  ...
                  skill_table[sn]->circle[cl] = circ;
               }
               fMatch = TRUE;
               break;
            }
            KEY( "Skilladept", Class->skill_adept, fread_number( fp ) );
            KEY( "Subclasses", Class->subclass, fread_bitvector( fp ) );
...
void write_class_file( int cl )
{
   ...
   fprintf( fpout, "SavingStatOne %d\n", Class->saving_throw_one );
   fprintf( fpout, "SavingStatTwo %d\n", Class->saving_throw_two );
   fprintf( fpout, "SpellModStat  %d\n", Class->spell_mod_stat );
   ...
   fprintf( fpout, "CasterType    %d\n", Class->caster_type );
   fprintf( fpout, "Subclasses    %s\n", print_bitvector( &Class->subclass ) );
   ...
   for( x = 0; x < num_skills; ++x )
   {
      ...
      if( ( y = skill_table[x]->skill_level[cl] ) < LEVEL_IMMORTAL )
         fprintf( fpout, "Skill '%s' %d %d %d\n", skill_table[x]->name, y, skill_table[x]->skill_adept[cl], skill_table[x]->circle[cl] );
   }
...
bool load_race_file( const char *fname )
{
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'S':
            KEY( "Size", race->size, fread_number( fp ) );
            KEY( "Speed", race->speed, fread_number( fp ) );
      ...
      if( race_name != NULL )
         snprintf( race->race_name, 17, "%-.16s", race_name );
...
void fwrite_skill( FILE * fpout, SKILLTYPE * skill )
{
   ...
   if( skill->spell_slot )
      fprintf( fpout, "SpellSlot     %d\n", skill->spell_slot );
   if( !xIS_EMPTY( skill->subclasses ) )
      fprintf( fpout, "Subclasses    %s\n", print_bitvector( &skill->subclasses ) );
   if( !xIS_EMPTY( skill->resource_cost ) )
      fprintf( fpout, "ResourceCost  %s\n", print_bitvector( &skill->resource_cost ) );
   ...
   if( skill->type != SKILL_HERB )
   {
      ...
      for( y = 0; y < MAX_CLASS; ++y )
         if( skill->circle[y] < min )
            min = skill->circle[y];
      fprintf( fpout, "Circle       %d\n", min );
...
int get_skill( const char *skilltype )
{
   ...
   if( !str_cmp( skilltype, "Herb" ) )
      return SKILL_HERB;
   if( !str_cmp( skilltype, "Proficiency" ) )
...
SKILLTYPE *fread_skill( FILE * fp )
{
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'C':
            KEY( "Castchar", skill->cast_char, fread_string_nohash( fp ) );
            KEY( "Castroom", skill->cast_room, fread_string_nohash( fp ) );
            KEY( "Castvict", skill->cast_vict, fread_string_nohash( fp ) );
            KEY( "Castself", skill->cast_self, fread_string_nohash( fp ) );
         ...
         case 'H':
            KEY( "HelpData", skill->help_data, fread_string_nohash( fp ) );
         ...
         case 'M':
            ...
            KEY( "SpellSlot", skill->spell_slot, fread_number( fp ) );
         ...
         case 'R':
            ...
            KEY( "ResourceCost", skill->resource_cost, fread_bitvector( fp ) );
         ...
         case 'S':
            ...
            KEY( "SelfcastRoom", skill->cast_self_room, fread_string_nohash( fp ) );
            KEY( "Slot", skill->slot, fread_number( fp ) );
            KEY( "Ssector", skill->spell_sector, fread_number( fp ) );
            KEY( "Subclasses", skill->subclasses, fread_bitvector( fp ) );
            break;
