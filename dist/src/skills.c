...
const char *const spell_saves[] = { "none", "str", "dex", "con", "int", "wis" };

const char *const spell_save_effect[] = { "none", "negate", "eightdam", "quarterdam", "halfdam", "3qtrdam",
   "reflect", "absorb"
};

const char *const spell_damage[] = { "none", "fire", "cold", "lightning", "force", "acid", "poison", "necrotic",
   "psychic", "thunder", "radiant", "slash", "blunt", "pierce" };
...
void free_skill( SKILLTYPE * skill )
{
   ...
   DISPOSE( skill->cast_char );
   DISPOSE( skill->cast_vict );
   DISPOSE( skill->cast_room );
   DISPOSE( skill->cast_self );
   DISPOSE( skill->cast_self_room );
   DISPOSE( skill->help_data );
   DISPOSE( skill->dice );
...
void do_slookup( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !str_cmp( arg, "all" ) )
   {
      for( sn = 0; sn < num_skills && skill_table[sn] && skill_table[sn]->name; ++sn )
         pager_printf( ch, "Sn: %4d Slot: %4d Skill/spell: '%-20s' Damtype: %s   &RSpell Info&w: [&C%d&w]\r\n",
                       sn, skill_table[sn]->slot, skill_table[sn]->name, spell_damage[SPELL_DAMAGE( skill_table[sn] )], skill_table[sn]->info );
   }
   ...
   else
   {
      ...
      ch_printf( ch, "&wResource Cost: &c%s&w\r\n", !xIS_EMPTY( skill->resource_cost ) ? ext_flag_string( &skill->resource_cost, resource_flags ) : "Mana" );
      ...
      ch_printf( ch, "Sectors Allowed: %s\n", skill->spell_sector ? flag_string( skill->spell_sector, sect_types ) : "All" );
      ch_printf( ch, "Subclasses Allowed: %s\n", !xIS_EMPTY( skill->subclasses ) ? ext_flag_string( &skill->subclasses, subclass_flags ) : "None" );
      ch_printf( ch, "Dammsg: %s\r\nWearoff: %s\n", skill->noun_damage, skill->msg_off ? skill->msg_off : "(none set)" );
      if( skill->dice && skill->dice[0] != '\0' )
         ch_printf( ch, "Dice: %s\r\n", skill->dice );
      if( skill->type == SKILL_SPELL ) /* Spell Memorization 1/17 JL */
         ch_printf( ch, "Spell Slot: %d\r\n", skill->spell_slot );
      if( skill->cooldown )
         ch_printf( ch, "Cooldown: %d\r\n", skill->cooldown );
      ...
      if( skill->cast_char && skill->cast_char[0] != '\0' )
         ch_printf( ch, "Castchar  : %s\r\n", skill->cast_char );
      if( skill->cast_vict && skill->cast_vict[0] != '\0' )
         ch_printf( ch, "Castvict  : %s\r\n", skill->cast_vict );
      if( skill->cast_room && skill->cast_room[0] != '\0' )
         ch_printf( ch, "Castroom  : %s\r\n", skill->cast_room );
      if( skill->cast_self && skill->cast_self[0] != '\0' )
         ch_printf( ch, "Castself  : %s\r\n", skill->cast_self );
      if( skill->cast_self_room && skill->cast_self_room[0] != '\0' )
         ch_printf( ch, "Selfcast Room: %s\r\n", skill->cast_self_room );
      if( skill->help_data && skill->help_data[0] != '\0' )
         ch_printf( ch, "Castchar  : %s\r\n", skill->help_data );
      ...
      if( skill->type != SKILL_HERB )
      {
         if( skill->type != SKILL_RACIAL )
         {
            ...
            for( iClass = 0; iClass < MAX_PC_CLASS; iClass++ )
            {
               ...
               snprintf( buf + 3, MAX_STRING_LENGTH - 3, ") lvl: %3d circ: %2d", skill->skill_level[iClass],
                         skill->circle[iClass] );
               if( iClass % 3 == 2 )
                  mudstrlcat( buf, "\r\n", MAX_STRING_LENGTH );
               else
...
void do_sset( CHAR_DATA* ch, const char* argument)
{
   ...
   if( arg1[0] == '\0' || arg2[0] == '\0' || argument[0] == '\0' )
   {
      ...
      if( get_trust( ch ) > LEVEL_GREATER )
      {
         ...
         send_to_char( "  sector spellslot circle subclasses cooldown resourcecost\r\n", ch );
       ...
       && ( !str_cmp( arg2, "skill" ) || !str_cmp( arg2, "herb" ) || !str_cmp( arg2, "ability" ) ) )
   {
      ...
      for( i = 0; i < MAX_PC_CLASS; i++ )
      {
         ...
         skill->circle[i] = MAX_CIRCLE_LEVEL;
      }
            ...
            || ( is_number( arg1 ) && ( sn = atoi( arg1 ) ) >= 0 ) ) )
   {
      ...
      if( !str_cmp( arg2, "spellslot" ) )
      {
         skill->spell_slot = URANGE( 0, atoi(argument ), 9 );
         send_to_char( "Ok.\r\n", ch );
         return;
      }
      if( !str_cmp( arg2, "cooldown" ) )
      {
         skill->cooldown = URANGE( 0, atoi( argument ), 30000 );
         send_to_char( "Ok.\r\n", ch );
         return;
      }
      
      if( !str_cmp( arg2, "subclasses" ) )
      {
         char tmp_arg[MAX_STRING_LENGTH];
         if( !argument || argument[0] == '\0' )
         {
            send_to_char( "Usage: sset <skill> subclass <flag> [flag]...\r\n", ch );
            return;
         }
         while( argument[0] != '\0' )
         {
            argument = one_argument( argument, tmp_arg );
            value = get_subclassflag( tmp_arg );
            if( value < 0 || value >= MAX_BITS )
               ch_printf( ch, "Unknown flag: %s\r\n", tmp_arg );
            else
               xTOGGLE_BIT( skill->subclasses, value );
         }
         return;
      }
     
      if( !str_cmp( arg2, "resourcecost" ) )
      {
         char tmp_arg[MAX_STRING_LENGTH];
         if( !argument || argument[0] == '\0' )
         {
            send_to_char( "Usage: sset <skill> resourcecost <flag> [flag]...\r\n", ch );
            return;
         }
         while( argument[0] != '\0' )
         {
            argument = one_argument( argument, tmp_arg );
            value = get_resourceflag( tmp_arg );
            if( value < 0 || value >= MAX_BITS )
               ch_printf( ch, "Unknown flag: %s\r\n", tmp_arg );
            else
               xTOGGLE_BIT( skill->resource_cost, value );
         }
         return;
      }
      ...
      if( !str_cmp( arg2, "affect" ) )
      {
         ...
         if( loc == APPLY_AFFECT )
         {
            ...
            log_printf( "Aflag: %s: %d", modifier, get_aflag( modifier ) );
            if( modval < 0 )
               modval = 0;
      ...
      if( !str_cmp( arg2, "circle" ) )
      {
         ...
         if( Class >= MAX_PC_CLASS || Class < 0 )
            ...
            skill->circle[Class] = URANGE( 0, atoi( argument ), 100 );
         return;
      }
      ...
      if( !str_cmp( arg2, "castchar" ) )
      {
         if( skill->cast_char )
            DISPOSE( skill->cast_char );
         if( str_cmp( argument, "clear" ) )
            skill->cast_char = str_dup( argument );
         send_to_char( "Ok.\r\n", ch );
         return;
      }
      if( !str_cmp( arg2, "castvict" ) )
      {
         if( skill->cast_vict )
            DISPOSE( skill->cast_vict );
         if( str_cmp( argument, "clear" ) )
            skill->cast_vict = str_dup( argument );
         send_to_char( "Ok.\r\n", ch );
         return;
      }
      if( !str_cmp( arg2, "castroom" ) )
      {
         if( skill->cast_room )
            DISPOSE( skill->cast_room );
         if( str_cmp( argument, "clear" ) )
            skill->cast_room = str_dup( argument );
         send_to_char( "Ok.\r\n", ch );
         return;
      }
      
      if( !str_cmp( arg2, "castself" ) )
      {
         if( skill->cast_room )
            DISPOSE( skill->cast_self );
         if( str_cmp( argument, "clear" ) )
            skill->cast_self = str_dup( argument );
         send_to_char( "Ok.\r\n", ch );
         return;
      }
      if( !str_cmp( arg2, "selfroom" ) )
      {
         if( skill->cast_room )
            DISPOSE( skill->cast_self_room );
         if( str_cmp( argument, "clear" ) )
            skill->cast_self_room = str_dup( argument );
         send_to_char( "Ok.\r\n", ch );
         return;
      }
      if( !str_cmp( arg2, "helpdata" ) )
      {
         if( skill->help_data )
            DISPOSE( skill->help_data );
         if( str_cmp( argument, "clear" ) )
            skill->help_data = str_dup( argument );
         send_to_char( "Ok.\r\n", ch );
         return;
      }
...
void do_search( CHAR_DATA* ch, const char* argument )
{
   ...
   OBJ_DATA *startobj;
   int percent, door;
   int ch_check = 0, vch_check = 0;

   door = -1;
   
   if( ch->move <= 11 )
   {
      send_to_char( "You're too tired to search.\r\n", ch );
      return;
   }
   ...
   switch ( ch->substate )
   {
      default:
         ...
         act( AT_SKILL, "$n begins searching the area carefully...", ch, NULL, NULL, TO_ROOM );
         ch->alloc_ptr = str_dup( arg );
         return;
   ...
   ch->substate = SUB_NONE;
   ch_check = dc_perception( ch, FALSE );
   ch->move -= 10;
   ...
   {
      send_to_char( "You fail to find anything.\r\n", ch );
   ...
   {
      if( IS_AFFECTED( vch, AFF_HIDE ) && !can_see( ch, vch, FALSE ) )
      {
         vch_check = dc_stealth( vch, FALSE );
         bug( "DEBUG: %s: ch_check = %d, vch->check = %d", __func__, ch_check, vch_check );
         if( ch_check > vch_check )
         {
            xREMOVE_BIT( vch->affected_by, AFF_HIDE );
            act( AT_SKILL, "$n finds $N!", ch, NULL, vch, TO_NOTVICT );
            act( AT_SKILL, "In your search, you find $N!", ch, NULL, vch, TO_CHAR );
            act( AT_SKILL, "$n finds you in $s search!", ch, NULL, vch, TO_VICT );
            return;
         }
      }
   }
...
void do_pounce( CHAR_DATA * ch, const char *argument )
{
   ...
   else
   {
      ...
      global_retcode = damage( ch, victim, 0, gsn_pounce, FALSE );
      check_illegal_pk( ch, victim );
   }
   return;
}
...
void do_backstab( CHAR_DATA* ch, const char* argument)
{
   ...
   {
      if( obj->value[4] != WEP_RAPIER || obj->value[4] != WEP_SHORT_SWORD )
      {
         send_to_char( "You need to wield a piercing or stabbing weapon.\r\n", ch );
         return;
      }
   ...
   else
   {
      ...
      global_retcode = damage( ch, victim, 0, gsn_backstab, FALSE );
      check_illegal_pk( ch, victim );
   }
   return;
}
   ...
   switch ( ch->in_room->sector_type )
   {
      ...
      case SECT_MOUNTAIN:
         if( ch->Class == CLASS_DRUID )
            ...
            managain += 2;
         break;
/*** Unused with new meditate code 1/5/2017 JL ***
      case SECT_WATER_SWIM:
      case SECT_WATER_NOSWIM:
      case SECT_UNDERWATER:
   ...
   switch ( ch->in_room->sector_type )
   {
      ...
      case SECT_OCEANFLOOR:
         ...
         if( !IS_AFFECTED( ch, AFF_AQUA_BREATH ) )
            managain -= 2;
         break;
/*** Removed for new meditate system 1/2017 JL **
      case SECT_AIR:
      if( ch->race == RACE_PIXIE )
            managain += 3;
         else if( !IS_AFFECTED( ch, AFF_FLYING ) )
...
void do_kick( CHAR_DATA* ch, const char* argument)
{
   ...
   if( can_use_skill( ch, number_percent(  ), gsn_kick ) )
   {
      ...
      global_retcode = damage( ch, victim, number_range( 1, ch->level ), gsn_kick, FALSE );
   }
   else
   {
      learn_from_failure( ch, gsn_kick );
      global_retcode = damage( ch, victim, 0, gsn_kick, FALSE  );
   }
   return;
}
...
void do_punch( CHAR_DATA* ch, const char* argument)
{
   ...
   if( can_use_skill( ch, number_percent(  ), gsn_punch ) )
   {
      ...
      global_retcode = damage( ch, victim, number_range( 1, ch->level ), gsn_punch, FALSE );
   }
   else
   {
      learn_from_failure( ch, gsn_punch );
      global_retcode = damage( ch, victim, 0, gsn_punch, FALSE );
   }
   return;
}
...
void do_bite( CHAR_DATA* ch, const char* argument)
{
   ...
   if( can_use_skill( ch, number_percent(  ), gsn_bite ) )
   {
      ...
      global_retcode = damage( ch, victim, number_range( 1, ch->level ), gsn_bite, FALSE );
   }
   else
   {
      learn_from_failure( ch, gsn_bite );
      global_retcode = damage( ch, victim, 0, gsn_bite, FALSE );
   }
   return;
}
...
void do_claw( CHAR_DATA* ch, const char* argument)
{
   ...
   if( can_use_skill( ch, number_percent(  ), gsn_claw ) )
   {
      ...
      global_retcode = damage( ch, victim, number_range( 1, ch->level ), gsn_claw, FALSE );
   }
   else
   {
      learn_from_failure( ch, gsn_claw );
      global_retcode = damage( ch, victim, 0, gsn_claw, FALSE );
   }
   return;
}
...
void do_sting( CHAR_DATA* ch, const char* argument)
{
   ...
   if( can_use_skill( ch, number_percent(  ), gsn_sting ) )
   {
      ...
      global_retcode = damage( ch, victim, number_range( 1, ch->level ), gsn_sting, FALSE );
   }
   else
   {
      learn_from_failure( ch, gsn_sting );
      global_retcode = damage( ch, victim, 0, gsn_sting, FALSE );
   }
   return;
}
...
void do_tail( CHAR_DATA* ch, const char* argument)
{
   ...
   if( can_use_skill( ch, number_percent(  ), gsn_tail ) )
   {
      ...
      global_retcode = damage( ch, victim, number_range( 1, ch->level ), gsn_tail, FALSE );
   }
   else
   {
      learn_from_failure( ch, gsn_tail );
      global_retcode = damage( ch, victim, 0, gsn_tail, FALSE );
   }
   return;
}
...
void do_bash( CHAR_DATA* ch, const char* argument)
{
   ...
   if( can_use_skill( ch, ( number_percent(  ) + schance ), gsn_bash ) )
   {
      ...
      global_retcode = damage( ch, victim, number_range( 1, ch->level ), gsn_bash, FALSE );
   }
   else
   {
      learn_from_failure( ch, gsn_bash );
      global_retcode = damage( ch, victim, 0, gsn_bash, FALSE );
   }
   return;
}
...
void do_bloodlet( CHAR_DATA* ch, const char* argument)
{
   ...
   if( can_use_skill( ch, number_percent(  ), gsn_bloodlet ) )
   {
      ...
      damage( ch, ch, ch->level / 5, gsn_bloodlet, FALSE );
   }
   else
   {
...
void disarm( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   if( get_eq_char( ch, WEAR_MAIN_HAND ) == NULL ) //&& number_bits( 1 ) == 0 )
   {
      log_printf( "CH : %s not wielding a weapon.", ch->name );
   ...
   {
      act( AT_SKILL, "You tighten your grip around your weapon as $n tries to disarm you and fails!", ch, NULL, victim, TO_VICT );
      act( AT_SKILL, "$N tightens their grip around their weapon as you try to disarm them and fail!", ch, NULL, victim, TO_CHAR );
      act( AT_SKILL, "$N tightens their grip around their weapon as $n tries to disarm $m and fails!", ch, NULL, victim, TO_NOTVICT );
      
      learn_from_failure( ch, gsn_disarm );
      return;
   }

   if( ch->classvalues[0] <= 0 ) // Combat Superiority Dice Remaining?
   {
      ch_printf( ch, "You attempt to disarm, but you lack the strength to muster the technique.\r\n" );
      return;
   }

   charroll = dc_maneuver( ch );
   victstr = dc_strength( victim );
   log_printf( "Disarming strike: Ch (%d) vs Vict (%d) [%d]", charroll, victstr, charroll-victstr );

   
   act( AT_WHITE, "$n twists $s weapon, attempting to disarm $N!", ch, NULL, victim, TO_NOTVICT );
   act( AT_WHITE, "You twist your weapon with the blow, attemping to disarm $N!", ch, NULL, victim, TO_CHAR );
   act( AT_WHITE, "$n twists $s weapon with the blow, attemping to disarm you!", ch, NULL, victim, TO_VICT );
   ch->classvalues[0]--;

   if( charroll > victstr )
   {
      act( AT_SKILL, "$n DISARMS you!", ch, NULL, victim, TO_VICT );
      act( AT_SKILL, "$N's weapon gets sent flying as your blow knocks it from their hands!", ch, NULL, victim, TO_CHAR );
      act( AT_SKILL, "$n disarms $N!", ch, NULL, victim, TO_NOTVICT );
      learn_from_success( ch, gsn_disarm );
   }
   else
   {
      act( AT_WHITE, "$N deftly avoids your disarming strike!", ch, NULL, victim, TO_CHAR );
      act( AT_WHITE, "$N deftly avoids $n's attempt!", ch, NULL, victim, TO_NOTVICT );
      act( AT_WHITE, "You deftly avoid $n's attempt!", ch, NULL, victim, TO_VICT );
      return;
   }
...
void do_disarm( CHAR_DATA* ch, const char* argument)
{
   ...
   if( ( get_eq_char( ch, WEAR_WIELD ) == NULL ) && ( get_eq_char( ch, WEAR_MAIN_HAND ) == NULL ) )
   {
   ...
   if( ( obj = get_eq_char( victim, WEAR_WIELD ) ) && ( get_eq_char( ch, WEAR_MAIN_HAND ) == NULL ) )
   {
...
{
   short charroll, victstr = 0;
   if( IS_AFFECTED( victim, AFF_FLYING ) || IS_AFFECTED( victim, AFF_FLOATING ) )
      return;

   charroll = dc_maneuver( ch );
   victstr = dc_strength( victim );
   log_printf( "Trip attack: Ch (%d) vs Vict (%d) [%d]", charroll, victstr, charroll-victstr );
   ...
   if( victim->wait == 0 )
   {
      ...
      xSET_BIT( victim->affected_by, AFF_PRONE );
      ch->advantage = TRUE;
...
void do_cleave( CHAR_DATA * ch, const char *argument )
{
   ...
   if( ( obj = get_wielded_obj( ch, FALSE ) ) == NULL )
   {
      send_to_char( "You need to be wielding a weapon.\r\n", ch );
      return;
   } 
   ...
   if( can_use_skill( ch, number_percent(  ), gsn_cleave ) )
   {
      ...
      global_retcode = damage( ch, victim, dam, gsn_cleave, FALSE );
   }
   else
   {
      learn_from_failure( ch, gsn_cleave );
      global_retcode = damage( ch, victim, 0, gsn_cleave, FALSE );
   }
   return;
}
...
void do_circle( CHAR_DATA* ch, const char* argument)
{
   ...
   if( obj->value[4] != WEP_DAGGER )
   {
      if( obj->value[4] != WEP_SHORT_SWORD || obj->value[4] != WEP_RAPIER )
      {
         send_to_char( "You need to wield a piercing or stabbing weapon.\r\n", ch );
         return;
      }
   ...
   else
   {
      ...
      WAIT_STATE( ch, 2 * PULSE_VIOLENCE );
      global_retcode = damage( ch, victim, 0, gsn_circle, FALSE );
   }
   return;
}
...
void do_hitall( CHAR_DATA* ch, const char* argument)
{
   ...
   act( AT_WHITE, "$n spins into a flurry of attacks, breaking the horde!", ch, vch, NULL, TO_ROOM );
   act( AT_WHITE, "You spin into a flurry of attacks, breaking the horde!" , ch, vch, NULL, TO_CHAR );
                          ...
                          OBJ_DATA * projectile, short dist, short dt, const char *stxt, short color )
{
   ...
   if( IS_NPC( victim ) && xIS_SET( victim->act, ACT_SENTINEL ) && ch->in_room != victim->in_room )
   {
      ...
      if( projectile )
      {
         ...
         switch( projectile->value[3] )
         {
            case 13:
            case 14:
               learn_from_failure( ch, gsn_bows );
               break;
      ...
      return damage( ch, victim, 0, dt, FALSE );
   }
   ...
   else
   {
      ...
      global_retcode = damage( ch, victim, 0, dt, FALSE );
...
void do_fire( CHAR_DATA* ch, const char* argument)
{
   ...
   {
      mudstrlcat( buf, "You have nothing to fire...\r\n", MAX_STRING_LENGTH );

      switch( bow->value[5] )
      {
         case PROJ_BOLT:  mudstrlcat( buf, "You have no bolts...\r\n", MAX_STRING_LENGTH );	   break;
         case PROJ_ARROW: mudstrlcat( buf, "You have no arrows...\r\n", MAX_STRING_LENGTH );   break;
         case PROJ_DART:  mudstrlcat( buf, "You have no darts...\r\n", MAX_STRING_LENGTH );	break;
         case PROJ_STONE: mudstrlcat( buf, "You have no slingstones...\r\n", MAX_STRING_LENGTH);	break;
      }
      send_to_char( buf, ch );
      return;
   }
...
void do_slice( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !can_use_skill( ch, number_percent(  ), gsn_slice ) && !IS_IMMORTAL( ch ) )
   {
      ...
      if( number_percent(  ) + ( get_curr_dex( ch ) - 13 ) < 10 )
      {
         ...
         damage( ch, ch, ch->level, gsn_slice, FALSE );
      }
      return;
   }
