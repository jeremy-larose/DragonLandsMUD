...
/*
 * How much experience a character is worth
 */
int get_exp_worth( CHAR_DATA * ch )
{
   int wexp;
   
   wexp = ( challenge_table[ch->challenge_rating].exp );
...
short get_exp_base( CHAR_DATA * ch )
{
   if( IS_NPC( ch ) )
      return 1000;
   return level_prog_table[ch->level].exp_needed;
}
...
int exp_level( CHAR_DATA * ch, short level )
{
   ...
   lvl = UMAX( 0, level - 1 );
   return ( level_prog_table[lvl].exp_needed );
}
...
short dc_spellcasting( CHAR_DATA * ch )
{
   ...
   if ( IS_NPC( ch ) )
      save = ( mod + ( challenge_table[ch->challenge_rating].bonus ) );
   else
      save = ( mod + ( 8 + level_prog_table[ch->level].bonus ) );

   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Spellcasting DC: %d+%d]\r\n", save, mod );
   return save;      
}
...
short dc_athletics( CHAR_DATA *ch )
{
   ...
   if( IS_NPC( ch ) )
      mod = challenge_table[ch->challenge_rating].bonus;
   else
   {
      if( ch->pcdata->learned[gsn_athletics] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += str_app[get_curr_str( ch )].bonus;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Athletics Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
...
short dc_acrobatics( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_acrobatics] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += dex_app[get_curr_dex( ch )].bonus;
   ...
   roll = dice+mod;
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Acrobatics Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
...
short dc_animal_handling( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_animal_handling] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += wis_app[get_curr_wis( ch )].bonus;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Animal Handling Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
...
short dc_arcana( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_arcana] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += int_app[get_curr_int( ch )].bonus;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Arcana Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
...
short dc_deception( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_deception] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += cha_app[get_curr_cha( ch )].bonus;
   roll = dice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Deception Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
...
short dc_history( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_history] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += int_app[get_curr_int( ch )].bonus;
   roll = dice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[History Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
...
short dc_insight( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_insight] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += wis_app[get_curr_wis( ch )].bonus;
   roll = dice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Insight Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
   return roll;
}
...
short dc_intimidation( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_intimidation] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += cha_app[get_curr_cha( ch )].bonus;
   roll = dice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Intimidation Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
   return roll;
}
...
short dc_investigation( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_investigation] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += int_app[get_curr_int( ch )].bonus;
   roll = dice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Investigation Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
   return roll;
}
...
short dc_medicine( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_medicine] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += wis_app[get_curr_wis( ch )].bonus;
   roll = dice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Medicine Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
   return roll;
}
...
short dc_nature( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_nature] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += int_app[get_curr_int( ch )].bonus;
   roll = dice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Nature Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
   return roll;
}
...
short dc_perception( CHAR_DATA *ch, bool silent )
{
   ...
   {
      if( ch->pcdata->learned[gsn_perception] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += wis_app[get_curr_wis( ch )].bonus;
   roll = dice+mod;

   if( xIS_SET( ch->act, PLR_DICEROLLS ) && silent == FALSE )
      ch_printf( ch, "[Perception Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
   return roll;
}
...
short dc_performance( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_performance] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += cha_app[get_curr_cha( ch )].bonus;
   roll = dice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Performance Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
   return roll;
}
...
short dc_persuasion( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_persuasion] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Persuasion Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
   return roll;
}
...
short dc_religion( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_religion] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += int_app[get_curr_int( ch )].bonus;
   roll = dice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Religion Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
   return roll;
}
...
short dc_sleight( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_sleight_of_hand] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += dex_app[get_curr_dex( ch )].bonus;
   roll = dice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Sleight of Hand Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
   return roll;
}
...
short dc_stealth( CHAR_DATA *ch, bool silent )
{
   ...
   {
      if( ch->pcdata->learned[gsn_stealth] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += dex_app[get_curr_dex( ch )].bonus;
   roll = dice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) && silent == FALSE )
      ch_printf( ch, "[Stealth Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
   return roll;
}
...
short dc_survival( CHAR_DATA *ch )
{
   ...
   {
      if( ch->pcdata->learned[gsn_survival] > 0 )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += wis_app[get_curr_wis( ch )].bonus;
   roll = dice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Survival Check: Rolled %d (%d+%d)]\r\n", roll, dice, mod );
   return roll;
}
...
short dc_constitution( CHAR_DATA *ch )
{
   ...
   {
      short newroll = dice( 1, 20 );
      log_printf( "Disadvantaged roll! Prev dice: %d, new roll: %d.", randomdice, newroll );
      if( randomdice > newroll )
         randomdice = newroll;
   }
   ...
   {
      if( class_table[ch->Class]->spell_mod_stat == APPLY_CON )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += con_app[get_curr_con( ch )].bonus;
   roll = randomdice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Constitution Check: Rolled %d (%d+%d)]\r\n", roll, randomdice, mod );
   return roll;
}
...
short dc_dexterity( CHAR_DATA *ch )
{
   ...
   {
      if( class_table[ch->Class]->spell_mod_stat == APPLY_DEX )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += dex_app[get_curr_dex( ch )].bonus;
   roll = randomdice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Dexterity Check: Rolled %d (%d+%d)]\r\n", roll, randomdice, mod );
   return roll;
}
...
short dc_wisdom( CHAR_DATA *ch )
{
   ...
   {
      if( class_table[ch->Class]->spell_mod_stat == APPLY_WIS )
         mod = level_prog_table[ch->level].bonus;
   }
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Wisdom Check: Rolled %d (%d+%d)]\r\n", roll, randomdice, mod );
   return roll;
}
...
short dc_intelligence( CHAR_DATA *ch )
{
   ...
   {
      if( class_table[ch->Class]->spell_mod_stat == APPLY_INT )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += int_app[get_curr_int( ch )].bonus;
   roll = randomdice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Intelligence Check: Rolled %d (%d+%d)]\r\n", roll, randomdice, mod );
   return roll;
}
...
short dc_charisma( CHAR_DATA *ch )
{
   ...
   {
      if( class_table[ch->Class]->spell_mod_stat == APPLY_CHA )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += cha_app[get_curr_cha( ch )].bonus;
   roll = randomdice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Charisma Check: Rolled %d (%d+%d)]\r\n", roll, randomdice, mod );
   return roll;
}
...
short dc_strength( CHAR_DATA *ch )
{
   ...
   {
      if( class_table[ch->Class]->spell_mod_stat == APPLY_STR )
         mod = level_prog_table[ch->level].bonus;
   }
   mod += str_app[get_curr_str( ch )].bonus;
   roll = randomdice+mod;
   ...
   if( xIS_SET( ch->act, PLR_DICEROLLS ) )
      ch_printf( ch, "[Strength Check: Rolled %d (%d+%d)]\r\n", roll, randomdice, mod );
   return roll;
}
...
short dc_maneuver( CHAR_DATA * ch )
{
   ...
   if( str_app[get_curr_str( ch )].bonus > dex_app[get_curr_dex( ch )].bonus )
      mod += str_app[get_curr_str( ch )].bonus;
   else
      mod += dex_app[get_curr_dex( ch )].bonus;
   ...
   roll = mod+base;
   if( xIS_SET( ch->act, PLR_DICEROLLS ) ) 
      ch_printf( ch, "[Maneuver DC: %d (%d+%d)]\r\n", roll, base, mod );
...
short get_curr_str( CHAR_DATA * ch )
{
   ...
   return URANGE( 0, ch->perm_str + ch->mod_str, max );
}
...
short get_curr_int( CHAR_DATA * ch )
{
   ...
   return URANGE( 0, ch->perm_int + ch->mod_int, max );
}
...
short get_curr_wis( CHAR_DATA * ch )
{
   ...
   return URANGE( 0, ch->perm_wis + ch->mod_wis, max );
}
...
short get_curr_dex( CHAR_DATA * ch )
{
   ...
   return URANGE( 0, ch->perm_dex + ch->mod_dex, max );
}
...
short get_curr_con( CHAR_DATA * ch )
{
   ...
   return URANGE( 0, ch->perm_con + ch->mod_con, max );
}
...
short get_curr_cha( CHAR_DATA * ch )
{
   ...
   return URANGE( 0, ch->perm_cha + ch->mod_cha, max );
}
...
short get_curr_lck( CHAR_DATA * ch )
{
   ...
   return URANGE( 0, ch->perm_lck + ch->mod_lck, max );
}
...
/* 
 * 5th Edition D&D Spell Slot Casting -- Seikyr 1/17
 * max_spell_slot_lookup checks PC class to see what type of caster
 * they are, then returns the maximum number of slots they can cast
 * from. 
 */
int max_spell_slot_lookup(CHAR_DATA *ch,int slot)
{
   int x, max = 0;
   
   if ( class_table[ch->Class]->caster_type == FULL_CASTER )
   {
      for ( x = 0; full_cast_table[x].level < MAX_LEVEL; x++)
         if( ch->level >= full_cast_table[x].level )
            max = full_cast_table[x].slot_max[slot];
   }
   else if ( class_table[ch->Class]->caster_type == HALF_CASTER )
   {
      for ( x = 0; half_cast_table[x].level < MAX_LEVEL; x++ )
         if( ch->level >= half_cast_table[x].level )
            max = half_cast_table[x].slot_max[slot];
   }
   else /* ( class_table[ch->Class]->caster_type == THIRD_CASTER ) */
   {
      for ( x = 0; third_cast_table[x].level < MAX_LEVEL; x++ )
         if( ch->level >= third_cast_table[x].level )
            max = third_cast_table[x].slot_max[slot];
   }            
   return max;
}

int find_spell_slot( CHAR_DATA *ch, int search_slot )
{
  int x, max_spells = 0;
  ...
  max_spells = max_spell_slot_lookup( ch,search_slot );

  /* can't cast any spells of that level */
  if (max_spells == 0)
    return -2;

  /* find an empty slot to cast the spell */
  for (x = 0; x < max_spells; x++)
  {
    if (ch->pcdata->slots[search_slot][x].free < 0)
      return x;
  }
   return -1;
}
 ...
 * Thale (7/10/98)
 */
int find_cast_slot(CHAR_DATA *ch,int sn)
{
  ...
  max_spells = max_spell_slot_lookup(ch,circle);
  ...
  for (x = 0; x < max_spells; x++)
  {
    if (ch->pcdata->slots[circle][x].free >= 1 )
      return x;
  }
...
/*
 * Find if the PC has the spell prepared.
 * Returns the spell level if it's fully prepared.
 */
int find_spell_level( CHAR_DATA *ch, int sn )
{
   SPELL_PREP_DATA *spell;
   ...
   for( spell = ch->first_prep; spell; spell = spell->next )
   {
      if( spell->sn == sn && spell->fully_prepped == TRUE )
      return skill_table[sn]->circle[ch->Class]-1;
   }
 ...
 * Find the slot a spell is memorized in
 * Thale (7/10/98)
 * Return 0 if the spell does not need to be prepared to cast. (Cantrips, etc)
 */
int find_mem_spell(CHAR_DATA *ch,int sn)
{
   ...
   * logic and the first value of arrays.
   */
  circle = (skill_table[sn]->circle[ch->Class]); /* REMOVED -1 HERE SPELLBOOKTEST */
...
/*   Non-prepared spells go here, so any domain spells they may know. - Seikyr  
  if ( circle == 0 )
   return 0;
   */

  /* ch_printf( ch, "Find_Mem_Spell: Circle = %d", circle ); */
  /* find where the spell is memorized*/
  for (x = 0; x < max_spells; x++)
  {
...
bool has_class( CHAR_DATA *ch, int value )
{
...
int class_level( CHAR_DATA * ch, int classname )
{
   MULTICLASS_DATA *multiclass;

   if( ch->Class == classname )
      return ch->level;
   
   for( multiclass = ch->first_multiclass; multiclass; multiclass = multiclass->next )
   {
      if( multiclass->classname == classname )
         return multiclass->level;
   }

   return -1;
}

bool has_cooldown( CHAR_DATA *ch, int sn )
{
   ...
   {
      if( cooldown->sn == sn && cooldown->curr_available >= 1 )
      {
         bug( "%s: Cooldown found, sn = %d and curr_available = %d", __func__, cooldown->sn, cooldown->curr_available );
         return TRUE;
      }
   }
...
void affect_modify( CHAR_DATA * ch, AFFECT_DATA * paf, bool fAdd )
{
   ...
   else
   {
      ...
      switch ( location )
      {
         ...
         case APPLY_RESISTANT:
            xREMOVE_BIT( ch->resistant, mod );
            return;
         case APPLY_IMMUNE:
            xREMOVE_BIT( ch->immune, mod );
            return;
         case APPLY_SUSCEPTIBLE:
            xREMOVE_BIT( ch->susceptible, mod );
            return;
         case APPLY_REMOVE:
   ...
   switch ( location )
   {
      ...
      case APPLY_BASE_AC:
         if( fAdd )
         {
            if( ch->base_armor <= mod )
               ch->base_armor = mod;
            break;
         }
         else
         {
            ch->base_armor = GET_BASE_AC( ch );
               break;
         }
      case APPLY_HITROLL:
         ch->hitroll += mod;
      ...
      case APPLY_EXT_AFFECT:
         ...
         break;
      case APPLY_RESISTANT:
         xSET_BIT( ch->resistant, mod );
         break;
      case APPLY_IMMUNE:
         xSET_BIT( ch->immune, mod );
         break;
      case APPLY_SUSCEPTIBLE:
         xSET_BIT( ch->susceptible, mod );
         break;
      ...
      case APPLY_REMOVESPELL:
         if( xIS_SET( ch->in_room->room_flags, ROOM_NO_MAGIC ) || xIS_SET( ch->immune, RIS_MAGIC )
    ...
    */
   ToDrop = WPos = get_eq_char( ch, WEAR_MAIN_HAND );
   ToDropW = WWeight = WPos ? get_obj_weight( WPos ) : 0;
   DWeight = MWeight = 0;
   DPos = get_eq_char( ch, WEAR_OFF_HAND );
...
void aris_affect( CHAR_DATA * ch, AFFECT_DATA * paf )
{
   ...
   switch ( location )
   {
      case APPLY_AFFECT:
         ...
         break;
      case APPLY_RESISTANT:
         xSET_BIT( ch->resistant, paf->modifier );
         break;
      case APPLY_SUSCEPTIBLE:
         xSET_BIT( ch->susceptible, paf->modifier );
         break;
      case APPLY_IMMUNE:
         xSET_BIT( ch->immune, paf->modifier );
         break;
   }
}
...
void update_aris( CHAR_DATA * ch )
{
   ...
   if( ch->pcdata->deity )
   {
      ...
      if( ch->pcdata->favor > ch->pcdata->deity->elementnum )
         xSET_BITS( ch->resistant, ch->pcdata->deity->element );
      if( ch->pcdata->favor < ch->pcdata->deity->susceptnum )
         xSET_BITS( ch->susceptible, ch->pcdata->deity->suscept );
   }
...
int apply_ac( OBJ_DATA * obj, int iWear )
{
   ...
   {
      case WEAR_BODY:
            return armor_table[obj->armor_type].base_ac;
      case WEAR_HEAD:
         return 0;
      case WEAR_LEGS:
         return 0;
...
void equip_char( CHAR_DATA * ch, OBJ_DATA * obj, int iWear )
{
   ...
   if( ch->base_armor < armor_table[obj->armor_type].base_ac )
      ch->base_armor = apply_ac( obj, iWear );
   ch->armor -= armor_table[obj->armor_type].max_dex_mod;
...
void unequip_char( CHAR_DATA * ch, OBJ_DATA * obj )
{
   ...
   {
      ch->base_armor = 10;
      if( armor_table[obj->armor_type].max_dex_mod > 0 )
         ch->armor -= UMIN( armor_table[obj->armor_type].max_dex_mod, dex_app[get_curr_dex(ch)].bonus );
      if( IS_AFFECTED( ch, AFF_MAGE_ARMOR ) )
         ch->base_armor = 13;
...
CHAR_DATA *get_char_room( CHAR_DATA * ch, const char *argument )
{
   ...
   for( rch = ch->in_room->first_person; rch; rch = rch->next_in_room )
   {
      if( can_see( ch, rch, FALSE ) && !IS_NPC( rch ) )
      {
         if( nifty_is_name_prefix( arg, rch->adjective ) )
            return rch;
      }
   }
...
OBJ_DATA *get_wielded_obj( CHAR_DATA *ch, bool dual_flip )
{
   OBJ_DATA *wield;
...
/* Find an exact obj by vnum # 
*/

OBJ_DATA *get_bonded_obj_world( CHAR_DATA *ch, int vnum )
{
   OBJ_DATA *obj;

   for( obj = first_object; obj; obj = obj->next )
   {
      if( can_see_obj( ch, obj ) && vnum == obj->pIndexData->vnum )
      {
         if( str_infix( "bonded", obj->name ) ) 
         {
            log_printf( "Weapon found, but not bonded: %s", obj->name );
            continue;
         }
         if( str_infix( ch->name, obj->name ) )
         {
            log_printf( "Bonded weapon found, but not %s's: %s.", ch->name, obj->name );
            continue;
         } 
         else
         {
            log_printf( "Exact match found! %s", obj->name );
            return obj;
         }
      }
      if( !obj )
   ...
   //log_printf( "No bonded object exists in the world!" );
   return NULL;
}
...
{
   int ch_check, vict_check = 0;
   if ( !victim )            /* Gorog - panicked attempt to stop crashes */
   {
      bug( "%s: null victim! Ch %s has tried to see it.", __func__, ch->name );
      return FALSE;
   }
...
/* Deadlies link-dead over 2 ticks aren't seen by mortals -- Blodkai */
   ...
   if ( !IS_AFFECTED(ch, AFF_TRUESIGHT) )
   {
      ...
      if ( IS_AFFECTED( victim, AFF_INVISIBLE ) && !IS_AFFECTED( ch, AFF_DETECT_INVIS ) )
      {
         if( ch->Class == CLASS_ROGUE && ch->level >= 28 && !IS_AFFECTED( ch, AFF_DEAF ) ) /* Blindsense Rogue ability */
         {
            return TRUE;
         }
         else
...
bool is_greeted( CHAR_DATA * ch, CHAR_DATA * victim )
{
   GREET_DATA *greet;
   if( !victim )  /* Gorog - panicked attempt to stop crashes */
      return FALSE;
   ...
   for( greet = ch->first_greet; greet; greet = greet->next )
   {
      if( is_name2( victim->name, greet->name ) )
         return TRUE;
      else
         return FALSE;
   }
...
const char *affect_loc_name( int location )
{
   switch ( location )
   {
      ...
      case APPLY_DAMROLL:
         return "damage roll";
      case APPLY_SAVING_STR:
         return "strength saves";
      case APPLY_SAVING_DEX:
         return "dexterity saves";
      case APPLY_SAVING_CON:
         return "constitution saves";
      case APPLY_SAVING_INT:
         return "intelligence saves";
      case APPLY_SAVING_WIS:
         return "wisdom saves";
      case APPLY_SAVING_CHA:
         return "charisma saves";
      case APPLY_HEIGHT:
         return "height";
      case APPLY_WEIGHT:
         return "weight";
...
const char *affect_bit_name( EXT_BV * vector )
{
   ...
   if( xIS_SET( *vector, AFF_BARKSKIN ) )
      mudstrlcat( buf, " barkskin", 512 );
   if( xIS_SET( *vector, AFF_MAGE_ARMOR ) )
      mudstrlcat( buf, " mage_armor", 512 );
   if( xIS_SET( *vector, AFF_SPIRITGUARDIANS ) )
      mudstrlcat( buf, " spirit_guardians", 512 );
   if( xIS_SET( *vector, AFF_DEAF ) )
      mudstrlcat( buf, " deaf", 512 );
   if( xIS_SET( *vector, AFF_LUCKY ) )
      mudstrlcat( buf, " lucky", 512 );
   if( xIS_SET( *vector, AFF_DISENGAGE ) )
      mudstrlcat( buf, " disengage", 512 );
   if( xIS_SET( *vector, AFF_PRONE ) )
      mudstrlcat( buf, " prone", 512 );
   if( xIS_SET( *vector, AFF_GOADED ) )
      mudstrlcat( buf, " goaded", 512 );
   if( xIS_SET( *vector, AFF_FEINTED ) )
      mudstrlcat( buf, " feinted", 512 );
   if( xIS_SET( *vector, AFF_EARTHQUAKE ) )
      mudstrlcat( buf, " earthquake", 512 );
   if( xIS_SET( *vector, AFF_FRIGHTEN ) )
      mudstrlcat( buf, " frighten", 512 );
   if( xIS_SET( *vector, AFF_ELDRITCH ) )
      mudstrlcat( buf, " eldritch_strike", 512 );
   if( xIS_SET( *vector, AFF_SPIRIT ) )
      mudstrlcat( buf, " spirit", 512 );
...
const char *extra_bit_name( EXT_BV * extra_flags )
{
   ...
   if( xIS_SET( *extra_flags, ITEM_ANTI_WIZARD ) )
      mudstrlcat( buf, " anti-wizard", 512 );
   if( xIS_SET( *extra_flags, ITEM_ANTI_ROGUE ) )
      mudstrlcat( buf, " anti-rogue", 512 );
   if( xIS_SET( *extra_flags, ITEM_ANTI_FIGHTER ) )
      mudstrlcat( buf, " anti-fighter", 512 );
   ...
   if( xIS_SET( *extra_flags, ITEM_ANTI_BARBARIAN ) )
      mudstrlcat( buf, " anti-barbarian", 512 );
   ...
   if( xIS_SET( *extra_flags, ITEM_ANTI_MONK ) )
      mudstrlcat( buf, " anti-monk", 512 );
    if( xIS_SET( *extra_flags, ITEM_ANTI_WARLOCK ) )
      mudstrlcat( buf, " anti-warlock", 512 );
   if( xIS_SET( *extra_flags, ITEM_ANTI_SORCERER ) )
      mudstrlcat( buf, " anti-sorcerer", 512 );
   if( xIS_SET( *extra_flags, ITEM_ANTI_PALADIN) )
      mudstrlcat( buf, " anti-paladin", 512 );
   if( xIS_SET( *extra_flags, ITEM_ANTI_RANGER ) )
      mudstrlcat( buf, " anti-ranger", 512 );
   if( xIS_SET( *extra_flags, ITEM_ANTI_BARD ) )
      mudstrlcat( buf, " anti-bard", 512 );
   if( xIS_SET( *extra_flags, ITEM_SILVERY ) )
      mudstrlcat( buf, " silver", 512 );
   if( xIS_SET( *extra_flags, ITEM_VERSATILE ) )
      mudstrlcat( buf, " versatile", 512 );
   if( xIS_SET( *extra_flags, ITEM_FINESSE ) )
      mudstrlcat( buf, " finesse", 512 );
   if( xIS_SET( *extra_flags, ITEM_SPIRIT ) )
      mudstrlcat( buf, " spirit", 512 );
   return ( buf[0] != '\0' ) ? buf + 1 : ( char * )"none";
}
...
ch_ret spring_trap( CHAR_DATA * ch, OBJ_DATA * obj )
{
   ...
   switch ( typ )
   {
      ...
      case TRAP_TYPE_POISON_ARROW:
         ...
         if( retcode == rNONE )
            retcode = damage( ch, ch, dam, TYPE_UNDEFINED, FALSE );
         break;
      case TRAP_TYPE_POISON_GAS:
         retcode = obj_cast_spell( gsn_poison, lev, ch, ch, NULL );
         break;
...
void name_stamp_stats( CHAR_DATA * ch )
{
   ...
   ch->perm_str = dice( 3, 6 )-1*6;
   ch->perm_dex = dice( 3, 6 )-1*6;
   ch->perm_wis = dice( 3, 6 )-1*6;
   ch->perm_int = dice( 3, 6 )-1*6;
   ch->perm_con = dice( 3, 6 )-1*6;
   ch->perm_cha = dice( 3, 6 )-1*6;
   ch->perm_lck = dice( 3, 6 )-1*6;
}
...
bool can_astral( CHAR_DATA * ch, CHAR_DATA * victim )
{
   if( victim == ch
       ...
       || ( IS_NPC( victim ) && saves_int( ch->level, victim ) )
...
OBJ_DATA *clone_object( OBJ_DATA * obj )
{
   ...
   clone->timer = obj->timer;
    ...
    clone->x = obj->x;
    clone->y = obj->y;
   clone->armor_type = obj->armor_type;
...
OBJ_DATA *group_object( OBJ_DATA * obj1, OBJ_DATA * obj2 )
{
   ...
   if( obj1->pIndexData == obj2->pIndexData
      ...
      && obj1->armor_type == obj2->armor_type
