...
void do_rstat( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !str_cmp( arg, "ex" ) || !str_cmp( arg, "exits" ) )
   {
      ...
      for( cnt = 0, pexit = location->first_exit; pexit; pexit = pexit->next )
         ch_printf_color( ch,
                          "&W%2d) &w%2s to %-5d  &cKey: &w%d  &cFlags: &w%d  &cKeywords: '&w%s&c'\r\n     Exdesc: &w%s     &cBack link: &w%d  &cVnum: &w%d  &cDistance: &w%d  &cPulltype: &w%s  &cPull: &w%d  &cMap: &w(%d, %d)\r\n",
                          ++cnt,
                          dir_text[pexit->vdir],
   ...
   ch_printf_color( ch, "&wName: &C%s\r\n&wArea: &C%s  &wFilename: &C%s\r\n",
                    location->name,
...
*/
   ch_printf( ch, "&wVnum: &C%d   &wLight: &C%d     &wTeleDelay: &C%d    &wTeleVnum: &C%d   &wTunnel: &C%d   &wMax Weight: &C%d&w/&C%d\r\n",
                    location->vnum, location->light, location->tele_delay, location->tele_vnum, location->tunnel,
                    location->weight, location->max_weight );
   ch_printf( ch, "&wCeiling Height: &C%d\r\n", location->height );
   ch_printf( ch, "&wRoom flags: &C%s\r\n", ext_flag_string( &location->room_flags, r_flags ) );
   ch_printf( ch, "&wSector: &C%d - %s\r\n", location->sector_type, sect_types[location->sector_type] );
   ch_printf( ch, "&wDescription:\r\n&w%s\r\n", location->description );
   ch_printf_color( ch, "&wPerception (&C%d&w)\r\n&w%s\r\n", location->percheck, location->perdesc );
   if( location->nightdesc )
      ch_printf_color( ch, "&wNight Description:\r\n&w%s\r\n", location->nightdesc );
   send_to_char( "&wPermanent affects: &C", ch );
   ...
   if( location->first_extradesc )
   {
      ...
      send_to_char_color( "&wExtra description keywords: &C'", ch );
   ...
   for( paf = location->first_affect; paf; paf = paf->next )
      ch_printf_color( ch, "&wAffect: &C%s &wby &C%d.\r\n", affect_loc_name( paf->location ), paf->modifier );

   send_to_char_color( "&wCharacters in room: &C\r\n", ch );
   for( rch = location->first_person; rch; rch = rch->next_in_room )
   {
      if( can_see( ch, rch, FALSE ) )
         ch_printf( ch, "  &w(&C%d&w) &C%s\r\n", IS_NPC( rch ) ? rch->pIndexData->vnum : 0, rch->name );
   }
   ...
   if( location->first_exit )
      send_to_char_color( "&w------------------- &CEXITS &w-------------------\r\n", ch );
   for( cnt = 0, pexit = location->first_exit; pexit; pexit = pexit->next )
      ch_printf( ch,
                 "&w%2d) &C%-2s&w to &C%-5d   &wKey: &C%d &wMap: (&C%d, %d&w) &wKeywords: &C%s   &wFlags: &C%s\r\n",
                 ++cnt,
                 dir_text[pexit->vdir],
                 pexit->to_room ? pexit->to_room->vnum : 0,
                 pexit->key,
...
void do_ostat( CHAR_DATA* ch, const char* argument)
{
   ...
   ch_printf_color( ch, "&wName   : &C%s\r\n", obj->name );
   ch_printf_color( ch, "&wShort  : &C%s\r\n", obj->short_descr );
   ch_printf_color( ch, "&wLong   : &C%s\r\n", obj->description );
   ch_printf_color( ch, "&wVnum   : &C%d   ", obj->pIndexData->vnum );
   ch_printf_color( ch, "&wType   : &C%s   ", item_type_name( obj ) );
   ch_printf_color( ch, "&wCount  : &C%d   &wGcount: &C%d\r\n", obj->pIndexData->count, obj->count );
   ch_printf_color( ch, "&wSerial#: &C%d   &wTopIdxSerial#: &C%d   &wTopSerial#: &C%d\r\n", obj->serial, obj->pIndexData->serial, cur_obj_serial );
   ch_printf_color( ch, "&wNumber : &C%d/%d  ", 1, get_obj_number( obj ) );
   ch_printf_color( ch, "&wWeight: &C%d/%d   ", obj->weight, get_obj_weight( obj ) );
   ch_printf_color( ch, "&wLayers: &C%d   ", obj->pIndexData->layers );
   ch_printf_color( ch, "&wWear_loc: &C%d\r\n", obj->wear_loc );
   if( obj->action_desc && obj->action_desc[0] != '\0' )
      ch_printf_color( ch, "&wAction description: &C%s\r\n", obj->action_desc );
   if( IS_OBJ_STAT( obj, ITEM_PERSONAL ) && obj->owner && obj->owner[0] != '\0' )
      ch_printf_color( ch, "&wOwner: &Y%s\r\n", obj->owner );
   ch_printf_color( ch, "&wWear flags : &C%s\r\n", flag_string( obj->wear_flags, w_flags ) );
   ch_printf_color( ch, "&wExtra flags: &C%s\r\n", ext_flag_string( &obj->extra_flags, o_flags ) );
   ch_printf_color( ch, "&wMagic flags: &C%s\r\n", magic_bit_name( obj->magic_flags ) );

   ch_printf_color( ch, "&wCost    :  &Y%d &wGold, &Y%d &wSilver, &Y%d &wCopper\n\r", 
  obj->gold_cost, obj->silver_cost, obj->copper_cost );

   ch_printf_color( ch, "&wOn map: &C%s ", IS_OBJ_STAT( obj, ITEM_ONMAP ) ? map_names[obj->map] : "(none)" );
   ch_printf_color( ch, "&wCoords: &C%d %d\r\n", obj->x, obj->y );
   send_to_char_color( "&wTimer   : ", ch );
   if( obj->timer > 0 )
      ch_printf_color( ch, "&R%d  ", obj->timer );
   else
      ch_printf_color( ch, "&C%d  ", obj->timer );
   ch_printf_color( ch, "&wRent: &C%d    ", obj->pIndexData->rent );
   ch_printf_color( ch, "&wLevel: &C%d    ", obj->level );
   ch_printf_color( ch, "&wIndex level: &C%d\r\n", obj->pIndexData->level );
   ch_printf_color( ch, "&wIn room: &C%d  ", obj->in_room == NULL ? 0 : obj->in_room->vnum );
   ch_printf_color( ch, "&wIn object: &C%s  ", obj->in_obj == NULL ? "(none)" : obj->in_obj->short_descr );
   ch_printf_color( ch, "&wCarried by: &C%s\r\n", obj->carried_by == NULL ? "(none)" : obj->carried_by->name );
   ch_printf_color( ch, "&wIndex Values : &C%d %d %d %d %d %d.\r\n",
   ...
   ch_printf_color( ch, "&wObject Values: &C%d %d %d %d %d %d.\r\n",
                    obj->value[0], obj->value[1], obj->value[2], obj->value[3], obj->value[4], obj->value[5] );
   
   ch_printf_color( ch, "&wSize: &C%d/%s\r\n", obj->pIndexData->size, size_table[obj->pIndexData->size] );
 
   ch_printf( ch, "&wAdditional Object information:\r\n" );

   switch ( obj->item_type )
   {
      default:
         ch_printf( ch, "Additional details not available. This item type is not yet supported.\r\n" );
         ch_printf( ch, "Please report this to your immortal supervisor so this can be resolved.\r\n" );
         break;

      case ITEM_LIGHT:
         ch_printf( ch, "Value[2] - Hours left: " );
         if( obj->value[2] >= 0 )
            ch_printf( ch, "&c%d\r\n", obj->value[2] );
         else
            ch_printf( ch, "&cInfinite\r\n" );
         break;
      ...
      case ITEM_SCROLL:
         ...
         for( x = 1; x <= 3; ++x )
         {
            if( obj->value[x] >= 0 && ( sktmp = get_skilltype( obj->value[x] ) ) != NULL )
               ch_printf( ch, "Value[%d] - Spell (%d): &c%s\r\n", x, obj->value[x], sktmp->name );
            else
               ch_printf( ch, "Value[%d] - Spell: None\r\n", x );
         }

         if( obj->item_type == ITEM_PILL )
            ch_printf( ch, "Value[4] - Food Value: &c%d\r\n", obj->value[4] );
         break;

      case ITEM_SALVE:
      case ITEM_WAND:
      case ITEM_STAFF:
         ch_printf( ch, "Value[0] - Spell Level: &c%d\r\n", obj->value[0] );
         ch_printf( ch, "Value[1] - Max Charges: &c%d\r\n", obj->value[1] );
         ch_printf( ch, "Value[2] - Charges Remaining: &c%d\r\n", obj->value[2] );
         if( obj->item_type != ITEM_SALVE )
         {
            if( obj->value[3] >= 0 && ( sktmp = get_skilltype( obj->value[3] ) ) != NULL )
               ch_printf( ch, "Value[3] - Spell (%d): &c%s\r\n", obj->value[3], sktmp->name );
            else
               ch_printf( ch, "Value[3] - Spell: &cNone\r\n" );
            break;
         }
         ch_printf( ch, "Value[3] - Delay (beats): &c%d\r\n", obj->value[3] );
         for( x = 4; x <= 5; ++x )
         {
      ...
      case ITEM_MISSILE_WEAPON:
         ch_printf( ch, "Value[0] - Base Condition: &c%d\r\n", obj->value[0] );
         ch_printf( ch, "Value[1] - Min. Damage: &c%d\r\n", obj->value[1] );
         ch_printf( ch, "Value[2] - Max Damage: &c%d\r\n", obj->value[2] );
         ch_printf( ch, "&RAverage Hit: %d\r\n", ( obj->value[1] + obj->value[2] ) / 2 );
         if( obj->item_type != ITEM_MISSILE_WEAPON )
            ch_printf( ch, "&cValue[3] - Damage Type (%d): &c%s\r\n", obj->value[3], attack_table[obj->value[3]] );
         ch_printf( ch, "Value[4] - Skill Required (%d): &c%s\r\n", obj->value[4], weapon_skills[obj->value[4]] );
         if( obj->item_type == ITEM_MISSILE_WEAPON )
         {
            ch_printf( ch, "Value[5] - Projectile Fired (%d): &c%s\r\n", obj->value[5], projectiles[obj->value[5]] );
            ch_printf( ch, "Projectile fired must match on the projectiles this weapon fires.\r\n" );
         }
/*         ch_printf( ch, "%sValue[6] - Current Condition: &c%d\r\n", olc ? "&gK&w) " : "&w", obj->value[6] );
         ...
         ch_printf( ch, "&WThe following 3 settings only apply to automatically generated weapons.\r\n" );
         ch_printf( ch, "Value[8] - Weapon Type (%d): &c%s\r\n", olc ? "&gM&w) " : "&w", obj->value[8], weapon_type[obj->value[8]].name );
         ch_printf( ch, "Value[9] - Weapon Material (%d): &c%s\r\n", olc ? "&gN&w) " : "&w", obj->value[9], materials[obj->value[9]].name );
         ch_printf( ch, "Value[10] - Weapon Quality (%d): &c%s\r\n", olc ? "&gO&w) " : "&w", obj->value[10], weapon_quality[obj->value[10]] ); */
         break;

      case ITEM_ARMOR:
         if( obj->pIndexData->armor_type == 0 )
            sprintf( buf, "None/Naked" );
         else if( obj->pIndexData->armor_type == 1 )
            sprintf( buf, "Cloth" );
         else if( obj->pIndexData->armor_type == 2 )
            sprintf( buf, "Padded" );
         else if( obj->pIndexData->armor_type == 3 )
            sprintf( buf, "Leather" );
         else if( obj->pIndexData->armor_type == 4 )
            sprintf( buf, "Studded Leather" );
         else if( obj->pIndexData->armor_type == 5 )
            sprintf( buf, "Hide" );
         else if( obj->pIndexData->armor_type == 6 )
            sprintf( buf, "Chain Shirt" );
         else if( obj->pIndexData->armor_type == 7 )
            sprintf( buf, "Mage Armor" );
         else if( obj->pIndexData->armor_type == 8 )
            sprintf( buf, "Draconic Resilience" );
         else if( obj->pIndexData->armor_type == 9 )
            sprintf( buf, "Scale Mail" );
         else if( obj->pIndexData->armor_type == 10 )
            sprintf( buf, "Breast Plate" );
         else if( obj->pIndexData->armor_type == 11 )
            sprintf( buf, "Half Plate" );
         else if( obj->pIndexData->armor_type == 12 )
            sprintf( buf, "Ring Mail" );
         else if( obj->pIndexData->armor_type == 13 )
            sprintf( buf, "Chain Mail" );
         else if( obj->pIndexData->armor_type == 14 )
            sprintf( buf, "Splint Mail" );
         else if( obj->pIndexData->armor_type == 15 )
            sprintf( buf, "Plate Mail" );
         else
            sprintf( buf, "BUG: Inform Seikyr" );
         ch_printf( ch, "&cArmor Type: (%d/%d) &W%s\r\n", obj->pIndexData->armor_type, obj->armor_type, buf );
         
         ch_printf( ch, "Value[0] - &c%d\r\n", obj->value[0] );
         ch_printf( ch, "Value[1] - &c%d\r\n", obj->value[1] );
         break;

      case ITEM_COOK:
      case ITEM_FOOD:
         ch_printf( ch, "Value[0] - Food Value: &c%d\r\n", obj->value[0] );
         ch_printf( ch, "Value[1] - Condition (%d): &c", obj->value[1] );
         ...
         {
            ch_printf( ch, "Value[2] - Condition (%d): &c", obj->value[2] );
            dam = obj->value[2];
            if( dam >= 3 )
         ...
         if( obj->value[3] != 0 )
         {
            ch_printf( ch, "Value[3] - Poisoned (%d): &cYes\r\n", obj->value[3] );
         ...
         {
            ch_printf( ch, "Poison: Yes\r\n" );
            x = 2 * obj->value[0] * ( obj->value[3] > 0 ? obj->value[3] : 1 );
            ch_printf( ch, "Duration: %d\r\n", x );
         }
         if( obj->value[4] )
            ch_printf( ch, "Value[4] - Timer: &c%d\r\n", obj->value[4] );
         break;
      ...
      case ITEM_DRINK_CON:
         ...
         ch_printf( ch, "Value[0] - Capacity: &c%d\r\n", obj->value[0] );
         ch_printf( ch, "Value[1] - Quantity Left (%d): &c",  obj->value[1] );
         if( obj->value[1] > obj->value[0] )
            ...
            ch_printf( ch, "Empty\r\n" );
         ch_printf( ch, "Value[2] - Liquid (%d): &c%s\r\n", obj->value[2], liq->name );
      ...
      case ITEM_HERB:
         ch_printf( ch, "Value[1] - Charges: &c%d\r\n", obj->value[1] );
         ch_printf( ch, "Value[2] - Herb #: &cY%d\r\n", obj->value[2] );
         break;

      case ITEM_CONTAINER:
         ch_printf( ch, "Value[0] - Capacity (%d): &c", obj->value[0] );
         ch_printf( ch, "%s\r\n",
         ...
         ch_printf( ch, "Value[1] - Flags (%d): &c", obj->value[1] );
/*         if( obj->value[1] <= 0 )
            ch_printf( ch, " None\r\n" );
         else
            ch_printf( ch, "%s\r\n", flag_string( obj->value[1], container_flags ) ); */
         ch_printf( ch, "Value[2] - Key (%d): &c", obj->value[2] );
         if( obj->value[2] <= 0 )
            ch_printf( ch, "None\r\n" );
         else
         {
            ...
            if( key )
               ...
               ch_printf( ch, "ERROR: Key does not exist!\r\n" );
         }
         ch_printf( ch, "Value[3] - Condition: &c%d\r\n", obj->value[3] );
         if( obj->timer )
            ch_printf( ch, "Object Timer, Time Left: %d\r\n", obj->timer );
         break;

      case ITEM_PROJECTILE:
         ch_printf( ch, "Value[0] - Condition: &c%d\r\n",obj->value[0] );
         ch_printf( ch, "Value[1] - Min. Damage: &c%d\r\n", obj->value[1] );
         ch_printf( ch, "Value[2] - Max Damage: &c%d\r\n", obj->value[2] );
         ch_printf( ch, "Value[3] - Damage Type (%d): &c%s\r\n", obj->value[3], attack_table[obj->value[3]] );
         ch_printf( ch, "Value[4] - Projectile Type (%d): &c%s\r\n", obj->value[4], projectiles[obj->value[4]] );
         ch_printf( ch, "Projectile type must match on the missileweapon which fires it.\r\n" );
         ch_printf( ch, "Value[5] - Current Condition: &c%d\r\n", obj->value[5] );
         break;

      case ITEM_TRAP:
         ch_printf( ch, "Value[0] - Charges Remaining: &c%d\r\n", obj->value[0] );
/*         ch_printf( ch, "Value[1] - Type (%d): &c%s\r\n", obj->value[1], trap_types[obj->value[1]] ); */
         switch ( obj->value[1] )
         {
            default:
            ...
            case TRAP_TYPE_POISON_GAS:
               ...
               ch_printf( ch, "Casts spell: Poison\r\n" );
               break;
            ...
            case TRAP_TYPE_SEX_CHANGE:
               ...
               ch_printf( ch, "Casts spell: Change Sex\r\n" );
               break;
         }
         ch_printf( ch, "Text Displayed: %s\r\n", buf );
         ch_printf( ch, "Value[3] - Trap Flags (%d): &c", obj->value[3] );
         ch_printf( ch, "%s\r\n", flag_string( obj->value[3], trap_flags ) );
         ch_printf( ch, "Value[4] - Min. Damage: &c%d\r\n", obj->value[4] );
         ch_printf( ch, "Value[5] - Max Damage: &c%d\r\n", obj->value[5] );
         break;

      case ITEM_KEY:
         ch_printf( ch, "Value[0] - Lock #: &c%d\r\n", obj->value[0] );
         ch_printf( ch, "Value[4] - Durability: &c%d\r\n", obj->value[4] );
         ch_printf( ch, "Value[5] - Container Lock Number: &c%d\r\n", obj->value[5] );
         break;
      case ITEM_BLOOD:
         ch_printf( ch, "Value[1] - Amount Remaining: &c%d\r\n", obj->value[1] );
         if( obj->timer )
            ch_printf( ch, "Object Timer, Time Left: %d\r\n", obj->timer );
         break;
   }
...
void do_mstat( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !IS_NPC( victim ) )
      pager_printf_color( ch, "&cAdjective: &C%-20s", victim->adjective );
   ...
   pager_printf_color( ch, "&RMap   : &c%s &w &cCoords: &w( &cX:&w%d&c, Y:&w %d&c, Z: &w%d &c)\r\n",
                      IS_PLR_FLAG( victim, PLR_ONMAP ) ? map_names[victim->map] : "None", victim->x, victim->y, victim->z );
   pager_printf_color( ch, "&cSpeech Color: &w(%d&w) %sSample\r\n", victim->speechcolor, color_str( victim->speechcolor, ch ) );
   pager_printf_color( ch,
...
/*   pager_printf_color( ch, "&cCR: &P%-2d &w(+&P%-2d&w) ", IS_NPC(victim) ? victim->challenge_rating : victim->level/2, IS_NPC(victim) ? challenge_table[victim->challenge_rating].prof : level_prog_table[ch->level].prof_bonus ); */
   if( IS_NPC( victim ) )
      pager_printf_color( ch, "&cCR: &P%-2d &w(+&P%-2d&w) XP: (%-6d) ", victim->challenge_rating, challenge_table[victim->challenge_rating].bonus, challenge_table[victim->challenge_rating].exp );
   else                  
      pager_printf_color( ch, "&cLevel: &w%-2d (+%-2d)           ", victim->level, level_prog_table[victim->level].bonus );
   ...
   ch_printf( ch, "&cSubclasses: &w%s\r\n", ext_flag_string( &victim->subclasses, subclass_flags ) );
   ch_printf( ch, "&cSubrace: &w%s\r\n", ext_flag_string( &victim->subrace, subrace_flags ) );
   
   if ( IS_NPC( victim ) )
   pager_printf_color( ch, "&cType: &w%-2d/%-10s       &cSize: &w%d/%s\r\n",
                      victim->type, npc_type[victim->type], victim->size, size_table[victim->size] );
   ...
   if( IS_VAMPIRE( victim ) && !IS_NPC( victim ) )
   {
      ...
      pager_printf_color( ch, "&cHps     : &w%-12s    &cMana   : &w%-12s    &cMove      : &w%-12s\r\n",
                          hpbuf, mnbuf, mvbuf );
   pager_printf_color( ch, "&cHitroll : &C%-5d           &cAlign  : &w%-5d           &cArmor     : &w%d Base, Actual %d\r\n",
                       GET_HITROLL( victim ), victim->alignment, victim->base_armor, GET_AC( victim ) );
   pager_printf_color( ch, "&cDamroll : &C%-5d           &cWimpy  : &w%-5d           &cPosition  : &w%d\r\n",
                       GET_DAMROLL( victim ), victim->wimpy, victim->position );
   pager_printf_color( ch, "&cNumattacks: &w%-3d   \r\n",
                      victim->numattacks );
   ...
   if( !IS_NPC( victim ) )
      ...
      send_to_pager( "\r\n", ch );
   pager_printf_color( ch, "&cSaving Throws: &w%d %d %d %d %d %d    &cItems: &w(%d/%d)  &cWeight &w(%d/%d)\r\n",
                       victim->saving_str,
                       victim->saving_dex,
                       victim->saving_con,
                       victim->saving_int,
   ...
   if( IS_NPC( victim ) )
   {
      ...
      pager_printf_color( ch, "&cLore       : &w%s\r\n", victim->lore );
   }
   if( !IS_NPC( victim ) && !xIS_EMPTY( victim->feats ) )
      pager_printf_color( ch, "&cFeats      : &w%s\r\n", ext_flag_string( &victim->feats, feat_flags ) ); 
    
   if( !IS_NPC( victim ) && !xIS_EMPTY( victim->abilities ) )
      pager_printf_color( ch, "&cAbilities  : &w%s\r\n", ext_flag_string( &victim->abilities, ability_flags ) );

   if( !IS_NPC( victim ) )
      pager_printf_color( ch, "&cClassValues: &w%d&c/&w%d\r\n", victim->classvalues[0], victim->classvalues[1] );
...
/*
   for( hash = 0; hash < MAX_KEY_HASH; hash++ )
      for( pObjIndex = obj_index_hash[hash]; pObjIndex; pObjIndex = pObjIndex->next )
         ...
         {
            nMatch++;
            pager_printf( ch, "&Y( &w%d&Y) &Y[&w%5d&Y] &G%s", nMatch, pObjIndex->vnum, capitalize( pObjIndex->short_descr ) );
         }*/
   for( iobj = first_object; iobj; iobj = obj->next )
   {
      obj = iobj;
      ...
      nMatch++;
      pager_printf( ch, "&Y( &w%d&Y) &Y[&w%5d&Y] &G%50s", nMatch, obj->pIndexData->vnum, capitalize( obj->short_descr ) );

      if( obj->carried_by )
         pager_printf( ch, " &Cinvent &Y[&w%5d&Y] &C%s\r\n", IS_NPC( obj->carried_by ) ? obj->carried_by->pIndexData->vnum : 0, PERS( obj->carried_by, ch, TRUE ) );
      else if( obj->in_room )
      {
         if( xIS_SET( obj->extra_flags, ITEM_ONMAP ) )
            pager_printf( ch, " &Coverland &Y[&w%s&Y] &C(%d, %d)\r\n", map_names[obj->map], obj->x, obj->y );
         else
            pager_printf( ch, " &Croom &Y[&w%5d&Y] &C%s\r\n", obj->in_room->vnum, obj->in_room->name );
      }
      else
         pager_printf( ch, "\r\n" );
   }
   ...
   if( deity || !str_cmp( arg, "all" ) )
   {
      ...
      for( vch = first_char; vch; vch = vch_next )
      {
         ...
         if( !IS_NPC( vch ) && !IS_IMMORTAL( vch ) && !CAN_PKILL( vch ) && !in_arena( vch ) )
         {
            ...
            vch->classvalues[0] = vch->classvalues[1];

            for ( x = 0; x < MAX_PER_SLOT; x++ )
            {
               for ( y = 0; y < max_spell_slot_lookup( vch, x ); y++ )
               {
                  if ( vch->pcdata->slots[x][y].free == 0 )
                  {
/*                        ch_printf( ch, "Empty slot found. [%d][%d], ticks remaining until refreshed: %d\r\n", x, y, ch->pcdata->slots[x][y].ticks ); */
                     if ( vch->pcdata->slots[x][y].ticks > 0 )
                        vch->pcdata->slots[x][y].ticks = 0;
                     if ( vch->pcdata->slots[x][y].ticks == 0 )
                     {
                        /* ch_printf( ch, "Spell slot refreshed: Level %d, slot %d.\r\n", x, y ); */
                        vch->pcdata->slots[x][y].free = 1;
                     }
                  }
               }
            }
   ...
   else
   {
      ...
      if( victim->pcdata )
         ...
         {            
               for ( x = 0; x < MAX_PER_SLOT; x++ )
                  for ( y = 0; y < max_spell_slot_lookup( victim, x ); y++ )
                  {
                     if ( victim->pcdata->slots[x][y].free == 0 )
                     {
/*                        ch_printf( ch, "Empty slot found. [%d][%d], ticks remaining until refreshed: %d\r\n", x, y, ch->pcdata->slots[x][y].ticks ); */
                        if ( victim->pcdata->slots[x][y].ticks > 0 )
                           --victim->pcdata->slots[x][y].ticks;
                        if ( victim->pcdata->slots[x][y].ticks == 0 )
                        {
                           send_to_char( "You feel your spell slots refresh.\r\n", victim );
                           /* ch_printf( ch, "Spell slot refreshed: Level %d, slot %d.\r\n", x, y ); */
                           victim->pcdata->slots[x][y].free = 1;
                        }
                     }
                  }
         }
...
void do_hell( CHAR_DATA* ch, const char* argument)
{
   ...
   ch_printf( ch, "&R%s will be released from Carceri at %24.24s.\r\n", victim->name, ctime( &victim->pcdata->release_date ) );
   act( AT_MAGIC, "&R$n suddenly disappears in a cloud of hellish red light.", victim, NULL, ch, TO_NOTVICT );
   char_from_room( victim );
   char_to_room( victim, get_room_index( ROOM_VNUM_HELL ) );
   act( AT_MAGIC, "&R$n appears in a cloud of hellish red light.", victim, NULL, ch, TO_NOTVICT );
   do_look( victim, "auto" );
   if( !victim->desc )
      add_loginmsg( victim->name, 10, NULL );
   else
      ch_printf( victim, "&RThe gods are not pleased with your actions.\r\n"
              "&rYou shall remain in Carceri for %d %s%s.\r\n", htime, ( h_d ? "hour" : "day" ), ( htime == 1 ? "" : "s" ) );
...
void do_unhell( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !*arg )
   {
      send_to_char( "Remove whom from Carceri?\r\n", ch );
      return;
   }
   location = ch->in_room;
...
/*ch->in_room = location;          The case of unhell self, etc.*/
   ...
   {
      send_to_char( "No one like that is in Carceri.\r\n", ch );
      return;
   }
   ...
   if( !location )
      location = ch->in_room;
   MOBtrigger = FALSE;
   act( AT_MAGIC, "&Y$n disappears in a cloud of godly light.", victim, NULL, ch, TO_NOTVICT );
   char_from_room( victim );
   char_to_room( victim, location );
   send_to_char( "&YThe gods have smiled on you and released you from Carceri early!\r\n", victim );
   do_look( victim, "auto" );
   if( victim != ch )
      send_to_char( "&YThey have been released.\r\n", ch );
   ...
   MOBtrigger = FALSE;
   act( AT_MAGIC, "&Y$n suddenly appears in a cloud of godly light.", victim, NULL, ch, TO_NOTVICT );
...
void do_showclass( CHAR_DATA* ch, const char* argument)
{
   ...
   pager_printf_color( ch, "&wCLASS: &W%s\r\n&wSaving Throw Attribute 1: &W%-14s  &wWeapon: &W%-5d      &wGuild: &W%-5d\r\n",
                       Class->who_name, affect_loc_name( Class->saving_throw_one ), Class->weapon, Class->guild );
   pager_printf_color( ch, "&wSaving Throw Attribute 2: &W%-14s  \r\n&wSpell Save DC/Spell Attack Mod:  &W%-14s\r\n",
                       affect_loc_name( Class->saving_throw_two ), affect_loc_name( Class->spell_mod_stat ) );
   pager_printf_color( ch, "&wHp Min/Hp Max  : &W%-2d/%-2d\r\n", 
                       Class->hp_min, Class->hp_max  );
   pager_printf_color( ch, "&wAffected by:  &W%s\r\n", affect_bit_name( &Class->affected ) );
   pager_printf_color( ch, "&wResistant to: &W%s\r\n", ext_flag_string( &Class->resist, ris_flags ) );
   pager_printf_color( ch, "&wSusceptible to: &W%s\r\n", ext_flag_string( &Class->suscept, ris_flags ) );
   pager_printf_color( ch, "&wCaster Type: &W" );
   if( Class->caster_type == 0 ) pager_printf_color( ch, "%s\r\n", "Full" );
   else if( Class->caster_type == 1 ) pager_printf_color( ch, "%s\r\n", "Half" );
   else if( Class->caster_type == 2 ) pager_printf_color( ch, "%s\r\n", "1/3rd" );
   else pager_printf_color( ch, "%s\r\n", "None" );
   
   pager_printf_color( ch, "&wSubclasses: &W%s\r\n", ext_flag_string( &Class->subclass, subclass_flags ) );
...
bool create_new_class( int rcindex, const char *argument )
{
   ...
   class_table[rcindex]->caster_type = -1;
   class_table[rcindex]->saving_throw_one = 0;
   class_table[rcindex]->saving_throw_two = 0;
   class_table[rcindex]->spell_mod_stat = 0;
...
void do_setclass( CHAR_DATA* ch, const char* argument)
{
   ...
   if( arg1[0] == '\0' )
   {
      ...
      send_to_char( "\r\nField being one of:\r\n", ch );
      send_to_char( "  name savingone weapon guild thac0 thac32\r\n", ch );
      send_to_char( "  hpmin hpmax mana expbase mtitle ftitle\r\n", ch );
      send_to_char( "  savingtwo spellmodstat affected resist suscept skill cast_type\r\n", ch );
      send_to_char( "  subclass\r\n", ch );
      return;
   }
   if( is_number( arg1 ) && ( cl = atoi( arg1 ) ) >= 0 && cl < MAX_CLASS )
      Class = class_table[cl];
   ...
   if( !str_cmp( arg2, "subclass" ) )
   {
      if( !argument || argument[0] == '\0' )
      {
         send_to_char( "Usage: setclass <class> subclass <flag> [flag]...\r\n", ch );
         return;
      }
      while( argument[0] != '\0' )
      {
         argument = one_argument( argument, arg2 );
         value = get_subclassflag( arg2 );
         if( value < 0 || value >= MAX_BITS )
            ch_printf( ch, "Unknown flag: %s\r\n", arg2 );
         value = get_subclassflag( arg2 );
         xTOGGLE_BIT( Class->subclass, value );
      }
      send_to_char( "Done.\r\n", ch );
      write_class_file( cl );
      return;
   }
   if( !str_cmp( arg2, "savingtwo" ) )
   {
      int x = get_atype( argument );

      if( x < APPLY_NONE || ( x > APPLY_CON && x != APPLY_LCK && x != APPLY_CHA ) )
         send_to_char( "Invalid secondary save stat!\r\n", ch );
      else
      {
         Class->saving_throw_two = x;
         send_to_char( "Secondary save attribute set.\r\n", ch );
         write_class_file( cl );
      }
      return;
   }

   if( !str_cmp( arg2, "spellmodstat" ) )
   {
      int x = get_atype( argument );

      if( x < APPLY_NONE || ( x > APPLY_CON && x != APPLY_LCK && x != APPLY_CHA ) )
         send_to_char( "Invalid spell modifier attribute!\r\n", ch );
      else
      {
         Class->spell_mod_stat = x;
         send_to_char( "Spell modifier attribute set.\r\n", ch );
         write_class_file( cl );
      }
      return;
   }

   if( !str_cmp( arg2, "savingone" ) )
   {
      int x = get_atype( argument );

      if( x < APPLY_NONE || ( x > APPLY_CON && x != APPLY_LCK && x != APPLY_CHA ) )
         send_to_char( "Invalid primary save attribute!\r\n", ch );
      else
      {
         Class->saving_throw_one = x;
         send_to_char( "Primary save attribute set.\r\n", ch );
         write_class_file( cl );
      }
      return;
   }
   ...
   if( !str_cmp( arg2, "casttype" ) )
   {
      if( !argument || argument[0] == '\0' )
      {
         send_to_char( "Usage: setclass <class> casttype <#>, where type is FULL(0), HALF(1), or THIRD(2)\r\n", ch );
         return;
      }

      Class->caster_type = atoi( argument );
      write_class_file( cl );
      if ( Class->caster_type == 0 )
         send_to_char( "Caster Type set to Full\r\n", ch );
      else if ( Class->caster_type == 1 )
         send_to_char( "Caster Type set to Half\r\n", ch );
      else if ( Class->caster_type == 2 )
         send_to_char( "Caster Type set to Third\r\n", ch );
      return;      
   }
...
bool create_new_race( int rcindex, char *argument )
{
   ...
   snprintf( race_table[rcindex]->race_name, 17, "%-.16s", argument );
...
void do_setrace( CHAR_DATA* ch, const char* argument)
{
   ...
   if( arg1[0] == '\0' || arg2[0] == '\0' )
   {
      ...
      send_to_char( "  saving_str saving_dex saving_cha      \r\n", ch );
      send_to_char( "  saving_con saving_wis saving_int     \r\n", ch );
      send_to_char( "  race_recall       \r\n", ch );
      send_to_char( "  mana_regen hp_regen size             \r\n", ch );
      return;
   }
   ...
   if( !str_cmp( arg2, "name" ) )
   {
      ...
      snprintf( race->race_name, 32, "%s", capitalize( argument ) );
      write_race_file( ra );
      for( i = 0; i < MAX_PC_RACE; ++i )
         fprintf( fpList, "%s.race\n", race_table[i]->race_name );
      fprintf( fpList, "%s", "$\n" );
   ...
   {
      race->speed = ( short )atoi( argument );
      write_race_file( ra );
      send_to_char( "Done.\r\n", ch );
      return;
   }
   ...
   if( !str_cmp( arg2, "suscept" ) )
   {
      ...
      write_race_file( ra );
      send_to_char( "Racial susceptibilities set.\r\n", ch );
      return;
   }
   ...
   if( !str_cmp( arg2, "saving_con" ) )
   {
      race->saving_con = atoi( argument );
      write_race_file( ra );
      send_to_char( "Done.\r\n", ch );
      return;
   }
   if( !str_cmp( arg2, "saving_wis" ) )
   {
      race->saving_wis = atoi( argument );
      write_race_file( ra );
      send_to_char( "Done.\r\n", ch );
      return;
   }
   if( !str_cmp( arg2, "saving_dex" ) )
   {
      race->saving_dex = atoi( argument );
      write_race_file( ra );
      send_to_char( "Done.\r\n", ch );
      return;
   }
   if( !str_cmp( arg2, "saving_str" ) )
   {
      race->saving_str = atoi( argument );
      write_race_file( ra );
      send_to_char( "Done.\r\n", ch );
      return;
   }
   if( !str_cmp( arg2, "saving_int" ) )
   {
      race->saving_int = atoi( argument );
      write_race_file( ra );
      send_to_char( "Done.\r\n", ch );
      return;
   }
   
   if( !str_cmp( arg2, "saving_cha" ) )
   {
      race->saving_cha = atoi( argument );
      write_race_file( ra );
      send_to_char( "Done.\r\n", ch );
      return;
   }
...
void do_showrace( CHAR_DATA* ch, const char* argument)
{
   ...
   ch_printf( ch, "Hit Pts:  %-3d\tSpeed: %-3d\tAlign: %-4d\tAC: %-d\r\n",
              race->hit, race->speed, race->alignment, race->ac_plus );
   ch_printf( ch, "Min Align: %d\tMax Align: %-d\tXP Mult: %-d%%\tSize: %-d\r\n",
              race->minalign, race->maxalign, race->exp_multiplier, race->size );
   ...
   ch_printf( ch, "Saves: (Str) %d (Dex) %d (Con) %d (Int) %d (Wis) %d (Cha) %d\r\n",
              race->saving_str, race->saving_dex, race->saving_con,
              race->saving_int, race->saving_wis, race->saving_cha );
...
void do_message( CHAR_DATA *ch, const char *argument )
{
   ...
   if( argument[0] == '\0' )
   {
      send_to_char( "OOC: Leave a login message for which of your faithful?\r\n", ch );
      return;
   }
   ...
   if( !str_cmp( name, "list" ) && get_trust( ch ) >= LEVEL_GREATER )
   {
      ...
      {
         ch_printf_color( ch, "&CFaithful: &c%-20s &CType: &c%d\r\n", capitalize(lmsg->name), lmsg->type );

         if( lmsg->text )
            ch_printf_color( ch, "&CMessage:\r\n  &c%s\r\n", lmsg->text );
   ...
   if( access( checkname, F_OK ) == 0 )
   {
      ...
      for( temp = first_char; temp; temp = temp->next )
      {
         ...
         {
            send_to_char( "OOC: They are online. Use a different method to contact them.\r\n", ch );
            return;
         }
      }
      ...
      {
         send_to_char( "Send them what divine message?\r\n", ch );
         return;
      }

      add_loginmsg( name, type, argument );
      ch_printf( ch, "OOC: You have sent %s this divine message for when they login next:\r\n", capitalize(name) );
   ...
   {
      send_to_char( "That faithful does not exist.\r\n", ch );
      return;
   }
}
