...
void get_obj( CHAR_DATA * ch, OBJ_DATA * obj, OBJ_DATA * container )
{
    ...
    */
   if( obj->item_type != ITEM_GOLD || obj->item_type != ITEM_SILVER || obj->item_type != ITEM_COPPER )
   {
      if( obj->in_obj )
      {
...
void do_get( CHAR_DATA* ch, const char* argument)
{
   ...
   if( arg2[0] == '\0' )
   {
      if( number <= 1 && str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
      {
         ...
         if( ch->fighting )
         {
            if( ch->bonus_action == FALSE )
            {
               ch_printf( ch, "%s lunges forward, taking advantage of the opportunity!\r\n", PERS( ch->fighting->who, ch, FALSE ) );
               one_hit( ch->fighting->who, ch, TYPE_UNDEFINED );
            }
            else
...
bool remove_obj( CHAR_DATA * ch, int iWear, bool fReplace )
{
   ...
   if( obj == get_eq_char( ch, WEAR_MAIN_HAND ) && ( tmpobj = get_eq_char( ch, WEAR_OFF_HAND ) ) != NULL )
      tmpobj->wear_loc = WEAR_MAIN_HAND;

   if( obj == get_eq_char( ch, WEAR_WAIST ) && ( tmpobj = get_eq_char( ch, WEAR_BELT_A ) ) != NULL )
      unequip_char( ch, tmpobj );
      
   if( obj == get_eq_char( ch, WEAR_WAIST ) && ( tmpobj = get_eq_char( ch, WEAR_BELT_B ) ) != NULL )
      unequip_char( ch, tmpobj );
   unequip_char( ch, obj );
...
void wear_obj( CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace, short wear_bit )
{
   ...
   if( !IS_IMMORTAL( ch )
       && ( ( IS_OBJ_STAT( obj, ITEM_ANTI_WIZARD ) && ch->Class == CLASS_WIZARD )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_CLERIC ) && ch->Class == CLASS_CLERIC )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_FIGHTER ) && ch->Class == CLASS_FIGHTER )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_PALADIN ) && ch->Class == CLASS_PALADIN )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_RANGER ) && ch->Class == CLASS_RANGER )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_DRUID ) && ch->Class == CLASS_DRUID )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_ROGUE ) && ch->Class == CLASS_ROGUE )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_MONK ) && ch->Class == CLASS_MONK )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_SORCERER ) && ch->Class == CLASS_SORCERER )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_WARLOCK ) && ch->Class == CLASS_WARLOCK )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_BARBARIAN ) && ch->Class == CLASS_BARBARIAN )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_BARD ) && ch->Class == CLASS_BARD )
   ...
   {
      send_to_char( "That item is too large for you to use properly.\r\n", ch );
      return;
   }
   
   if( obj->size < ch->size-1 )
   {
      send_to_char( "That item is too small for you to use properly.\r\n", ch );
      return;
   }
   ...
   if( obj->item_type == ITEM_LIGHT )
   {
      if( get_eq_char( ch, WEAR_MAIN_HAND )
         && get_eq_char( ch, WEAR_OFF_HAND )
         && !remove_obj( ch, WEAR_MAIN_HAND, fReplace ) && !remove_obj( ch, WEAR_OFF_HAND, fReplace ) )
         return;

      if( get_eq_char( ch, WEAR_TWOHAND ) )
      {
      ...
      {
         act( AT_ACTION, "$n holds $p in your off hand.", ch, obj, NULL, TO_ROOM );
         act( AT_ACTION, "You hold $p in your off hand.", ch, obj, NULL, TO_CHAR );
      }
      equip_char( ch, obj, WEAR_OFF_HAND );
   ...
   switch ( 1 << bit )
   {
      ...
      case ITEM_WEAR_BELT:
         if( get_eq_char( ch, WEAR_BELT_A )
             && get_eq_char( ch, WEAR_BELT_B )
             && !remove_obj( ch, WEAR_BELT_A, fReplace ) && !remove_obj( ch, WEAR_BELT_B, fReplace ) )
            return;

         if( !get_eq_char( ch, WEAR_WAIST ) )
         {
            send_to_char( "You aren't even wearing a belt!\r\n", ch );
            return;
         }
         ...
         if( !get_eq_char( ch, WEAR_BELT_A ) )
         {
            if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
            {
               act( AT_ACTION, "$n affixes $p onto $s belt.", ch, obj, NULL, TO_ROOM );
               act( AT_ACTION, "You affix $p onto your belt.", ch, obj, NULL, TO_CHAR );
            }
            equip_char( ch, obj, WEAR_BELT_A );
            oprog_wear_trigger( ch, obj );
            return;
         }

         if( !get_eq_char( ch, WEAR_BELT_B ) )
         {
            if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
            {
               act( AT_ACTION, "$n wears $p on $s belt.", ch, obj, NULL, TO_ROOM );
               act( AT_ACTION, "You wear $p on your belt.", ch, obj, NULL, TO_CHAR );
            }
            equip_char( ch, obj, WEAR_BELT_B );
            oprog_wear_trigger( ch, obj );
            return;
         }

         bug( "%s: no free belt slot.", __func__ );
         send_to_char( "You already wear two items on your belt.\r\n", ch );
         return;

      case ITEM_WEAR_ONEHAND:
         if( get_eq_char( ch, WEAR_MAIN_HAND )
             && get_eq_char( ch, WEAR_OFF_HAND )
             && !remove_obj( ch, WEAR_MAIN_HAND, fReplace ) && !remove_obj( ch, WEAR_OFF_HAND, fReplace ) )
            return;
         ...
         if( !get_eq_char( ch, WEAR_MAIN_HAND ) )
         {
            if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
            {
               act( AT_ACTION, "$n holds $p in $s main hand.", ch, obj, NULL, TO_ROOM );
               act( AT_ACTION, "You hold $p in your main hand.", ch, obj, NULL, TO_CHAR );
            }
         ...
         if( !get_eq_char( ch, WEAR_OFF_HAND ) )
         {
            if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
            {
               act( AT_ACTION, "$n holds $p in $s off hand.", ch, obj, NULL, TO_ROOM );
               act( AT_ACTION, "You hold $p in your off hand.", ch, obj, NULL, TO_CHAR );
            }
            equip_char( ch, obj, WEAR_OFF_HAND );
            oprog_wear_trigger( ch, obj );
         ...
         bug( "%s: no free hand slot.", __func__ );
         send_to_char( "Your hands are full.\r\n", ch );
         return;         

         
      case ITEM_WEAR_TWOHAND:
         if( get_eq_char( ch, WEAR_MAIN_HAND ) || get_eq_char( ch, WEAR_OFF_HAND ) )
         {
            send_to_char( "You don't have a free hand.\r\n", ch );
            return;
         }
         ...
         if( get_eq_char( ch, WEAR_TWOHAND ) )
         {
            send_to_char( "Both your hands are full.\r\n", ch );
            return;
         }
         
         if ( !remove_obj( ch, WEAR_TWOHAND, fReplace ) )
            return;
         
         if( !oprog_use_trigger( ch, obj, NULL, NULL ) )
         {
            act( AT_ACTION, "$n takes up $p in both hands.", ch, obj, NULL, TO_ROOM );
            act( AT_ACTION, "You take up $p in both hands.", ch, obj, NULL, TO_CHAR );
         }
         equip_char( ch, obj, WEAR_TWOHAND );
         oprog_wear_trigger( ch, obj );
         return;
   }
}
...
*/ int dam = fall_count * obj->weight / 2;
         ...
         if( obj->in_room->first_person && number_percent(  ) > 15 )
         {
            ...
            if( IS_NPC( vch ) && xIS_SET( vch->act, ACT_HARDHAT ) )
               ...
               damage( vch, vch, dam * vch->level, TYPE_UNDEFINED, FALSE );
         }
         /*
          * Damage objects 
          */
