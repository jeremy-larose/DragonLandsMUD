...
//   "map1.raw", "map2.raw", "map3.raw", "dloc.raw"
//      "map1.png"
//	"map2.png"
//	"march2020.png"
//	"april2020.png"
...
/*   Sector Type		Color	Symbol Description	Passable?	Move  R  G  B	*/
   ...
   {SECT_SWAMP, "&z", "~", "swamp", TRUE, 3, 218, 176, 56},
...
void fread_entrance( ENTRANCE_DATA * enter, FILE * fp )
{
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'D':
            KEY( "Description", enter->description, fread_string( fp ) );
            break;
...
void modify_entrance( ENTRANCE_DATA * enter, short tomap, short onmap, short hereX, short hereY, short thereX, short thereY,
                      int vnum, const char *area, const char *description )
{
   if( !enter )
   {
   ...
   if( description )
   {
      STRFREE( enter->description );
      enter->description = STRALLOC( description );
   }
...
void do_showexit( CHAR_DATA *ch, const char *argument )
{
   ...
   if( IS_NPC( ch ) )
   {
      send_to_char( "Sorry, NPCs can't see the exit data.\r\n", ch );
      return;
   }
   ...
   if( enter )
   {
      send_to_char( "Exit Data:\r\n", ch );
      ch_printf( ch, "Exit Vnum: %d\r\n", enter->vnum );
      ch_printf( ch, "Exit ThereX,Y: (%d, %d)\r\n", enter->therex, enter->therey );
      ch_printf( ch, "Exit Area: %s\r\n", enter->area );
      ch_printf( ch, "Exit Description: %s\r\n", enter->description );
      return;
   }
   else
   {
      send_to_char( "No Exit found.\r\n", ch );
      return;
   }
...
void do_setexit( CHAR_DATA * ch, const char *argument )
{
   ...
   if( arg[0] == '\0' || !str_cmp( arg, "help" ) )
   {
      ...
      send_to_char( "Usage: setexit desc <description>\r\n", ch );
      return;
   }
   ...
   if( !str_cmp( arg, "map" ) )
   {
      ...
      modify_entrance( enter, map, ch->map, ch->x, ch->y, x, y, -1, NULL, NULL );
      putterr( ch->map, ch->x, ch->y, SECT_EXIT );
   ...
   if( !str_cmp( arg, "area" ) )
   {
      ...
      modify_entrance( enter, enter->onmap, ch->map, ch->x, ch->y, ch->x, ch->y, enter->vnum, argument, enter->description );
   ...
   if( !str_cmp( arg, "desc"  ) )
   {
      ...
      {
         STRFREE( enter->description );
         enter->description = STRALLOC( argument );
      }
      else
      enter->description = STRALLOC( argument );
      ch_printf( ch, "Exit description created: %s\r\n", argument );
      save_entrances( );
      return;
   }
   ...
   if( !str_cmp( arg, "vnum" ) )
   {
      ...
      modify_entrance( enter, -1, ch->map, ch->x, ch->y, -1, -1, vnum, NULL, NULL );
      putterr( ch->map, ch->x, ch->y, SECT_EXIT );
...
void display_map( CHAR_DATA * ch )
{
   ...
   else
   {
      ...
      if( time_info.hour == HOUR_SUNRISE || time_info.hour == HOUR_SUNSET )
         mod = 4;
/* Not compatible with new weather system
...
*/
      ...
      startx = ( short )( UMAX( ch->x - ( mod * 1.5 ), ch->x - 19 ) );
      starty = ( short )( UMAX( ch->y - mod, ch->y - 8 ) );
      endx = ( short )( UMIN( ch->x + ( mod * 1.5 ), ch->x + 19 ) );
      endy = ( short )( UMIN( ch->y + mod, ch->y + 8 ) );
   }
...
#endif
   {
      ch_printf( ch, "&GYou are traveling through the realm of %s.\r\n", map_names[ch->map] );
      landmark = check_landmark( ch->map, ch->x, ch->y );
      ...
      else if( sector == SECT_EXIT )
      {
         ...
         if( enter && enter->description != NULL )
         ch_printf( ch, "&G%s\r\n", enter->description );
      }
      else
...
void check_random_mobs( CHAR_DATA * ch )
{
   ...
   /* BUG: For some reason, calling get_mob_index here sometimes crushes vnum and makes it 1.6bil -Seikyr 11/18 Overland Code */
...
/*      log_printf( "DEBUG: terrain = %d", terrain );
      log_printf( "DEBUG: vnum = %d", vnum );
      log_printf( "DEBUG: UMIN( ) = %d", UMIN( 25, number_range( 0, ch->level / 2 ) ) );
      log_printf( "DEBUG: Random Mobs = %d", random_mobs[terrain][UMIN( 25, number_range( 0, ch->level / 2 ) ) ] );
      log_printf( "DEBUG: imob = %d", imob->vnum );*/
      /* log_string( log_buf ); */
      return;
   }
   ...
   mob->sector = terrain;
   mob->map = ch->map;
   mob->x = ( ch->x + number_range( -10, 10 ) );
   mob->y = ( ch->y + number_range( -10, 10 ) );
   mob->timer = 400;
   /*
...
/* Added dir-2 check here to make sure you don't automatically enter exits when entering the room.
 * Hopefully doesn't break stuff? -- Seikyr 11/18 Overland Map */
   if( sector == SECT_EXIT && dir == -2 ) 
   {
      ENTRANCE_DATA *enter;
...
/*         case ACON_C2:
   ...
   original = ch->in_room;
   act( AT_ACTION, "$n arrives into the wilderness.", ch, NULL, NULL, TO_NOTVICT );
   char_from_room( ch );
   char_to_room( ch, maproom );
   act( AT_ACTION, "$n arrives in the wilderness.", ch, NULL, NULL, TO_ROOM );
...
void leave_map( CHAR_DATA * ch, CHAR_DATA * victim, ROOM_INDEX_DATA * target )
{
   ...
   if( target != NULL )
   {
      ...
      act( AT_ACTION, "$n arrives from the wilderness.", ch, NULL, NULL, TO_NOTVICT );
      fix_maps( victim, ch );
      collect_followers( ch, from, target );
      do_look( ch, "auto" );
      /* Might make big bugs here on entry -- Seikyr 11/18 Overland */
      if( ch->in_room->first_content )
         check_room_for_traps( ch, TRAP_ENTER_ROOM );
      mprog_entry_trigger( ch );
      rprog_enter_trigger( ch );
      mprog_greet_trigger( ch );
      oprog_greet_trigger( ch );
...
const char *bamf_print( char *fmt, CHAR_DATA * ch )
{
