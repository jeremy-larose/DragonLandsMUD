...
void fwrite_morph( FILE * fp, MORPH_DATA * morph )
{
   ...
   if( morph->parry != 0 )
      fprintf( fp, "Parry        	%d\n", morph->parry );
   if( morph->saving_str != 0 )
      fprintf( fp, "SaveStr      	%d\n", morph->saving_str );
   if( morph->saving_dex != 0 )
      fprintf( fp, "SaveDex        	%d\n", morph->saving_dex );
   if( morph->saving_con != 0 )
      fprintf( fp, "SaveCon      	%d\n", morph->saving_con );
   if( morph->saving_int != 0 )
      fprintf( fp, "SaveInt       	%d\n", morph->saving_int );
   if( morph->saving_wis != 0 )
      fprintf( fp, "SaveWis        	%d\n", morph->saving_wis );
   if( morph->saving_cha != 0 )
      fprintf( fp, "SaveCha		%d\n", morph->saving_cha );
   if( morph->str != 0 )
...
void do_morphset( CHAR_DATA* ch, const char* argument)
{
   ...
   else if( !str_cmp( arg2, "sav1" ) || !str_cmp( arg2, "savestr" ) )
   {
      ...
      morph->saving_con = value;
   }
   else if( !str_cmp( arg2, "sav2" ) || !str_cmp( arg2, "savedex" ) )
   {
      if( value < -30 || value > 30 )
      {
         send_to_char( "Saving throw range is -30 to 30.\r\n", ch );
      ...
      morph->saving_wis = value;
   }
   else if( !str_cmp( arg2, "sav3" ) || !str_cmp( arg2, "savecon" ) )
   {
      if( value < -30 || value > 30 )
      {
         send_to_char( "Saving throw range is -30 to 30.\r\n", ch );
      ...
      morph->saving_dex = value;
   }
   else if( !str_cmp( arg2, "sav4" ) || !str_cmp( arg2, "saveint" ) )
   {
      if( value < -30 || value > 30 )
      {
         send_to_char( "Saving throw range is -30 to 30.\r\n", ch );
      ...
      morph->saving_str = value;
   }
   else if( !str_cmp( arg2, "sav5" ) || !str_cmp( arg2, "savewis" ) )
   {
      if( value < -30 || value > 30 )
      {
         send_to_char( "Saving throw range is -30 to 30.\r\n", ch );
      ...
      morph->saving_int = value;
   }
   else if( !str_cmp( arg2, "sav6" ) || !str_cmp( arg2, "savecha" ) )
   {
      if( value < -30 || value > 30 )
      {
         send_to_char( "Saving throw range is -30 to 30.\r\n", ch );
...
void do_morphstat( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !argument || argument[0] == '\0' )
   {
      ...
      pager_printf( ch, "  &cSaving Throws: &w%d %d %d %d %d %d      &cDodge: &w%d  &cParry: &w%d  &cTumble: &w%d\r\n",
                    morph->saving_str, morph->saving_dex, morph->saving_con, morph->saving_int,
                    morph->saving_wis, morph->saving_cha, morph->dodge, morph->parry, morph->tumble );
      ...
      pager_printf( ch, "  &cImmune     : &w%s\r\n", ext_flag_string( &morph->immune, ris_flags ) );
      pager_printf( ch, "  &cSusceptible: &w%s\r\n", ext_flag_string( &morph->suscept, ris_flags ) );
      pager_printf( ch, "  &cResistant  : &w%s\r\n", ext_flag_string( &morph->resistant, ris_flags ) );
      ...
      pager_printf( ch, "  &cNot Immune     : &w%s\r\n", ext_flag_string( &morph->no_immune, ris_flags ) );
      pager_printf( ch, "  &cNot Susceptible: &w%s\r\n", ext_flag_string( &morph->no_suscept, ris_flags ) );
      pager_printf( ch, "  &cNot Resistant  : &w%s\r\n", ext_flag_string( &morph->no_resistant, ris_flags ) );
...
CHAR_MORPH *make_char_morph( MORPH_DATA * morph )
{
   ...
   ch_morph->saving_str = morph->saving_str;
   ch_morph->saving_dex = morph->saving_dex;
   ch_morph->saving_con = morph->saving_con;
   ch_morph->saving_int = morph->saving_int;
   ch_morph->saving_wis = morph->saving_wis;
...
void do_morph( CHAR_DATA * ch, MORPH_DATA * morph )
{
   ...
   ch->saving_str += morph->saving_str;
   ch->saving_dex += morph->saving_dex;
   ch->saving_con += morph->saving_con;
   ch->saving_int += morph->saving_int;
   ch->saving_wis += morph->saving_wis;
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'R':
            ...
            break;
         case 'S':
            KEY( "SaveStr", morph->saving_str, fread_number( fp ) );
            KEY( "SaveDex", morph->saving_dex, fread_number( fp ) );
            KEY( "SaveCon", morph->saving_con, fread_number( fp ) );
            KEY( "SaveInt", morph->saving_int, fread_number( fp ) );
            KEY( "SaveWis", morph->saving_wis, fread_number( fp ) );
	    KEY( "SaveCha", morph->saving_cha, fread_number( fp ) );
...
void copy_morph( MORPH_DATA * morph, MORPH_DATA * temp )
{
   ...
   morph->saving_str = temp->saving_str;
   morph->saving_dex = temp->saving_dex;
   morph->saving_con = temp->saving_con;
   morph->saving_int = temp->saving_int;
   morph->saving_wis = temp->saving_wis;
   morph->saving_cha = temp->saving_cha;
   morph->str = temp->str;
