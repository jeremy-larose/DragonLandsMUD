...
SPEC_DATA *first_spec;
SPEC_DATA *last_spec;
void fread_spec( SPEC_DATA * spec, FILE * fp );
bool load_spec_file( const char *specfile );
void write_spec_list( void );

void free_specfuns( void )
{
   SPEC_DATA *specfun, *next_specfun;

   for( specfun = first_spec; specfun; specfun = next_specfun )
   {
      next_specfun = specfun->next;

      UNLINK( specfun, first_spec, last_spec, next, prev );
      DISPOSE( specfun->name );
      DISPOSE( specfun->filename );
      DISPOSE( specfun->description );
      DISPOSE( specfun->deathmsg );
      DISPOSE( specfun );
   }
}

void write_spec_list(  )
{
   SPEC_DATA *tspec;
   FILE *fpout;
   char filename[256];

   snprintf( filename, 256, "%s%s", SPEC_DIR, SPEC_LIST );
   ...
   for( tspec = first_spec; tspec; tspec = tspec->next )
      fprintf( fpout, "%s\n", tspec->filename );
   fprintf( fpout, "$\n" );
   fclose( fpout );
}
...
SPEC_DATA *get_spec( const char *name )
{
   ...
   for( spec = first_spec; spec; spec = spec->next )
      if( !str_cmp( name, spec->name ) )
         return spec;
   return NULL;
}

/*
 * Load in all the spec files.
 */
void load_specs(  )
{
   FILE *fpList;
   ...
   log_string( "Loading specs..." );

   snprintf( speclist, 256, "%s%s", SPEC_DIR, SPEC_LIST );
   if( ( fpList = fopen( speclist, "r" ) ) == NULL )
   {
      perror( speclist );
      exit( 1 );
   }
   ...
   for( ;; )
   {
      ...
      if( !load_spec_file( filename ) )
      {
         bug( "%s: Cannot load spec file: %s", __func__, filename );
      }
   }
   fclose( fpList );
   log_string( " Done specs " );
   return;
}
...
bool load_spec_file( const char *specfile )
{
   ...
   CREATE( spec, SPEC_DATA, 1 );

   /*
    * Make sure we default these to 1 --Seikyr
    */ 
   spec->blood = 1;
   spec->corpse = 1;
   found = FALSE;
   snprintf( filename, 256, "%s%s", SPEC_DIR, specfile );
   ...
   if( found )
      LINK( spec, first_spec, last_spec, next, prev );
   else
      DISPOSE( spec );
...
void fread_spec( SPEC_DATA * spec, FILE *fp )
{
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'A':
            if( !str_cmp( word, "Attack" ) )
            {
               ...
               CREATE( attack, ATTACK_DATA, 1 );
                  attack->sn = sn;
                  attack->minlevel = fread_number( fp );
                  attack->uses = fread_number( fp );
               LINK( attack, spec->first_attack, spec->last_attack, next, prev );
               fMatch = TRUE;
               break;
            } 
            break;
         ...
         case 'B':
            KEY( "Blood", spec->blood, fread_number( fp ) );
            break;

         case 'C':
            KEY( "Corpse", spec->corpse, fread_number( fp ) );
            break;

         case 'D':
            KEY( "Deathmsg", spec->deathmsg, fread_string( fp ) );
            KEY( "Description", spec->description, fread_string( fp ) );
            break;
         ...
         case 'E':
            if( !str_cmp( word, "End" ) )
            {
               if( !spec->name )
                  spec->name = STRALLOC( "" );
               if( !spec->description )
                  spec->description = STRALLOC( "" );
               if( !spec->deathmsg )
                  spec->deathmsg = STRALLOC( "" );
               return;
            }
            break;

         case 'F':
            KEY( "Filename", spec->filename, fread_string_nohash( fp ) );
            break;

         case 'N':
            KEY( "Name", spec->name, fread_string( fp ) );
            break;
      }

      if( !fMatch )
      {
         bug( "Fread_spec: no match: %s", word );
         fread_to_eol( fp );
      }
   }
}

void save_spec( SPEC_DATA * spec )
{
   FILE *fp;
   char filename[256];
   ...
   if( !spec )
   {
      bug( "%s: %s null spec pointer!", __func__, spec->name );
      return;
   }

   if( !spec->filename || spec->filename[0] == '\0' )
   {
      bug( "%s: %s has no filename", __func__, spec->name );
      return;
   }

   snprintf( filename, 256, "%s%s", SPEC_DIR, spec->filename );

   if( ( fp = fopen( filename, "w" ) ) == NULL )
   {
      bug( "save_spec: cant open %s", filename );
      perror( filename );
   }
   else
   {
      ...
      fprintf( fp, "Corpse       %d\n", spec->corpse );
      fprintf( fp, "Blood        %d\n", spec->blood );
      fprintf( fp, "Deathmsg     %s~\n", spec->deathmsg );

      if( spec->first_attack )
      {
         ATTACK_DATA *attack;

         for( attack = spec->first_attack; attack; attack = attack->next )
            fprintf( fp, "Attack        '%s' %d %d\n",
                    skill_table[attack->sn]->name, attack->minlevel, attack->uses );
      }
      fprintf( fp, "%s", "End\n\n" );
      fprintf( fp, "%s", "#END\n" );
...
void do_setspec( CHAR_DATA *ch, const char *argument )
{
   ...
   if( arg1[0] == '\0' )
   {
      send_to_char( "Usage: setspec <spec> <field>\r\n", ch );
      send_to_char( "\r\nField being one of:\r\n", ch );
      send_to_char( " name filename description attack rmattack\r\n", ch );
      send_to_char( " blood corpse deathmsg\r\n", ch );
      return;
   }
   ...
   {
      SPEC_DATA *uspec = NULL;

      if( !argument || argument[0] == '\0' )
      {
         send_to_char( "You can't name a spec nothing.\r\n", ch );
         return;
      }
      if( ( uspec = get_spec( argument ) ) )
      {
         send_to_char( "There is already another spec with that name.\r\n", ch );
         return;
      }
      STRFREE( spec->name );
      spec->name = STRALLOC( argument );
      send_to_char( "Done.\r\n", ch );
      save_spec( spec );
      return;
   }
   ...
   if( !str_cmp( arg2, "filename" ) )
   {
      ...
      if( !is_valid_filename( ch, SPEC_DIR, argument ) )
         return;

      snprintf( filename, sizeof( filename ), "%s%s", SPEC_DIR, spec->filename );
      if( !remove( filename ) )
         send_to_char( "Old spec file deleted.\r\n", ch );

      DISPOSE( spec->filename );
      spec->filename = str_dup( argument );
      send_to_char( "Done.\r\n", ch );
      save_spec( spec );
      write_spec_list(  );
      return;
   ...
   {
      STRFREE( spec->description );
      spec->description = STRALLOC( argument );
      send_to_char( "Done.\r\n", ch );
      save_spec( spec );
      return;
   }

   if( !str_cmp( arg2, "deathmsg" ) )
   {
      STRFREE( spec->deathmsg );
      spec->deathmsg = STRALLOC( argument );
      send_to_char( "Done.\r\n", ch );
      save_spec( spec );
      return;
   }
   ...
   if( !str_cmp( arg2, "corpse" ) )
   {
      if( spec->corpse == 0 )
         ...
         spec->corpse = 0;
      send_to_char( "Corpse toggle set.\r\n", ch );
      save_spec( spec );
      return;
   }
   
   if( !str_cmp( arg2, "rmattack" ) )
   {
      ...
      if( !spec->first_attack )
      {
         send_to_char( "This spec has no special attacks to remove.\r\n", ch );
         return;
      }
      
      for( attack = spec->first_attack; attack; attack = attack_next )
      {
         attack_next = attack->next;

         if( ++cnt == num )
         {
            UNLINK( attack, spec->first_attack, spec->last_attack, next, prev );
            DISPOSE( attack );
   ...
   if( !str_cmp( arg2, "attack" ) )
   {
      ...
      if( !str_cmp( arg3, "create" ) )
      {
         ...
         if( !arg4 || !arg5 || !argument )
         {
            send_to_char( "Usage: setspec <spec> create <skill> <minlevel> <uses>\r\n", ch );
            return;
         }
         ...
         else if( ( sn = skill_lookup( arg4 ) ) >= 0 )
         ...
         CREATE( attack, ATTACK_DATA, 1 );
            attack->sn = sn;
            attack->minlevel = atoi( arg5 );
            attack->uses = atoi( argument );
         LINK( attack, spec->first_attack, spec->last_attack, next, prev );
         save_spec( spec );
         return;
      }
...
void do_makespec( CHAR_DATA* ch, const char* argument)
{
   SPEC_DATA *spec;
   ...
   if( !argument || argument[0] == '\0' )
   {
      send_to_char( "Usage: makespec <spec name>\r\n", ch );
      return;
   }

   set_char_color( AT_PLAIN, ch );
   spec = get_spec( argument );
   if( spec )
   {
      send_to_char( "There is already a spec with that name.\r\n", ch );
      return;
   }
    ...
    * member without changing load_spec() and do_setspec() to employ hashstrings too... 
    */
   spec->filename = str_dup( "" );
   spec->description = STRALLOC( "" );
   spec->deathmsg = STRALLOC( "" );
   spec->blood = 1;
   spec->corpse = 1;
...
void do_showspec( CHAR_DATA *ch, const char *argument )
{
   SPEC_DATA *spec;

   if( argument[0] == '\0' )
   {
      send_to_char( "Mob Special Functions\r\n", ch );
      for( spec = first_spec; spec; spec = spec->next )
      {
         ch_printf( ch, "%s\r\n", spec->name );
      }
   ...
   ch_printf( ch, "Special Function: %s\r\n", spec->name );
   ch_printf( ch, "Description: %s\r\n", spec->description );
   ch_printf( ch, "Filename: %s\r\n", spec->filename );
   ch_printf( ch, "Leaves Corpses: %s\r\n", spec->corpse == 0 ? "N" : "Y" );
   ch_printf( ch, "Has Blood: %s\r\n", spec->blood == 0 ? "N" : "Y" );
   ch_printf( ch, "DeathMessage: %s\r\n", spec->deathmsg );
   if( spec->first_attack )
   {
      ATTACK_DATA *attack;
      ...
      for( attack = spec->first_attack; attack; attack = attack->next )
      {
         ch_printf( ch, "Attack (Lv%d, %d Uses): %s\r\n", attack->minlevel, attack->uses, skill_table[attack->sn]->name );
      }
   }
   return;
}
...
bool validate_spec_fun( const char *name )
{
   ...
   for( specfun = first_spec; specfun; specfun = specfun->next )
   {
...
bool spec_cast_adept( CHAR_DATA * ch )
{
   ...
   {
      case 0:
         act( AT_MAGIC, "You see $n hold a mirror while chanting, 'Vyd shinnil sek lazris!'", ch, NULL, NULL, TO_ROOM );
         spell_smaug( skill_lookup( "sanctuary" ), ch->level, ch, victim );
         return TRUE;

      case 1:
         act( AT_MAGIC, "You see $n sprinkle water while chanting, 'Boh cildre mu hassund!'", ch, NULL, NULL, TO_ROOM );
         spell_smaug( skill_lookup( "bless" ), ch->level, ch, victim );
         return TRUE;

      case 2:
         act( AT_MAGIC, "$n waves $s hands while chanting, 'Nord bebu cu niss!'", ch, NULL, NULL, TO_ROOM );
         spell_cure_blindness( skill_lookup( "remove blindness" ), ch->level, ch, victim );
         return TRUE;

      case 3:
         act( AT_MAGIC, "$n begins a series of hand motions while chanting, 'Pur whedon tral!'", ch, NULL, NULL, TO_ROOM );
         spell_smaug( skill_lookup( "cure light wounds" ), ch->level, ch, victim );
         return TRUE;

      case 4:
         act( AT_MAGIC, "$n crushes some charcoal in $s hand while chanting, 'Stosar shonor sa!'", ch, NULL, NULL, TO_ROOM );
         spell_cure_poison( skill_lookup( "neutralize poison" ), ch->level, ch, victim );
         return TRUE;

      case 5:
         act( AT_MAGIC, "$n makes a series of hand motions while chanting, 'Zhys rizu stranthis!'", ch, NULL, NULL, TO_ROOM );
         spell_smaug( skill_lookup( "refresh" ), ch->level, ch, victim );
         return TRUE;

      case 6:
         act( AT_MAGIC, "$n makes a series of hand motions while chanting, 'Hyd raldem han!'", ch, NULL, NULL, TO_ROOM );
         spell_smaug( skill_lookup( "cure serious wounds" ), ch->level, ch, victim );
         return TRUE;

      case 7:
         act( AT_MAGIC, "$n begins to chant fervently, 'Blostir seth cynsen dul!'", ch, NULL, NULL, TO_ROOM );
   ...
   for( trash = ch->in_room->first_content; trash; trash = trash_next )
   {
      ...
      if( IS_ACT_FLAG( ch, ACT_ONMAP ) )
	{
      ...
      if( trash->item_type == ITEM_DRINK_CON
          ...
          || trash->gold_cost < 1 || trash->silver_cost < 1 || trash->copper_cost < 10 || ( trash->pIndexData->vnum == OBJ_VNUM_SHOPPING_BAG && !trash->first_content ) )
      {
