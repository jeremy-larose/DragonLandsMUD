...
/* MPGift at the behest of Jason -- Seikyr 11/13/2018 */
void do_mpgift( CHAR_DATA *ch, const char *argument )
{
   ...
   if( arg1[0] == '\0' )
   {
      send_to_char( "Mpgift what?\r\n", ch );
      progbug( "Mpgift: no lowvnum. Syntax mpgift <lowvnum> <highvnum>", ch );
      return;
   }
   if( arg2[0] == '\0' )
   {
      send_to_char( "Invalid vnum range.\r\n", ch );
      progbug( "Mpgift: no highvnum. Syntax mpgift <lowvnum> <highvnum>", ch );
      return;
   }
   ...
   if( is_number( arg1 ) && is_number( arg2 ) )
   {
      ...
      if( hi_vnum <= lo_vnum )
      {
         ...
         progbug( "Mpgift: invalid vnum range", ch );
         return;
      }
   }
   else
   {
      ...
      progbug( "Mpgift: Invalid syntax", ch );
      return;
   }
   
   if( ( pObjIndex = get_obj_index( number_range( lo_vnum, hi_vnum ) ) ) == NULL )
   {
      progbug( "Mpgift: Bad vnum arg", ch );
      return;
   }
...
*/
   exp = atoi( argument );
   ch_printf( victim, "You receive %d experience points.\r\n", exp );
   gain_exp( victim, exp );
   return;
}
...
/* Am I asking for trouble here or what?  But I need it. -- Blodkai */
   if( !str_cmp( arg1, "all" ) )
   {
      for( victim = ch->in_room->first_person; victim; victim = nextinroom )
      {
         ...
         {
            snprintf( buf, MAX_STRING_LENGTH, "'%s' %s %s", victim->name, arg2, arg3 );
            do_mp_damage( ch, buf );
         }
      }
      return;
   ...
   if( arg2[0] == '\0' )
   {
      send_to_char( "mpdamage inflict what number of dice?\r\n", ch );
      progbug( "Mpdamage: invalid argument2", ch );
      return;
   }
   if( arg3[0] == '\0' )
   {
      send_to_char( "mpdamage inflict what size dice?\r\n", ch );
    ...
    */
   dam = number_range( 1, damnum * damsize );
