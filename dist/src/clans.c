...
CLAN_DATA *get_clan( const char *name )
{
   ...
   for( clan = first_clan; clan; clan = clan->next )
      if( !str_cmp( name, clan->name ) || ( ( clan->abbrev ) && !str_cmp( name, clan->abbrev ) ) )
         return clan;
   return NULL;
}
...
COUNCIL_DATA *get_council( const char *name )
{
   ...
   for( council = first_council; council; council = council->next )
      if( !str_cmp( name, council->name ) || ( ( council->abbrev ) && !str_cmp( name, council->abbrev ) ) )
         return council;
   return NULL;
}
...
/* Check for class, assign percentage based on that. ****REVISIT SEIKYR****
...
/* Check for race, adjust percentage based on that. ** Removed 1/5/17 JL **
   if( ch->race == RACE_ELF )
      race_bonus = -3;
