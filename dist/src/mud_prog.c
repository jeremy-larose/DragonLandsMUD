    ...
    if( chkchar || chkobj || chkroom )
    {
      ...
      else  
      {
         ...
         if( chkchar != NULL )  
         {
             ...
             {
                 if( IS_NPC( chkchar ) )
                   mudstrlcat( results, format( "%s", chkchar->short_descr ), MAX_STRING_LENGTH );
                 else
                   mudstrlcat( results, format( "%s", chkchar->name ), MAX_STRING_LENGTH );
             }
             if( !str_cmp( word, "sex" ) )
                 mudstrlcat( results, format( "%s", npc_sex[chkchar->sex] ), MAX_STRING_LENGTH );
             if( !str_cmp( word, "class" ) )
                 mudstrlcat( results, get_class( chkchar ), MAX_STRING_LENGTH );
             if( !str_cmp( word, "race") )
                  mudstrlcat( results, get_race( chkchar ), MAX_STRING_LENGTH );
             if( !str_cmp( word, "level") )
                 mudstrlcat( results, format( "%d", chkchar->level ), MAX_STRING_LENGTH );
         }
         if( chkobj != NULL )
         {
             if( !str_cmp( word, "level") )
                 mudstrlcat( results, format( "%d", chkobj->level ), MAX_STRING_LENGTH );
         }
         if ( cmnd[x] == '\0')
          return results;
         else
...
int mprog_do_ifcheck( const char *ifcheck, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, CHAR_DATA * victim, OBJ_DATA * target, CHAR_DATA * rndm )
{
 ...
 doneargs:
   ...
   while( isspace( *p ) )
      *p++ = '\0';
   rval = parse_var( p, mob, actor, obj, victim, rndm ); 
   /* rval = p; */
   /*
    * while ( *p && !isspace(*p) ) ++p;
    */
   ...
   *p = '\0';
   ...
   if( !str_cmp( chck, "chinarea" ) )
   {
      ...
      {
         progbug( "Bad vnum to 'chinarea'", mob );
         return BERR;
      }
      ...
      for( d = first_descriptor; d; d->next )
      {
         ...
         && can_see( mob, victim, TRUE ) ) 
         {
            return mprog_veval( 1, opr, 1, mob );
         }
         else
            found = mprog_veval( 0, opr, 1, mob );
      }
   }
   ...
   if( chkchar )
   {
      ...
      if( !str_cmp( chck, "sector" ) )
      {
         if( !is_number( rval ) )
            return FALSE;
         if( chkchar->in_room->sector_type == atoi( rval ) )
            return TRUE;
         else
            return FALSE;
      }
      ...
      if ( !str_cmp( chck, "athletics" ) )
      {  
         return mprog_veval( dc_athletics( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "acrobatics" ) )
      {  
         return mprog_veval( dc_acrobatics( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "animalhandling" ) )
      {  
         return mprog_veval( dc_animal_handling( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "arcana" ) )
      {  
         return mprog_veval( dc_arcana( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "deception" ) )
      {  
         return mprog_veval( dc_deception( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "history" ) )
      {  
         return mprog_veval( dc_history( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "insight" ) )
      {  
         return mprog_veval( dc_insight( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "intimidation" ) )
      {  
         return mprog_veval( dc_intimidation( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "investigation" ) )
      {  
         return mprog_veval( dc_investigation( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "medicine" ) )
      {  
         return mprog_veval( dc_medicine( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "nature" ) )
      {  
         return mprog_veval( dc_nature( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "perception" ) )
      {  
         return mprog_veval( dc_perception( chkchar, FALSE ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "performance" ) )
      {  
         return mprog_veval( dc_performance( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "persuasion" ) )
      {  
         return mprog_veval( dc_persuasion( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "religion" ) )
      {  
         return mprog_veval( dc_religion( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "sleight" ) )
      {  
         return mprog_veval( dc_sleight( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "stealth" ) )
      {  
         return mprog_veval( dc_stealth( chkchar, FALSE ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "survival" ) )
      {  
         return mprog_veval( dc_survival( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "strength" ) )
      {  
         return mprog_veval( dc_strength( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "dexterity" ) )
      {  
         return mprog_veval( dc_dexterity( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "wisdom" ) )
      {  
         return mprog_veval( dc_wisdom( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "intelligence" ) )
      {  
         return mprog_veval( dc_intelligence( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "constitution" ) )
      {  
         return mprog_veval( dc_constitution( chkchar ), opr, atoi( rval ), mob );
      }
      if ( !str_cmp( chck, "charisma" ) )
      {  
         return mprog_veval( dc_charisma( chkchar ), opr, atoi( rval ), mob );
      }
                      ...
                      OBJ_DATA * obj, CHAR_DATA * victim, OBJ_DATA * target, CHAR_DATA * rndm, bool ignore, bool ignore_ors )
{
   ...
   cmnd = parse_var( cmnd, mob, actor, obj, victim, rndm ); 
...
void mprog_bribe_trigger_copper( CHAR_DATA * mob, CHAR_DATA * ch, int amount )
{
  ...
  if ( IS_NPC( mob )
      ...
      && HAS_PROG( mob->pIndexData, BRIBE_SILVER_PROG ) )
    {
      ...
      for( mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next )
      {
         if( ( mprg->type == BRIBE_COPPER_PROG ) && ( amount >= atoi( mprg->arglist ) ) )
         {
            if( tprg )
            {
               if( atoi( tprg->arglist ) < atoi( mprg->arglist ) )
...
void oprog_move_trigger( CHAR_DATA *ch )
{
   MPROG_DATA *mprg;
   OBJ_DATA *vobj;
//   bug( "Move trigger: Stage II" );
if( !IS_NPC( ch ) )
   for( vobj = ch->first_carrying; vobj; vobj = vobj->next_content )
   {
      if( HAS_PROG( vobj->pIndexData, MOVE_PROG ) )
      {
         for( mprg = vobj->pIndexData->mudprogs; mprg; mprg = mprg->next )
         {
            ...
            {
               set_supermob( vobj );
               oprog_percent_check( supermob, ch, vobj, NULL, NULL, MOVE_PROG );
               ch->mpscriptpos = supermob->mpscriptpos;
               release_supermob(  );
            }
         }
      }
