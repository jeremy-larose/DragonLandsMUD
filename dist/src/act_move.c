...
ch_ret move_char( CHAR_DATA * ch, EXIT_DATA * pexit, int fall, int direction )
{
   ...
   if ( IS_SET( pexit->exit_info, EX_OVERLAND ) )
   {
      ...
      if( !IS_NPC( ch ) )
      {
         act( AT_ACTION, "$n leaves into the wilderness.", ch, NULL, NULL, TO_ROOM );
         ...
         for ( fch = from_room->first_person; fch && ( count < chars ); fch = nextinroom )
         {
            ...
            && (fch->position == POS_STANDING || fch->position == POS_MOUNTED) )
            {
               ...
               {
                  act( AT_ACTION, "$n leaves into the wilderness with $N.", fch, ch, NULL, TO_ROOM );
...
/*
      ...
      if( IS_SET( pexit->exit_info, EX_CLIMB ) )
      {
         ...
         if( !found && !ch->mount )
         {
            if( ( !IS_NPC( ch ) && number_percent(  ) > LEARNED( ch, gsn_climb ) ) || drunk || ch->mental_state < -90 )
            {
               ...
               WAIT_STATE( ch, 20 );
               retcode = damage( ch, ch, ( pexit->vdir == DIR_UP ? 10 : 5 ), TYPE_UNDEFINED, FALSE );
               return retcode;
            }
            found = TRUE;
   ...
   {
      CHAR_DATA *wf;
      for( wf = ch->in_room->first_person; wf; wf = wf->next_in_room )
      {
         if( who_fighting( wf ) == ch )
         {
            if( !IS_AFFECTED( ch, AFF_DISENGAGE ) && !ACTIVE_ABILITY( ch, ABIL_ARCANE_CHARGE ) )
            {
            //bug( "%s: Debug: Attack of Opportunity from %s to %s!\r\n", __func__, wf->name, ch->name );
            ch_printf( ch, "As you turn to leave, %s makes an attack of opportunity!\r\n", PERS( wf, ch, FALSE ) );
            one_hit( wf, ch, TYPE_UNDEFINED );
            start_hating( wf, ch );
            }
            else
            {
            //   bug( "%s: Debug: Making a clean getaway: %s fighting %s\r\n", __func__, wf->name, ch->name );
               start_hating( wf, ch );
               if( !ACTIVE_ABILITY( ch, ABIL_ARCANE_CHARGE ) )
                  send_to_char( "You run from combat!\r\n", ch );
               xREMOVE_BIT( ch->affected_by, AFF_DISENGAGE );
            }
         }
      }
   }
...
//   bug( "Move trigger: Stage I" );
   oprog_move_trigger( ch );
   /*
   ...
   if( !will_fall( ch, fall ) && fall > 0 )
   {
      if( !IS_AFFECTED( ch, AFF_FLOATING ) || ( ch->mount && !IS_AFFECTED( ch->mount, AFF_FLOATING ) ) )
      {
         ...
         send_to_char( "OUCH! You hit the ground!\r\n", ch );
         WAIT_STATE( ch, 20 );
         retcode = damage( ch, ch, 20 * fall, TYPE_UNDEFINED, FALSE );
      }
      else
      {
         set_char_color( AT_MAGIC, ch );
...
void do_bashdoor( CHAR_DATA* ch, const char* argument)
{
   ...
   if( ( pexit = find_door( ch, arg, FALSE ) ) != NULL )
   {
          ...
          && ch->move >= 15 && number_percent(  ) < ( schance + 4 * ( get_curr_str( ch ) - 19 ) ) )
      {
         ...
         damage( ch, ch, ( ch->max_hit / 20 ), gsn_bashdoor, FALSE );
      ...
      else
      {
         ...
         damage( ch, ch, ( ch->max_hit / 20 ) + 10, gsn_bashdoor, FALSE );
...
void do_stand( CHAR_DATA* ch, const char* argument)
{
   ...
   if( IS_AFFECTED( ch, AFF_EARTHQUAKE ) )
   {
      if( dc_strength( ch ) < 20 )
      {
         send_to_char( "You can't seem to unbury yourself to stand up!\r\n", ch );
         act( AT_WHITE, "$n attempts to stand, but is struggling to unbury $mself from the earthquake!", ch, NULL, NULL, TO_ROOM );
         WAIT_STATE( ch, PULSE_VIOLENCE );
         return;
      }
      else
      {
         send_to_char( "You unbury yourself from the earthquake!\r\n", ch );
         act( AT_WHITE, "$n unburies $mself from the earthquake and stands up!", ch, NULL, NULL, TO_ROOM );
         xREMOVE_BIT( ch->affected_by, AFF_PRONE );
         xREMOVE_BIT( ch->affected_by, AFF_EARTHQUAKE );
         return;
      }
   }
   ...
   switch ( ch->position )
   {
      ...
      case POS_MEMORIZE: /* Spell Memorization 1/17 JL */
         if ( !IS_NPC( ch ) )
         {
           send_to_char("You snap out of your meditation and rise to your feet.\n\r",ch);
           act( AT_ACTION, "$n closes a spellbook in frustration.", ch, NULL, NULL, TO_ROOM );
           ...
           if( stopped )
           {
             ...
             act( AT_ACTION, "$n closes $s spellbook in frustration.", ch, NULL, NULL, TO_ROOM );
           }
         }
         ch->position = POS_STANDING;
         break;
      ...
      case POS_BERSERK:
         ...
         {
            send_to_char( "You gather yourself from the ground and prepare to fight again!\r\n", ch );
            act( AT_WHITE, "$n gathers $mself from the ground and prepares to fight again!", ch, NULL, NULL, TO_ROOM );
            xREMOVE_BIT( ch->affected_by, AFF_PRONE );
...
void do_sit( CHAR_DATA* ch, const char* argument)
{
   ...
   switch ( ch->position )
   {
      ...
      case POS_MEMORIZE: /* Spell Memorization 1/17 JL */
         if (!IS_NPC(ch))
         {
           send_to_char( "You snap out of your studies and sit up.\n\r", ch );
           act( AT_ACTION, "$n closes a spellbook in frustration.", ch, NULL, NULL, TO_ROOM );
           for(x = 0; x < MAX_CIRCLE_LEVEL; x++)
             for (y = 0; y < MAX_PER_CIRCLE; y++)
             {
...
*/
      case POS_MEMORIZE:
         send_to_char( "You are already resting during your meditation.\r\n", ch );
...
void do_enter( CHAR_DATA* ch, const char* argument )
{
   ...
   if( argument[0] == '\0' )
   {
      /* Overland Map Stuff -- Seikyr 11/18 */
      if( !IS_NPC( ch ) && IS_PLR_FLAG( ch, PLR_ONMAP ) )
      {
         short sector = get_terrain( ch->map, ch->x, ch->y );
         if( sector == SECT_EXIT )
         {
 //           send_to_char( "You're on a map and this is an exit sector! act_move:2758\r\n", ch );
            act( AT_ACTION, "$n leaves the wilderness.", ch, NULL, NULL, TO_NOTVICT );
            process_exit( ch, ch->map, ch->x, ch->y, -2 ); /* Sets dir to -2 to show that they used enter command to process_exit */
            return;
         }
 //        else
 //        {
 //           send_to_char( "You're on a map but this sector is not an exit. act_move:2764\r\n", ch );
 //        }
      }
  //    else
  //    {
  //       send_to_char( "You're not on a map, apparently. act_move:2769\r\n", ch );
  //    }
      for( pexit = ch->in_room->first_exit; pexit; pexit = pexit->next )
