...
void fwrite_char( CHAR_DATA * ch, FILE * fp )
{
   ...
   fprintf( fp, "ClassValues  %d %d\n", ch->classvalues[0], ch->classvalues[1] );
   fprintf( fp, "BondedWeapon %d %d\n", ch->bondedweapon[0], ch->bondedweapon[1] );
   fprintf( fp, "Gold         %d\n", ch->gold );
   fprintf( fp, "Silver       %d\n", ch->silver );
   fprintf( fp, "Copper       %d\n", ch->copper );
   fprintf( fp, "Exp          %d\n", ch->exp );
   ...
   fprintf( fp, "SavingThrows %d %d %d %d %d %d\n",
            ch->saving_str, ch->saving_dex, ch->saving_con, ch->saving_int, ch->saving_wis, ch->saving_cha );
   fprintf( fp, "Size         %d\n", ch->size );
   ...
   fprintf( fp, "Armor        %d %d\n", ch->base_armor, ch->armor );
   fprintf( fp, "Speechcolor  %d\n", ch->speechcolor );
   if( ch->wimpy )
      fprintf( fp, "Wimpy        %d\n", ch->wimpy );
   if( ch->deaf )
      fprintf( fp, "Deaf         %d\n", ch->deaf );
   if( !xIS_EMPTY( ch->resistant ) )
      fprintf( fp, "Resistant    %s\n", print_bitvector( &ch->resistant ) );
   if( !xIS_EMPTY( ch->no_resistant ) )
      fprintf( fp, "NoResistant  %s\n", print_bitvector( &ch->no_resistant ) );
   if( !xIS_EMPTY( ch->immune ) )
      fprintf( fp, "Immune       %s\n", print_bitvector( &ch->immune ) );
   if( !xIS_EMPTY( ch->no_immune ) )
      fprintf( fp, "NoImmune     %s\n", print_bitvector( &ch->no_immune ) );
   if( !xIS_EMPTY( ch->susceptible ) )
      fprintf( fp, "Susceptible  %s\n", print_bitvector( &ch->susceptible ) );
   if( !xIS_EMPTY( ch->no_susceptible ) )
      fprintf( fp, "NoSusceptible  %s\n", print_bitvector( &ch->no_susceptible ) );
   if( !xIS_EMPTY( ch->subclasses ) )
      fprintf( fp, "Subclasses   %s\n", print_bitvector( &ch->subclasses ) );
   if( !xIS_EMPTY( ch->subrace ) )
      fprintf( fp, "Subrace      %s\n", print_bitvector( &ch->subrace ) );
   if( !xIS_EMPTY( ch->feats ) )
      fprintf( fp, "Feats        %s\n", print_bitvector( &ch->feats ) );
   if( !xIS_EMPTY( ch->abilities ) )
      fprintf( fp, "Abilities    %s\n", print_bitvector( &ch->abilities ) );
   ...
   if( ch->first_prep )
   {
      ...
      for( spell = ch->first_prep; spell; spell = spell->next )
      {
         if ( spell->sn < 1 )
            continue;
         fprintf( fp, "Prepped       '%s' %d %d\n",
                 skill_table[spell->sn]->name, spell->fully_prepped, (spell->fully_prepped == 0 ? spell->mem_wait : -1 ) );
      }
   }
   ...
   if( ch->first_cooldown )
   {
      ...
      for( cooldown = ch->first_cooldown; cooldown; cooldown = cooldown->next )
         fprintf( fp, "Cooldown        '%s' %d %d %d\n",
                 skill_table[cooldown->sn]->name, cooldown->max_available, cooldown->curr_available, cooldown->wait );
   }
...
void fwrite_obj( CHAR_DATA * ch, OBJ_DATA * obj, FILE * fp, int iNest, short os_type, bool hotboot )
{
   ...
   if( obj->lore && ( !obj->pIndexData->lore || str_cmp( obj->lore, obj->pIndexData->lore ) ) )
      fprintf( fp, "Lore         %s~\n", obj->lore ); 
   ...
   if( obj->value[0] || obj->value[1] || obj->value[2] || obj->value[3] || obj->value[4] || obj->value[5] )
      ...
      fprintf( fp, "Values2      %d %d\n", obj->armor_type, obj->size );
...
bool load_char_obj( DESCRIPTOR_DATA * d, char *name, bool preload, bool copyover )
{
   ...
   xCLEAR_BITS( ch->feats );
   xCLEAR_BITS( ch->abilities );
   ch->size = 0;
   ch->was_in_room = NULL;
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'A':
            ...
            KEY( "Adjective", ch->adjective, fread_string( fp ) );
            ...
            KEY( "Abilities", ch->abilities, fread_bitvector( fp ) );
            ...
            KEY( "Attacks", ch->numattacks, fread_number( fp ) );
            KEY( "AuthedBy", ch->pcdata->authed_by, fread_string( fp ) );
            break;

         case 'B':
            KEY( "Background", ch->pcdata->background, fread_number( fp ) );
            ...
            if( !str_cmp( word, "BondedWeapon" ) )
            {
               line = fread_line( fp );
               sscanf( line, "%d %d", &x1, &x2 );
               ch->bondedweapon[0] = x1;
               ch->bondedweapon[1] = x2;
               fMatch = TRUE;
               break;
            }
         ...
         case 'C':
            ...
            if( !str_cmp( word, "Cooldown" ) )
            {
               ...
               CREATE( skill, COOLDOWN_DATA, 1 );
                  skill->sn = sn;
                  skill->max_available = fread_number( fp );
                  skill->curr_available = fread_number( fp );
                  skill->wait = fread_number( fp );
               LINK( skill, ch->first_cooldown, ch->last_cooldown, next, prev );
               fMatch = TRUE;
               break;
            }
         ...
         case 'F':
            ...
            KEY( "Feats", ch->feats, fread_bitvector( fp ) );
            KEY( "Flags", ch->pcdata->flags, fread_number( fp ) );
            KEY( "FPrompt", ch->pcdata->fprompt, fread_string( fp ) );
            if( !str_cmp( word, "FreeSlots" ) )
            {
               line = fread_line( fp );
               x1 = x2 = x3 = x4 = x5 = x6 = x7 = x8 = x9 = x10 = 0;
               sscanf( line, "%d %d %d %d %d %d %d %d %d %d", &x1, &x2, &x3, &x4, &x5, &x6, &x7, &x8, &x9, &x10);
               ch->slot_free[0] = x1;
               ch->slot_free[1] = x2;
               ch->slot_free[2] = x3;
               ch->slot_free[3] = x4;
         ...
         case 'G':
            ...
            if( !str_cmp( word, "Greeted" ) )
            {
               GREET_DATA *greet;

               CREATE( greet, GREET_DATA, 1 );
               greet->name = fread_string( fp );
               LINK( greet, ch->first_greet, ch->last_greet, next, prev );
               fMatch = TRUE;
               break;
            }
            break;
         ...
         case 'I':
            ...
            KEY( "Immune", ch->immune, fread_bitvector( fp ) );
#ifdef IMC
         ...
         case 'L':
            ...
            KEY( "Lore", ch->lore, fread_string( fp ) );
         ...
         case 'M':
            ...
            if( !str_cmp( word, "Multiclass" ) )
            {
               MULTICLASS_DATA *multiclass;

               CREATE( multiclass, MULTICLASS_DATA, 1 );
               multiclass->classname = fread_number( fp );
               multiclass->level = fread_number( fp );
         ...
         case 'P':
            ...
            if( !str_cmp( word, "Prepped" ) )
            {
               SPELL_PREP_DATA *spell;
               int sn, circle;

               sn = find_spell( NULL, fread_word( fp ), FALSE );
	       circle = ( skill_table[sn]->circle[ch->Class] );
               CREATE( spell, SPELL_PREP_DATA, 1 );
                  spell->sn = sn;
                  spell->circle = circle;
                  spell->fully_prepped = fread_number( fp );
                  spell->mem_wait = fread_number( fp );
               LINK( spell, ch->first_prep, ch->last_prep, next, prev );
               fMatch = TRUE;
               break;
            }
            
            if (!str_cmp( word, "Prepared"))
            {
	      int sn, circle, slot;
	       ...
	       ch->pcdata->circles[circle][slot].ticks = 0;
            ...
            if( !strcmp( word, "Proficiency" ) )
            {
               ...
               else
               {
                  ...
                  sn = find_proficiency( NULL, fread_word( fp ), FALSE );
                  if( sn < 0 )
                     bug( "%s: unknown proficiency.", __func__ );
                  else
                  {
                     ch->pcdata->learned[sn] = value;
                     if( ch->level < LEVEL_IMMORTAL )
         ...
         case 'R':
            ...
            KEY( "Resistant", ch->resistant, fread_bitvector( fp ) );
         ...
         case 'S':
            ...
            KEY( "Subclasses", ch->subclasses, fread_bitvector( fp ) );
            KEY( "Subrace", ch->subrace, fread_bitvector( fp ) );
            KEY( "Susceptible", ch->susceptible, fread_bitvector( fp ) );
            if( !strcmp( word, "SavingThrow" ) )
            {
               ch->saving_int = fread_number( fp );
               ch->saving_dex = ch->saving_int;
               ch->saving_con = ch->saving_int;
               ch->saving_str = ch->saving_int;
               ch->saving_wis = ch->saving_int;
               ch->saving_cha = ch->saving_int;
               fMatch = TRUE;
               break;
            }
            ...
            {
               ch->saving_str = fread_number( fp );
               ch->saving_dex = fread_number( fp );
               ch->saving_con = fread_number( fp );
               ch->saving_int = fread_number( fp );
               ch->saving_wis = fread_number( fp );
               ch->saving_cha = fread_number( fp );
               fMatch = TRUE;
               break;
            }
            ...
            KEY( "SpeechColor", ch->speechcolor, fread_number( fp ) );
            ...
            if (!str_cmp( word, "SpellSlots"))
            {
               int x, y;
               for( x = 0; x < MAX_PER_SLOT; x++)
                  for( y = 0; y < max_spell_slot_lookup( ch, x ); y++ )
                  {
            	  ch->pcdata->slots[x][y].free = fread_number( fp );
                  ch->pcdata->slots[x][y].ticks = 0;
                  }
               fread_to_eol( fp );           
               fMatch = TRUE;
               break;
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'E':
            ...
            if( !strcmp( word, "End" ) )
            {
               ...
               else
               {
                  ...
                  if( !obj->lore )
                     obj->lore = QUICKLINK( obj->pIndexData->lore );
         ...
         case 'L':
            ...
            KEY( "Lore", obj->lore, fread_string( fp ) );
            break;
         ...
         case 'V':
            ...
            if( !strcmp( word, "Vnum" ) )
            {
               ...
               else
               {
                  ...
                  obj->armor_type = obj->pIndexData->armor_type;
               }
               fMatch = TRUE;
               break;
            }
...
void fwrite_mobile( FILE * fp, CHAR_DATA * mob )
{
   ...
   if( mob->description && mob->pIndexData->description && str_cmp( mob->description, mob->pIndexData->description ) )
      fprintf( fp, "Description %s~\n", mob->description );
   if( mob->lore && mob->pIndexData->lore && str_cmp( mob->lore, mob->pIndexData->lore ) )
      fprintf( fp, "Lore        %s~\n", mob->lore );
   fprintf( fp, "Position %d\n", mob->position );
