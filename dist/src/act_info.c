...
const char *const where_name[] = {
   "&G[&YUsed as light     &G]&w ",
   "&G[&wWorn on finger    &G]&w ",
   "&G[&wWorn on finger    &G]&w ",
   "&G[&wWorn around neck  &G]&w ",
   "&G[&wWorn around neck  &G]&w ",
   "&G[&wWorn on body      &G]&w ",
   "&G[&wWorn on head      &G]&w ",
   "&G[&wWorn on legs      &G]&w ",
   "&G[&wWorn on feet      &G]&w ",
   "&G[&wWorn on hands     &G]&w ",
   "&G[&wWorn on arms      &G]&w ",
   "&G[&wWorn as shield    &G]&w ",
   "&G[&wWorn about body   &G]&w ",
   "&G[&wWorn about waist  &G]&w ",
   "&G[&wWorn around wrist &G]&w ",
   "&G[&wWorn around wrist &G]&w ",
   "&G[&wWielded           &G]&w ",
   "&G[&wHeld              &G]&w ",
   "&G[&wDual-wielded      &G]&w ",
   "&G[&wWorn on ears      &G]&w ",
   "&G[&wWorn on eyes      &G]&w ",
   "&G[&wMissile wielded   &G]&w ",
   "&G[&wWorn on back      &G]&w ",
   "&G[&wWorn over face    &G]&w ",
   "&G[&wWorn on belt      &G]&w ",
   "&G[&wWorn on belt      &G]&w ",
   "&G[&wHeld in main hand &G]&w ",
   "&G[&wHeld in off hand  &G]&w ",
   "&G[&wHeld in both hands&G]&w "
};
...
char *format_obj_to_char( OBJ_DATA * obj, CHAR_DATA * ch, bool fShort )
{
   ...
   if( IS_OBJ_STAT( obj, ITEM_INVIS ) )
      mudstrlcat( buf, "&B(&zInvis&B)&w ", MAX_STRING_LENGTH );
   if( ( IS_AFFECTED( ch, AFF_DETECT_EVIL ) || ch->Class == CLASS_PALADIN ) && IS_OBJ_STAT( obj, ITEM_EVIL ) )
      mudstrlcat( buf, "&R(&rRed Aura&R)&w ", MAX_STRING_LENGTH );
   ...
   if( ( IS_AFFECTED( ch, AFF_DETECT_MAGIC ) || xIS_SET( ch->act, PLR_HOLYLIGHT ) ) && IS_OBJ_STAT( obj, ITEM_MAGIC ) )
      mudstrlcat( buf, "&P(&pMagical&P)&w ", MAX_STRING_LENGTH );
   if( !glowsee && IS_OBJ_STAT( obj, ITEM_GLOW ) )
      mudstrlcat( buf, "&Y(&wGlowing&Y)&w ", MAX_STRING_LENGTH );
   if( IS_OBJ_STAT( obj, ITEM_HUM ) )
      mudstrlcat( buf, "&G(&WHumming&G)&w ", MAX_STRING_LENGTH );
   if( IS_OBJ_STAT( obj, ITEM_HIDDEN ) )
      mudstrlcat( buf, "&G(&zHidden&G)&w ", MAX_STRING_LENGTH );
   if( IS_OBJ_STAT( obj, ITEM_BURIED ) )
      mudstrlcat( buf, "&O(&zBuried&O)&w ", MAX_STRING_LENGTH );
   if( IS_IMMORTAL( ch ) && IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) )
      mudstrlcat( buf, "&P(&GProto&P)&w ", MAX_STRING_LENGTH );
   if( ( IS_AFFECTED( ch, AFF_DETECTTRAPS ) || xIS_SET( ch->act, PLR_HOLYLIGHT ) ) && is_trapped( obj ) )
      mudstrlcat( buf, "&R(&GTrap&R)&w ", MAX_STRING_LENGTH );
...
*/
   if( victim->sex == SEX_FEMALE )
      mudstrlcpy( name, "She", MAX_STRING_LENGTH );
   else if( victim->sex == SEX_MALE )
      mudstrlcpy( name, "He", MAX_STRING_LENGTH );
   else
      mudstrlcpy( name, "It", MAX_STRING_LENGTH );
   ...
   if( IS_AFFECTED( victim, AFF_SPIRITGUARDIANS ) )
   {
      set_char_color( AT_FIRE, ch );
      ch_printf( ch, "&w...%s is protected by a large group of powerful looking spirits.\r\n", name );
   }
...
void show_char_to_char_0( CHAR_DATA * victim, CHAR_DATA * ch )
{
   ...
   if( IS_AFFECTED( victim, AFF_HIDE ) )
      mudstrlcat( buf, "&G(&zHiding&G)&w ", MAX_STRING_LENGTH );
   if( IS_AFFECTED( victim, AFF_PASS_DOOR ) )
      mudstrlcat( buf, "&P(&zTranslucent&P)&w ", MAX_STRING_LENGTH );
   if( IS_AFFECTED( victim, AFF_FAERIE_FIRE ) )
      mudstrlcat( buf, "&P(&pPink Aura&P)&w ", MAX_STRING_LENGTH );
   if( IS_EVIL( victim ) && ( IS_AFFECTED( ch, AFF_DETECT_EVIL ) || ch->Class == CLASS_PALADIN ) )
      mudstrlcat( buf, "&R(&rRed Aura&R)&w ", MAX_STRING_LENGTH );
   if( IS_NEUTRAL( victim ) && ch->Class == CLASS_PALADIN )
      mudstrlcat( buf, "&W(&zGrey Aura&W)&w ", MAX_STRING_LENGTH );
   if( IS_GOOD( victim ) && ch->Class == CLASS_PALADIN )
      mudstrlcat( buf, "&w(&WWhite Aura&w) ", MAX_STRING_LENGTH );

   if( IS_AFFECTED( victim, AFF_BERSERK ) )
      mudstrlcat( buf, "(Wild-eyed) ", MAX_STRING_LENGTH );
   if( IS_AFFECTED( victim, AFF_GRAPPLE ) )
      mudstrlcat( buf, "(Grappled) ", MAX_STRING_LENGTH );
   ...
   if( victim->desc && victim->desc->connected == CON_EDITING )
      mudstrlcat( buf, "&Y(&OWriting&Y)&w ", MAX_STRING_LENGTH );
...
/*   if( !IS_NPC( victim ) && !xIS_SET( ch->act, PLR_BRIEF ) )
      mudstrlcat( buf, victim->adjective, MAX_STRING_LENGTH ); */
   ...
   else
   {
      switch ( victim->position )
      {
         ...
         case POS_RESTING:
            ...
            break;
         case POS_MEMORIZE:
            if( victim->Class == CLASS_WIZARD )
               mudstrlcat( buf, " is sitting here, pouring over an open spellbook.", MAX_STRING_LENGTH );
            else if( victim->Class == CLASS_CLERIC )
               mudstrlcat( buf, " is sitting here, deep in prayer.", MAX_STRING_LENGTH );
            else
               mudstrlcat( buf, " is sitting here, deep in thought.", MAX_STRING_LENGTH );
            break;
         case POS_SITTING:
         ...
         case POS_STANDING:
            ...
            {
               if( victim->z - ch->z == 1 ) 
                  mudstrlcat( buf, " is flying just overhead.", MAX_STRING_LENGTH );
               else if( victim->z - ch->z == 2 )
                  mudstrlcat( buf, " is flying slightly above your reach.", MAX_STRING_LENGTH );
               else if( victim->z - ch->z == 3 )
                  mudstrlcat( buf, " is flying above you.", MAX_STRING_LENGTH );
               else if( victim->z - ch->z >= 4 )
                  mudstrlcat( buf, " is soaring far, far above you.", MAX_STRING_LENGTH );
               else
                  mudstrlcat( buf, " is hovering close to the ground.", MAX_STRING_LENGTH );
            }
            else
         ...
         case POS_MOUNTED:
            ...
            {
               mudstrlcat( buf, PERS( victim->mount, ch, FALSE ), MAX_STRING_LENGTH );
               mudstrlcat( buf, ".", MAX_STRING_LENGTH );
            }
            else
         ...
         case POS_BERSERK:
            ...
            {
               mudstrlcat( buf, PERS( victim->fighting->who, ch, FALSE ), MAX_STRING_LENGTH );
               mudstrlcat( buf, ".", MAX_STRING_LENGTH );
            }
            else
               mudstrlcat( buf, "someone who left??", MAX_STRING_LENGTH );
...
void show_char_to_char_1( CHAR_DATA * victim, CHAR_DATA * ch )
{
   ...
   {   
      if( IS_AFFECTED( victim, AFF_BARKSKIN ) )
         ch_printf( ch, "&O%s skin appears to be made of tree bark...&w\r\n", victim->sex == SEX_FEMALE ? "Her" : "His" );
      if( IS_AFFECTED( victim, AFF_MAGE_ARMOR ) )
         ch_printf( ch, "&C%s appears to be coated in a set of spectral plates...&w\r\n", victim->sex == SEX_FEMALE ? "She" : "He" );
   }
...
void show_char_to_char( CHAR_DATA * list, CHAR_DATA * ch )
{
   ...
   for( rch = list; rch; rch = rch->next_in_room )
   {
      ...
      else if( IS_AFFECTED( rch, AFF_HIDE ) && !IS_AFFECTED( ch, AFF_DETECT_HIDDEN ) && dc_perception( ch, TRUE ) > dc_stealth( rch, TRUE ) )
      {
         set_char_color( AT_DGREY, ch );
         send_to_char( "The faint outline of a humanoid figure is barely visible nearby.\r\n", ch );
      }
   }
   return;
}
   ...
   if( !xIS_SET( ch->act, PLR_BRIEF ) )
   {
      ...
      else
      {
         if( ch->in_room->nightdesc && ch->in_room->nightdesc[0] != '\0' )
            mudstrlcat( rdesc, ch->in_room->nightdesc, MAX_STRING_LENGTH );
         else if( ch->in_room->description && ch->in_room->description[0] != '\0' )
            mudstrlcat( rdesc, ch->in_room->description, MAX_STRING_LENGTH );  
      }
   }
...
void print_infoflags( CHAR_DATA *ch ) 
{
   ...
   if( IS_IMMORTAL( ch ) )
   {
      ch_printf( ch, "&c[&wArea: &w%s; %s (&c%s&w)&c]\r\n", ch->in_room->area->name, ch->in_room->area->filename, flag_string( tarea->flags, area_flags ) );
   ...
   if( arg1[0] == '\0' || !str_cmp( arg1, "auto" ) )
   {
      ...
      if( IS_PLR_FLAG( ch, PLR_ONMAP ) || IS_ACT_FLAG( ch, ACT_ONMAP ) )
	{
      ...
      else
      {
         ...
         {
            print_infoflags( ch );
            ch_printf( ch, "&c[&w#%d (&c%s&w) ", ch->in_room->vnum, sect_types[ch->in_room->sector_type] );
            ch_printf( ch, "&w(&c%s&w)&c]\r\n&w", ext_flag_string( &ch->in_room->room_flags, r_flags ) );
         }
      ...
      /* Passive Perception checks - Seikyr */
      send_to_char( "\r\n", ch );
      if ( ch->in_room->perdesc[0] != '\0' && dc_perception( ch, TRUE) >= ch->in_room->percheck )
      {
         send_to_char( ch->in_room->perdesc, ch );
         send_to_char( "\r\n", ch );
      }
      /*
...
/* start from last to first to get it in the proper order */
        ...
        d = ( fGroup ? ( whogr ? whogr->d : NULL ) : d->prev ) )
   {
      ...
      if( ( d->connected != CON_PLAYING && d->connected != CON_EDITING ) || !can_see( ch, d->character, FALSE ) || d->original, FALSE )
         continue;
      wch = d->original ? d->original : d->character;
      ...
      snprintf( class_text, MAX_INPUT_LENGTH, "&R%s%2d %s&W", NOT_AUTHED( wch ) ? "N" : " ", wch->level,
      ...
      switch ( wch->level )
      {
         ...
         case MAX_LEVEL - 3:
            ...
            break;
         case MAX_LEVEL - 4:
            Class = "&RQuest Master&W";
            break;
         case MAX_LEVEL - 5:
            Class = "&RNewbie Guide&W";
            break;
         case MAX_LEVEL - 6:
            Class = "&RGreater Power&W";
            break;
         case MAX_LEVEL - 7:
            Class = "&RGreater Power&W";
            break;
         case MAX_LEVEL - 8:
            Class = "&RIntermediate Power&W";
            break;
         case MAX_LEVEL - 9:
            Class = "&RIntermediate Power&W";
            break;
         case MAX_LEVEL - 10:
            Class = "&RLesser Power&W";
            break;
         case MAX_LEVEL - 11:
            Class = "&RLesser Power&W";
            break;
      ...
      if( xIS_SET( wch->act, PLR_WIZINVIS ) )
         snprintf( invis_str, MAX_INPUT_LENGTH, "&R(&W%d&R)&W ", wch->pcdata->wizinvis );
      else
         invis_str[0] = '\0';
      snprintf( buf, MAX_STRING_LENGTH, "%*s%-15s %s%s%s%s%s%s%s%s.%s%s%s\r\n",
                ...
                Class,
                invis_str,
                ( wch->desc && wch->desc->connected ) ? "&Y[&WOOC&Y]&W " : "",
                xIS_SET( wch->act, PLR_AFK ) ? "&Y[&WAFK&Y]&W " : "",
    ...
    */
    ...
    send_to_pager( "\r\n\r\n&R-&r.&Y,&O_&Y,&r.&R-&Y'&W`&Y'&R-&r.&Y,&O_&Y,&r.&R-&Y'&W`&Y'&R-&r.&Y,&O_&Y,&r.&R-&Y'&W`&Y'&r-&R.&r-&R=&Y[   &WDragonlands of Chaos   &Y]&R=&r-&R.&r-&Y'&W`&Y'&R-&r.&Y,&O_&Y,&r.&R-&Y'&W`&Y'&R-&r.&Y,&O_&Y,&r.&R-&Y'&W`&Y'&R-&r.&Y,&O_&Y,&r.&R-\r\n\r\n", ch );
...
/*      if ( !ch )
   ...
   if( first_imm )
   {
      if( !ch )
         fprintf( whoout, "%s",
                  "\r\n                          &O_____&Y,&r.&R-&Y'&W`&Y'&r-&R.&r-&R=&Y[   &WAdmins   &Y]&R=&r-&R.&r-&Y'&W`&Y'&R-&r.&Y,&O_____&W\r\n\r\n" );
      else
         send_to_pager( "\r\n                          &O_____&Y,&r.&R-&Y'&W`&Y'&r-&R.&r-&R=&Y[   &WAdmins   &Y]&R=&r-&R.&r-&Y'&W`&Y'&R-&r.&Y,&O_____&W\r\n\r\n", ch );
   }
   ...
   if( !ch )
   {
      fprintf( whoout, "&Y%d player%s online.\r\n", nMatch, nMatch == 1 ? "" : "s" );
      fclose( whoout );
      return;
   }

   set_char_color( AT_YELLOW, ch );
   ch_printf( ch, "&Y%d player%s online.\r\n", nMatch, nMatch == 1 ? "" : "s" );
   return;
}
      ...
      for( sn = 0; sn < num_skills; ++sn )
      {
         ...
         if( strcmp( skill->name, "reserved" ) == 0 && ( IS_IMMORTAL( ch ) || CAN_CAST( ch ) ) )
         {
            if( col % 3 != 0 )
               send_to_pager( "\r\n", ch );
            set_pager_color( AT_MAGIC, ch );
            send_to_pager_color( " &C----------------------------------&c[ &WSpells &c]&C----------------------------------\r\n",
                                 ch );
            col = 0;
         }
...
/*            if( !cnt )
         ...
         if( !IS_IMMORTAL( ch )
            ...
            continue;
/*   Weird code here that shows the player all the available skills for the mob's class, race, and level? Commented out -- Seikyr
         if( mob )
         {
         ...
         {*/
            ...
            if( !xIS_EMPTY( skill_table[sn]->subclasses ) )
            {
               if( !xHAS_BITS( skill_table[sn]->subclasses, ch->subclasses ) )
               is_ok = FALSE;
            }
...
/*         if( ch->pcdata->learned[normalSn] > 0 )
         ...
         pager_printf( ch, " &w%3d%% ", ch->pcdata->learned[normalSn] ); */
         if( ++col % 3 == 0 )
            send_to_pager( "\r\n", ch );
      }
      ...
      if( col % 3 != 0 )
         send_to_pager( "\r\n", ch );
      set_pager_color( AT_MAGIC, ch );
      pager_printf( ch, "&cYou have &W%d&c practice sessions left.\r\n", ch->practice );
   }
   else
   {
      int adept = 0;
      ...
      if( ch->practice <= 0 )
      {
         act( AT_TELL, "&C$n says to you, &c'&WYou must earn some more practice sessions.&c'&w", mob, NULL, ch, TO_VICT );
         return;
      }
      ...
      if( !xIS_EMPTY( skill_table[sn]->subclasses ) && !xHAS_BITS( skill_table[sn]->subclasses, ch->subclasses ) )
      {
         act( AT_TELL, "$n tells you, 'You aren't of the required subclass.'", mob, NULL, ch, TO_VICT );
         return;
      }
      /*
...
*/
      ...
      else
      {
         ...
         act( AT_ACTION, "You learn the technique of $T.", ch, NULL, skill_table[sn]->name, TO_CHAR );
         act( AT_ACTION, "$n learns the techniques of $T.", ch, NULL, skill_table[sn]->name, TO_ROOM );
...
/*          send_to_char_color( !IS_SET( ch->deaf, CHANNEL_PRAY )       ?
		" &G+PRAY"	:	" &g-pray", ch ); */
         if( get_trust( ch ) >= sysdata.muse_level )
            send_to_char_color( !IS_SET( ch->deaf, CHANNEL_HIGHGOD ) ? " &P+MUSE" : " &P-muse", ch );
...
void do_config( CHAR_DATA* ch, const char* argument)
{
   ...
   if( arg[0] == '\0' )
   {
      ...
      ch_printf( ch, "%-12s   %-12s   %-12s   %-12s\r\n           %-12s   %-12s   %-12s   %-12s\r\n           %-12s   %-12s  %-12s",
                 ...
                 xIS_SET( ch->act, PLR_DICEROLLS ) ? "[+] DICEROLLS" : "[-] dicerolls" );
      ...
      ch_printf( ch, "%-12s   %-12s   %-12s   %-12s   %-12s",
                 ...
                 xIS_SET( ch->act, PLR_AUTOCANTRIP ) ? "\r\n[+] AUTOCANTRIP"
                 : "\r\n[-] autocantrip" );
   ...
   else
   {
      ...
      if( !str_prefix( arg + 1, "autoexit" ) )
         ...
         bit = PLR_AUTOMAP;
      else if( !str_prefix( arg + 1, "dicerolls" ) )
         bit = PLR_DICEROLLS;
      else if( !str_prefix( arg + 1, "nice" ) )
         bit = PLR_NICE;
      else if( !str_prefix( arg + 1, "drag" ) )
         bit = PLR_SHOVEDRAG;
      else if( !str_prefix( arg + 1, "autocantrip" ) )
         bit = PLR_AUTOCANTRIP;
...
void do_slist( CHAR_DATA* ch, const char* argument)
{
   ...
   set_pager_color( AT_MAGIC, ch );
   send_to_pager( "                          &WSPELL&c & &WSKILL LIST\r\n", ch );
   send_to_pager( "&C ---------------------------------------------------------------------&W\r\n", ch );
      ...
      for( sn = 0; sn < num_skills; ++sn )
      {
         ...
         if( i == skill->skill_level[ch->Class] || i == skill->race_level[ch->race] )
         {
            ...
            switch ( skill->minimum_position )
            {
               ...
               case POS_DEAD:
                  snprintf( buf, MAX_INPUT_LENGTH, "&W%s", "&Wany" );
                  break;

               case POS_MORTAL:
                  snprintf( buf, MAX_INPUT_LENGTH, "&W%s", "&rmortally wounded&W" );
                  break;

               case POS_INCAP:
                  snprintf( buf, MAX_INPUT_LENGTH, "&W%s", "&rincapacitated&W" );
                  break;

               case POS_STUNNED:
                  snprintf( buf, MAX_INPUT_LENGTH, "&W%s", "&Ystunned&W" );
                  break;

               case POS_SLEEPING:
                  snprintf( buf, MAX_INPUT_LENGTH, "&W%s", "&csleeping&W" );
                  break;

               case POS_RESTING:
                  snprintf( buf, MAX_INPUT_LENGTH, "&W%s", "&cresting&W" );
                  break;

               case POS_STANDING:
                  snprintf( buf, MAX_INPUT_LENGTH, "&W%s", "&Gstanding&W" );
                  break;

               case POS_FIGHTING:
                  snprintf( buf, MAX_INPUT_LENGTH, "&W%s", "&Rfighting&W" );
                  break;

               case POS_EVASIVE:
                  snprintf( buf, MAX_INPUT_LENGTH, "%s", "&Rfighting (evasive)" ); /* Fighting style support -haus */
                  break;

               case POS_DEFENSIVE:
                  snprintf( buf, MAX_INPUT_LENGTH, "%s", "&Rfighting (defensive)" );
                  break;

               case POS_AGGRESSIVE:
                  snprintf( buf, MAX_INPUT_LENGTH, "%s", "&Rfighting (aggressive)" );
                  break;

               case POS_BERSERK:
                  snprintf( buf, MAX_INPUT_LENGTH, "%s", "&Rfighting (berserk)" );
                  break;
               case POS_MOUNTED:
                  snprintf( buf, MAX_INPUT_LENGTH, "&W%s", "&cmounted&W" );
                  break;

               case POS_SITTING:
                  snprintf( buf, MAX_INPUT_LENGTH, "&W%s", "&csitting&W" );
                  break;
            }

            pager_printf( ch, "&C%7s&c:&W %20.20s \t &CCurrent&c:&W %-3d &CMax&c:&W %-3d&C  MinPos&c:&W %s \r\n",
...
void do_spellbook( CHAR_DATA *ch, const char* argument )
{
   char arg[MAX_INPUT_LENGTH];
   int x, y = 0;
   ...
   if ( arg[0] == '\0' )
   {
      ...
      ch_printf( ch, "Spellbook for %s:\r\n", ch->name );
      send_to_char( "Currently prepared spell list: \r\n", ch );
      list_mem_spells( ch );
   
      send_to_char( "\r\n\r\n&wSpell Slot Information:\r\n", ch );
      for ( x = 0; x < MAX_PER_SLOT; x++ )
      {
         sprintf( buf1, " " );
         for ( y = 0; y < max_spell_slot_lookup( ch, x ); y++ )
         {  
            if ( ch->pcdata->slots[x][y].free == 1 )
               strcat( buf1, "&WX");
            else
               strcat( buf1, "&B-");
            ...
            if ( ( x == 0 ) )
            {
               send_to_char( "\n\r&CLevel One Slots&c  :&w", ch );
               send_to_char( buf1, ch );
            }
            else if ( (x == 1 ))
            {
               send_to_char( "\n\r&CLevel Two Slots&c  :&w", ch );
               send_to_char( buf1, ch );
            }
            else if ( (x == 2 ))
            {
               send_to_char( "\n\r&CLevel Three Slots&c:&w", ch );
               send_to_char( buf1, ch );
            }
            else if ( (x == 3 ))
            {
               send_to_char( "\n\r&CLevel Four Slots&c :&w", ch );
               send_to_char( buf1, ch );
            }
            else if ( (x == 4 ))
            {
               send_to_char( "\n\r&CLevel Five Slots&c :&w", ch );
               send_to_char( buf1, ch );
            }
            else if ( (x == 5 ))
            {
               send_to_char( "\n\r&CLevel Six Slots&c  :&w", ch );
               send_to_char( buf1, ch );
            }
            else if ( (x == 6 ))
            {
               send_to_char( "\n\r&CLevel Seven Slots&c:&w", ch );
               send_to_char( buf1, ch );
            }
            else if ( (x == 7 ))
            {
               send_to_char( "\n\r&CLevel Eight Slots&c:&w", ch );
               send_to_char( buf1, ch );
            }
            else if ( (x == 8 ))
            {
               send_to_char( "\n\r&CLevel &RNine &CSlots&c :&w", ch );
               send_to_char( buf1, ch );
            }
      }
      send_to_char( "\r\n", ch );
...
void do_questlog( CHAR_DATA * ch, const char *argument )
{
   ...
   ch_printf( ch, "You open your quest journal...\r\n" );
   
   if( !ch->pcdata->first_qbit )
   {
      send_to_char( "You have had no noteworthy adventures.\r\n", ch );
      return;
   }
   else
   {
