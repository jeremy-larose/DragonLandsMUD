...
const char *const day_name[] = {
   "Moonday", "Towerday", "Wandsday", "Tombsday", "Fireday",
   "Swordsday", "Soulday"
};

const char *const month_name[] = {
   "Wolf Moon", "Snow Moon", "Wyrm Moon", "Pale Sun",
   "Fertile Sun", "High Sun", "Thunder Sun", "Golden Sun", "Harvest Sun",
   "Hunters Moon", "Blood Moon", "High Moon"
};
...
void do_time( CHAR_DATA* ch, const char* argument)
{
   ...
   ch_printf( ch, "&wIt is &W%d&w o'clock &W%s&w, &W%s&w,&W %d%s&w day in the Month of the &W%s&w.\r\n"
...
/* Set the day */
   ...
   if( !str_cmp( arg2, "day" ) )
   {
      if( !arg3 || !is_number( arg3 ) || atoi( arg3 ) > sysdata.dayspermonth || atoi( arg3 ) <= 1 )
      {
...
/* Go through the months and find arg3 */
      ...
      if( !arg3 || !is_number( arg3 ) || atoi( arg3 ) > sysdata.monthsperyear || atoi( arg3 ) <= 1 )
      {
         send_to_char( "You must specify a valid month number:\r\n", ch );

/* List all the months with a counter next to them*/
         count = 1;
         while( month_name[x] && str_cmp(month_name[x], " ") && x < sysdata.monthsperyear)
...
/* What's the month? */
   ...
   {
      if( !arg3 || !str_cmp( arg3, " " ) || is_number( arg3 ) )
      {
