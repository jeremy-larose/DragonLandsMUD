...
void say_spell( CHAR_DATA * ch, int sn, CHAR_DATA *victim );
...
bool is_immune( CHAR_DATA * ch, short damtype )
{
   ...
   {
      case SD_FIRE:
         return ( xIS_SET( ch->immune, RIS_FIRE ) );
      case SD_COLD:
         return ( xIS_SET( ch->immune, RIS_COLD ) );
      case SD_LIGHTNING:
         return ( xIS_SET( ch->immune, RIS_LIGHTNING ) );
      case SD_FORCE:
         return ( xIS_SET( ch->immune, RIS_FORCE ) );
      case SD_ACID:
         return ( xIS_SET( ch->immune, RIS_ACID ) );
      case SD_POISON:
         return ( xIS_SET( ch->immune, RIS_POISON ) );
      case SD_NECROTIC:
         return ( xIS_SET( ch->immune, RIS_NECROTIC ) );
      case SD_PSYCHIC:
         return ( xIS_SET( ch->immune, RIS_PSYCHIC ) );
      case SD_THUNDER:
         return ( xIS_SET( ch->immune, RIS_THUNDER ) );
      case SD_RADIANT:
         return ( xIS_SET( ch->immune, RIS_RADIANT ) );
   }
   return FALSE;
}
...
{
   return find_skill_of_type( ch, name, know, SKILL_PROFICIENCY );
}
/*
...
int dispel_casting( AFFECT_DATA * paf, CHAR_DATA * ch, CHAR_DATA * victim, int affect, bool dispel )
{
   ...
   if( IS_NPC( ch ) || ch->Class == CLASS_WIZARD )
      is_mage = TRUE;
...
void say_spell( CHAR_DATA * ch, int sn, CHAR_DATA *victim )
{
   ...
   if( ch == victim )
   {
      if( skill->cast_self && skill->cast_self[0] != '\0' )
         act( AT_MAGIC, skill->cast_self, ch, NULL, victim, TO_CHAR );
   }
   else
      if( skill->cast_char && skill->cast_char[0] != '\0' )
         act( AT_MAGIC, skill->cast_char, ch, NULL, victim, TO_CHAR );
   if( victim && skill->cast_vict && skill->cast_vict[0] != '\0' )
      act( AT_MAGIC, skill->cast_vict, ch, NULL, victim, TO_VICT );
   if( !victim && skill->cast_room && skill->cast_room[0] != '\0' )
         act( AT_MAGIC, skill->cast_room, ch, NULL, victim, TO_NOTVICT );
      
   if( skill->cast_self_room && skill->cast_self_room[0] != '\0' )
      act( AT_MAGIC, skill->cast_self_room, ch, NULL, victim, TO_NOTVICT );
   ...
   for( rch = ch->in_room->first_person; rch; rch = rch->next_in_room )
   {
      if( rch != ch )
      {
         ...
         {
/*         if( !skill->cast_vict || !skill->cast_room ) 
            act( AT_MAGIC, buf2, ch, NULL, rch, TO_VICT ); */
         if( dc_arcana( rch ) >= dc_spellcasting( ch ) )
            ch_printf( rch, "You guess %s is casting '%s'.\n\r", PERS( ch, rch, FALSE ), skill->name );
         }
      }
      else
      {
         if( skill->target != TAR_CHAR_OFFENSIVE && skill->cast_self && skill->cast_self[0] != '\0' )
            act( AT_MAGIC, buf2, ch, NULL, rch, TO_VICT );
      }
   }
   return;
...
int ris_save( CHAR_DATA * ch, int schance, int ris )
{
   ...
   {
      if( IS_NPC( ch ) && xIS_SET( ch->immune, ris ) )
         modifier += 0;
      else
         modifier += 2;
   }
...
bool saves_str( int level, CHAR_DATA * victim )
{
   ...
   save = 50 + ( victim->level - level - victim->saving_con ) * 5;
...
bool saves_dex( int level, CHAR_DATA * victim )
{
   ...
   save = 50 + ( victim->level - level - victim->saving_wis ) * 5;
   save = URANGE( 5, save, 95 );
   return chance( victim, save );
}
...
bool saves_con( int level, CHAR_DATA * victim )
{
   ...
   save = 50 + ( victim->level - level - victim->saving_dex ) * 5;
   save = URANGE( 5, save, 95 );
   return chance( victim, save );
}
...
bool saves_int( int level, CHAR_DATA * victim )
{
   ...
   save = 50 + ( victim->level - level - victim->saving_str ) * 5;
   save = URANGE( 5, save, 95 );
   return chance( victim, save );
}
...
bool saves_wis( int level, CHAR_DATA * victim )
{
   ...
   if( IS_NPC( victim ) && level > 10 )
      level -= 5;
   save = 50 + ( victim->level - level - victim->saving_int ) * 5;
   save = URANGE( 5, save, 95 );
   return chance( victim, save );
}
bool saves_cha( int level, CHAR_DATA * victim )
{
   ...
   if( IS_NPC( victim ) && level > 10 )
      level -= 5;
   save = 50 + ( victim->level - level - victim->saving_cha ) * 5;
   save = URANGE( 5, save, 95 );
   return chance( victim, save );
}
...
bool process_spell_components( CHAR_DATA * ch, int sn )
{
   ...
   while( comp[0] != '\0' )
   {
      ...
      if( !found )
      {
         if( IS_IMMORTAL( ch ) )
         {
            send_to_char( "You don't have the components for that spell, but your godly powers allow you to cast it!\r\n", ch );
            return TRUE;
         }
         else
         {
            send_to_char( "You don't have the necessary components for that spell.\r\n", ch );
            return FALSE;
         }
      }
   ...
   switch ( ch->substate )
   {
      default:
         ...
         if( get_trust( ch ) < LEVEL_GOD )
         {
            ...
            if ( skill_table[sn]->circle[ch->Class] >= 1 )
            {                          
               if( find_spell_level( ch, sn ) < 0 )
               {
                  send_to_char( "You do not have such a spell prepared.\r\n", ch );
                  return;
               }
            }
         ...
         {
            level = find_spell_level( ch, sn );
            if( ( level >= 0 ) && ( sn >= 0 ) )
            {
               for( x = 0; x < max_spell_slot_lookup( ch, level ); x++ )
               {
                  if( ch->pcdata->slots[level][x].free == 1 )
                  {
/*                     ch_printf( ch, "Free slot found! Circle: %d slot: %d\r\n", circle, x );  */
                     ch->pcdata->slots[level][x].free = 0;
                     ch->pcdata->slots[level][x].ticks = circle_wait_table[level].mem_wait;
                     break;
                  }
                  else
                  {
                     slot++;
                     /* ch_printf( ch, "Class= %s Empty slot found. circle: %d slot:%d free:%d count: %d\r\n", 
                               class_table[ch->Class]->who_name, circle, x, ch->pcdata->slots[circle][x].free, slot );  */
                     if( slot >= max_spell_slot_lookup( ch, level ) )
                     {
                        send_to_char( "You do not have an available spell slot for that level spell.\r\n", ch );
                        return;
                     }
                     else
                     {
/*                        ch_printf( ch, "Count (%d) < max_spell_slot_lookup (%d): continuing. \r\n", slot, max_spell_slot_lookup( ch, circle ) ); */
                        continue;
                     }
                  }
               }            
   ...
   {
      if( victim )
         say_spell( ch, sn, victim );
      else
         say_spell( ch, sn, NULL );   
   }
   ...
   else
   {
      ...
      else
      {
         ...
         end_timer( &time_used );
         if( skill->target == TAR_CHAR_OFFENSIVE && xIS_SET( ch->subclasses, SUBCLASS_ELD_KNIGHT ) && class_level( ch, CLASS_FIGHTER ) >= 36 && ch->bonus_action == TRUE )
         {
...
void do_memorize( CHAR_DATA *ch, const char *argument )
{
   SPELL_PREP_DATA *spell;
   int sn, max_spells, count = 0;
   char arg[MAX_INPUT_LENGTH];

   
   if( IS_NPC( ch ) )
   {
      send_to_char( "NPCs can't prepare spells!\r\n", ch );
      return;
   }

   one_argument( argument, arg );
   sn = find_spell( ch, arg, TRUE );
   max_spells = ( ch->level/2 + wis_app[get_curr_wis(ch)].bonus );
   
   if( arg[0] == '\0' )
   {
      send_to_char( "&C============&c[ &wPrepared Spells &c]&C=============&w\r\n", ch );
      send_to_char( "You have the following spells prepared:\r\n", ch );
      list_mem_spells( ch );
      return;
   }
   ...
   if ( skill_table[sn]->circle[ch->Class] == 0 )
   {                          
      send_to_char( "Cantrips do not need to be prepared.\r\n", ch );
      return;
   }
   ...
   if( arg[0] != '\0' )
   {
      for( spell = ch->first_prep; spell; spell = spell->next )
      {
         ...
         if( spell->sn == sn )
         {  
            if ( spell->fully_prepped == TRUE )
            {
               send_to_char( "You already have that spell prepared.\r\n", ch );
               return;
            }
            else
            {
               send_to_char( "You're already working on preparing that spell.\r\n", ch );
               return;
            }
         }
      }
      ...
      if( count > max_spells )
      {
         ch_printf( ch, "You cannot prepare more than %d spells.\r\n", max_spells );
         return;
      }
      CREATE( spell, SPELL_PREP_DATA, 1 );
         spell->sn = sn;
         spell->circle = skill_table[sn]->circle[ch->Class];
         spell->mem_wait = circle_wait_table[spell->circle].mem_wait;
         spell->fully_prepped = FALSE;
      LINK( spell, ch->first_prep, ch->last_prep, next, prev );
      ch_printf( ch, "You begin memorizing '%s'. Ticks to completion: %d\r\n",
                skill_table[sn]->name, spell->mem_wait );
      ch->position = POS_MEMORIZE;
      ch_printf( ch, "%d spells memorized out of a possible %d.\r\n", count, max_spells );
      return;
   }
}
 ...
 * in later functions).
 */
void list_mem_spells( CHAR_DATA *ch )
{
   int x, max_spells = 0;
   SPELL_PREP_DATA *spell;
   char buf[MAX_INPUT_LENGTH];
   ...
   for( x = 0; x < MAX_CIRCLE_LEVEL; ++x )
   {
      ...
      for( spell = ch->first_prep; spell; spell = spell->next )
      {
         if( ( spell->circle == x ) )
         {
            ...
            {
               strcat( buf, "&W" );
               strcat( buf,skill_table[spell->sn]->name );
            }
            else
            {
               strcat( buf, "&z" );
      ...
      if (total > 0)
      {
         if ( x == 1 )
         {
            send_to_char( "\n\r&CLevel One Spells&c: &w", ch );
            send_to_char( buf, ch );
         }
         else if ( x == 2 )
         {
            send_to_char( "\n\r&CLevel Two Spells&c: &w", ch );
            send_to_char( buf, ch );
         }
         else if ( x == 3 )
         {
            send_to_char( "\n\r&CLevel Three Spells&c: &w", ch );
            send_to_char( buf, ch );
         }
         else if ( x == 4 )
         {
            send_to_char( "\n\r&CLevel Four Spells&c: &w", ch );
            send_to_char( buf, ch );
         }
         else if ( x == 5 )
         {
            send_to_char( "\n\r&CLevel Five Spells&c: &w", ch );
            send_to_char( buf, ch );
         }
         else if ( x == 6 )
         {
            send_to_char( "\n\r&CLevel Six Spells&c: &w", ch );
            send_to_char( buf, ch );
         }
         else if ( x == 7 )
         {
            send_to_char( "\n\r&CLevel Seven Spells&c: &w", ch );
            send_to_char( buf, ch );
         }
         else if ( x == 8 )
         {
            send_to_char( "\n\r&CLevel Eight Spells&c: &w", ch );
            send_to_char( buf, ch );
         }
         else if ( x == 9 )
         {
            send_to_char( "\n\r&CLevel &RNine &CSpells&c: &w", ch );
            send_to_char( buf, ch );
         }
      }
   }
   ch_printf( ch, "\r\n\r\nTotal Spells Prepared: %d/%d", total, max_spells );
   return;
}
...
void do_forget(CHAR_DATA *ch, const char *argument)
{
   ...
   for( spell = ch->first_prep; spell; spell = spell->next )
   {
      if ( spell->sn == sn )
      {
         ch_printf( ch, "'%s' has been forgotten.\r\n", skill_table[spell->sn]->name );
         UNLINK( spell, ch->first_prep, ch->last_prep, next, prev );
         DISPOSE( spell );
         return;
      }
   }
...
ch_ret spell_acid_blast( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   dam = dice( level, 6 );
   if( saves_dex( level, victim ) )
      dam /= 2;
   return damage( ch, victim, dam, sn, FALSE );
}
...
/*   if( SPELL_FLAG( skill, SF_PKSENSITIVE ) && !IS_NPC( ch ) && !IS_NPC( victim ) )
   ...
   if( IS_AFFECTED( victim, AFF_BLIND ) || dc_constitution( victim ) > dc_spellcasting( ch )  )
   {
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
   ...
   for( vch = first_char; vch; vch = trvch_wnext( lc ) )
   {
      ...
      if( vch->in_room == where )
      {
         ...
         if( vch != ch && ( IS_NPC( ch ) ? !IS_NPC( vch ) : IS_NPC( vch ) ) )
            retcode = damage( ch, vch, saves_wis( level, vch ) ? dam / 2 : dam, sn, FALSE );
...
{
   return damage( ch, ( CHAR_DATA * ) vo, dice( 1, 8 ) + level / 3, sn, FALSE );
}

ch_ret spell_cause_critical( int sn, int level, CHAR_DATA * ch, void *vo )
{
   return damage( ch, ( CHAR_DATA * ) vo, dice( 3, 8 ) + level - 6, sn, FALSE );
}

ch_ret spell_cause_serious( int sn, int level, CHAR_DATA * ch, void *vo )
{
   return damage( ch, ( CHAR_DATA * ) vo, dice( 2, 8 ) + level / 2, sn, FALSE );
}
...
ch_ret spell_charm_person( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( xIS_SET( victim->immune, RIS_MAGIC ) || xIS_SET( victim->immune, RIS_CHARM ) )
   {
      immune_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
   ...
   if( IS_AFFECTED( victim, AFF_CHARM )
       ...
       || level < victim->level || circle_follow( victim, ch ) || !can_charm( ch ) || saves_wis( schance, victim ) )
   {
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
...
ch_ret spell_sacral_divinity( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( xIS_SET( ch->immune, RIS_MAGIC ) )
   {
...
ch_ret spell_curse( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( IS_AFFECTED( victim, AFF_CURSE ) || saves_wis( level, victim ) )
   {
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
   ...
   if( IS_NPC( victim ) && !xIS_EMPTY( victim->affected_by ) )
   {
      ...
      if( found ) /* Ok lets see if it is a spell */
      {
         ...
         if( paf )   /*It is a spell lets remove the spell too */
         {
            ...
            if( level < victim->level || saves_wis( level, victim ) )
            {
   ...
   for( vch = first_char; vch; vch = trvch_wnext( lc ) )
   {
      ...
      if( vch->in_room == where )
      {
         ...
         if( vch != ch && ( IS_NPC( ch ) ? !IS_NPC( vch ) : IS_NPC( vch ) )
            ...
            retcode = damage( ch, vch, level + dice( 2, 8 ), sn, FALSE );
         if( retcode == rCHAR_DIED || char_died( ch ) )
         {
            ch_died = TRUE;
         }
...
ch_ret spell_energy_drain( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   schance = ris_save( victim, victim->level, RIS_NECROTIC );
   if( schance == 1000 || saves_wis( schance, victim ) )
   {
  ...
  obj->gold_cost, obj->silver_cost);
      ...
      switch ( obj->item_type )
      {
         ...
         case ITEM_WEAPON:
            ...
            ch_printf( ch, "Proficiency Needed: %s\r\n", weapon_skills[obj->value[4]] );
...
ch_ret spell_legend_lore( int sn, int level, CHAR_DATA * ch, void *vo )
{
   OBJ_DATA *obj;
   CHAR_DATA *victim;
   CLAN_DATA *pclan;

   act( AT_MAGIC, "$n's eyes glaze over and $e goes into a trance.", ch, NULL, NULL, TO_ROOM );
   
   if( target_name[0] == '\0' )
   {
      send_to_char( "\r\nYour vision goes hazy for a second, before images of your surroundings fill your mind...\r\n\r\n", ch );
      ch_printf( ch, "You are standing in %s...\r\n", ch->in_room->area->name );
      if ( ch->in_room->area->lore && ch->in_room->area->lore[0] != '\0' )
         ch_printf( ch, "%s\r\n", ch->in_room->area->lore );
   }

   if( ( obj = get_obj_carry( ch, target_name ) ) != NULL )
   {
      ch_printf( ch, "\r\nYour vision goes hazy for a second, and images from %s fill your mind...\r\n\r\n", obj->short_descr );
      if( obj->lore )
         ch_printf( ch, "%s", obj->lore );
      else
         send_to_char( "There is not much to know about this item...\r\n", ch );
   }
   else if( ( victim = get_char_room( ch, target_name ) ) != NULL )
   {
      ch_printf( ch, "\r\nYour vision goes hazy for a second, and images from %s's past fill your mind...\r\n", victim->name );
      ...
      if( IS_NPC( victim ) && victim->lore[0] != '\0' )
         ch_printf( ch, "%s\r\n", victim->lore );
      else
      {
         ...
         if( victim->pcdata->bio && victim->pcdata->bio[0] != '\0' )
            ...
            pager_printf( ch, " %s does not have any other notable history...\r\n", victim->name );
      }
   }
...
ch_ret spell_poison( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( schance == 1000 || saves_con( schance, victim ) )
   {
      set_char_color( AT_MAGIC, ch );
      send_to_char( "Your magic fails to take hold.\r\n", ch );
      return rSPELL_FAILED;
...
ch_ret spell_sleep( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( IS_AFFECTED( victim, AFF_SLEEP )
       ...
       || level < victim->level
       || ( victim != ch && xIS_SET( victim->in_room->room_flags, ROOM_SAFE ) ) || saves_wis( schance, victim ) )
   {
      failed_casting( skill, ch, victim, NULL );
      if( ch == victim )
         return rSPELL_FAILED;
...
ch_ret spell_summon( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( ( victim = get_char_world( ch, target_name ) ) == NULL
       ...
       || ( IS_NPC( victim ) && saves_wis( level, victim ) )
...
ch_ret spell_teleport( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( !victim->in_room
       ...
       || ( victim != ch && ( saves_wis( level, victim ) || saves_int( level, victim ) ) ) )
   {
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
   ...
   for( vch = ch->in_room->first_person; vch; vch = vch->next_in_room )
   {
      if( !is_name( speaker, vch->name ) )
      {
         ...
         if( get_trust( vch ) >= LEVEL_IMMORTAL )
            ...
            send_to_char( saves_wis( level, vch ) ? buf2 : buf1, vch );
      }
   }
   return rNONE;
}
...
ch_ret spell_weaken( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( is_affected( victim, sn ) || saves_int( level, victim ) )
   {
      send_to_char( "Your magic fails to take hold.\r\n", ch );
      return rSPELL_FAILED;
   }
...
ch_ret spell_acid_breath( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( chance( ch, 2 * level ) && !saves_dex( level, victim ) )
   {
...
ch_ret spell_gas_breath( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   for( vch = ch->in_room->first_person; vch; vch = trvch_next( loop_ctrl ) )
   {
      ...
      if( IS_NPC( ch ) ? !IS_NPC( vch ) : IS_NPC( vch ) )
      {
         hpch = UMAX( 10, ch->hit );
         dam = number_range( hpch / 16 + 1, hpch / 8 );
         if( saves_dex( level, vch ) )
            dam /= 2;
         if( damage( ch, vch, dam, sn, FALSE ) == rCHAR_DIED || char_died( ch ) )
            ch_died = TRUE;
      }
   }
...
ch_ret spell_portal( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    */
	...
	send_to_char("You utter an incantation, and a portal forms in front of you!\r\n", ch);
	 ...
	 act( AT_IGNORE, skill->hit_char, ch, NULL, victim, TO_CHAR );
    if ( !skill->hit_room || skill->hit_room[0] == '\0' )
	 act( AT_IGNORE, "$n utters an incantation, and a portal forms in front of you!", ch, NULL, NULL, TO_ROOM );
    else
	 act( AT_IGNORE, skill->hit_room, ch, NULL, victim, TO_ROOM );
    if ( !skill->hit_vict || skill->hit_vict[0] == '\0' )
	 act( AT_MAGIC, "A shimmering portal forms in front of you!", victim, NULL, NULL, TO_ROOM );
    else
	 act( AT_IGNORE, skill->hit_vict, victim, NULL, victim, TO_ROOM );
...
ch_ret spell_recharge( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( obj->item_type == ITEM_STAFF || obj->item_type == ITEM_WAND )
   {
      ...
      if( obj->value[2] == obj->value[1] || obj->value[1] > ( obj->pIndexData->value[1] * 4 ) )
      {
         ...
         extract_obj( obj );
         if( damage( ch, ch, obj->level * 2, TYPE_UNDEFINED, FALSE ) == rCHAR_DIED || char_died( ch ) )
            return rCHAR_DIED;
         else
            return rSPELL_FAILED;
      }
...
if ( (pMobIndex = get_mob_index((int) abs(corpse->gold_cost) )) == NULL )
   {
...
ch_ret spell_possess( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( IS_AFFECTED( victim, AFF_POSSESS )
       || IS_AFFECTED( ch, AFF_CHARM )
       || level < victim->level || victim->desc || saves_wis( ctry, victim ) || !chance( ch, 25 ) )
   {
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
...
ch_ret spell_scorching_surge( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   return damage( ch, victim, ( int )( dam * 1.4 ), sn, FALSE );
}
...
bool check_save( int sn, int level, CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   if( skill->saves )
      ...
      {
         case SS_STR:
            if( dc_strength( victim ) > dc_spellcasting( ch ) )
            {
               saved = TRUE;
               break;
            }
            ...
            {
               saved = FALSE; /*
               if( xIS_SET( victim->abilities, ABIL_INDOMITABLE ) )
               {
                  ch_printf( ch, "You roar as you attempt again!\r\n" );
                  log_string( "%s: Attempting to save again, indomitable.\r\n" );
                  xTOGGLE_BIT( victim->abilities, ABIL_INDOMITABLE );
                  check_save( sn, level, ch, victim );
               } */
               break;
            }
         case SS_DEX:
            if( dc_dexterity( victim ) > dc_spellcasting( ch ) )
            {
               saved = TRUE;
               break;
            }
         ...
         case SS_CON:
            if( dc_constitution( victim ) > dc_spellcasting( ch ) )
            {
               saved = TRUE;
               break;
            }
         ...
         case SS_INT:
            if( dc_intelligence( victim ) > dc_spellcasting( ch ) )
            {
               saved = TRUE;
               break;
            }
         ...
         case SS_WIS:
            if( dc_wisdom( victim ) > dc_spellcasting( ch ) )
            {
               saved = TRUE;
               break;
            }
...
ch_ret spell_attack( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( !saved )
   {
      //log_printf( "%s save: failed from %s's %s!\r\n", victim->name, ch->name, skill->name );
      if( xIS_SET( victim->abilities, ABIL_INDOMITABLE ) ) 
      {
         log_printf( "%s is indomitable! Attempting to save again.\r\n", victim->name );
         saved = check_save( sn, level, ch, victim );
         xTOGGLE_BIT( victim->abilities, ABIL_INDOMITABLE );
      }
   }
   ...
   {
      //log_printf( "%s save: negate from %s's %s!\r\n", victim->name, ch->name, skill->name );
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
   if( skill->dice )
   ...
   if( saved )
   {
      ...
      {
         case SE_3QTRDAM:
            //log_printf( "%s save: 3/4 from %s's %s!\r\n", victim->name, ch->name, skill->name );

            dam = ( dam * 3 ) / 4;
            break;
         case SE_HALFDAM:
            //log_printf( "%s save: half from %s's %s!\r\n", victim->name, ch->name, skill->name );

            dam >>= 1;
            break;
         case SE_QUARTERDAM:
            //log_printf( "%s save: 1/4 from %s's %s!\r\n", victim->name, ch->name, skill->name );

            dam >>= 2;
            break;
         case SE_EIGHTHDAM:
            //log_printf( "%s save: 1/8 from %s's %s!\r\n", victim->name, ch->name, skill->name );
            dam >>= 3;
            break;
...
/*   if( skill->hit_char && skill->hit_char[0] != '\0' )
   ...
   if( skill->cast_self && skill->cast_self[0] != '\0' )
      act( AT_MAGIC, skill->cast_self, ch, NULL, NULL, TO_CHAR );
   if( skill->hit_room && skill->hit_room[0] != '\0' )
         ...
         switch ( af.location )
         {
            case APPLY_HIT:
               ...
               if( IS_NPC( victim ) && victim->hit <= 0 )
                  damage( ch, victim, 5, TYPE_UNDEFINED, FALSE );
               break;
...
ch_ret spell_affect( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( !groupsp && !areasp )
   {
      ...
      if( ( skill->type != SKILL_HERB
            && xIS_SET( victim->immune, RIS_MAGIC ) ) || is_immune( victim, SPELL_DAMAGE( skill ) ) )
      {
         immune_casting( skill, ch, victim, NULL );
         return rSPELL_FAILED;
      }
