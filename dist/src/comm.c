...
bool flush_buffer( DESCRIPTOR_DATA * d, bool fPrompt )
{
    ...
    * Changed for 4K - Seikyr */
   if( !mud_down && d->outtop > 4096 )
   {
      memcpy( buf, d->outbuf, 4096 );
      d->outtop -= 4096;
      memmove( d->outbuf, d->outbuf + 4096, d->outtop );
...
void nanny_show_stats( DESCRIPTOR_DATA *d, const char *argument )
{
   CHAR_DATA *ch;
   ...
   ch_printf( ch, "\r\n1. Str: %d\r\n", ch->perm_str );
   ch_printf( ch, "2. Int: %d\r\n", ch->perm_int );
   ch_printf( ch, "3. Wis: %d\r\n", ch->perm_wis );
   ch_printf( ch, "4. Dex: %d\r\n", ch->perm_dex );
   ch_printf( ch, "5. Con: %d\r\n", ch->perm_con );
   ch_printf( ch, "6. Cha: %d\r\n", ch->perm_cha );
   //ch_printf( ch, "7. Lck: %d\r\n\r\n", ch->perm_lck );
   ch_printf( ch, "Choose an attribute to raise by +1: [1-6] \r\n" );
}

void nanny_raise_stats( DESCRIPTOR_DATA *d, const char *argument )
{
   ...
   while( ch->tempnum > 0 )
   {
      ...
      {
         default:
            ch_printf( ch, "Invalid choice. Choose 1-6:\r\n" );
            return;
         case '1':
            if( ch->tempnum > 1 )
            {
               ch_printf( ch, "Raising Strength by 1 to %d. Choose another attribute to raise: \r\n", ch->perm_str+1 );
               ch->perm_str++;
               ch->tempnum--;
               return;
            }
            ...
            {
               ch_printf( ch, "Raising Strength by 1 to %d.\r\n", ch->perm_str+1 );
               ch->perm_str++;
               ch->tempnum--;
               break;
            }
         case '2':
            if( ch->tempnum > 1 )
            {
               ch_printf( ch, "Raising Intelligence by 1. Choose another attribute to raise: \r\n" );
               ch->perm_int++;
               ch->tempnum--;
               return;
            }
            ...
            {
               ch_printf( ch, "Raising Intelligence by 1.\r\n" );
               ch->perm_str++;
               ch->tempnum--;
               break;
            }
         case '3':
            if( ch->tempnum > 1 )
            {
               ch_printf( ch, "Raising Wisdom by 1. Choose another attribute to raise: \r\n" );
               ch->perm_wis++;
               ch->tempnum--;
               return;
            }
            ...
            {
               ch_printf( ch, "Raising Wisdom by 1.\r\n" );
               ch->perm_wis++;
               ch->tempnum--;
               break;
            }     
         case '4':
            if( ch->tempnum > 1 )
            {
               ch_printf( ch, "Raising Dexterity by 1. Choose another attribute to raise: \r\n" );
               ch->perm_dex++;
               ch->tempnum--;
               return;
            }
            ...
            {
               ch_printf( ch, "Raising Dexterity by 1.\r\n" );
               ch->perm_dex++;
               ch->tempnum--;
               break;
            }
         case '5':
            if( ch->tempnum > 1 )
            {
               ch_printf( ch, "Raising Constitution by 1. Choose another attribute to raise: \r\n" );
               ch->perm_con++;
               ch->tempnum--;
               return;
            }
            ...
            {
               ch_printf( ch, "Raising Constitution by 1.\r\n" );
               ch->perm_con++;
               ch->tempnum--;
               break;
            }
         ...
         case '6':
            if( ch->tempnum > 1 )
            {
               ch_printf( ch, "Raising Charisma by 1. Choose another attribute to raise: \r\n" );
               ch->perm_cha++;
               ch->tempnum--;
               return;
            }
            ...
            {
               ch_printf( ch, "Raising Charisma by 1.\r\n" );
               ch->perm_cha++;
               ch->tempnum--;
               break;
            }
...
void nanny_choose_favored_enemy( DESCRIPTOR_DATA *d, const char *argument )
{
   CHAR_DATA *ch = d->character;
   ...
   {
      default:
         ch_printf( ch, "Invalid choice. Choose 1-10.\r\n" );
         return;

      case '1':
         write_to_buffer( d, "You now gain a +2 bonus to attack and damage rolls when fighting humanoid creatures.\r\n", 0 );
         d->connected = CON_PLAYING;
         return;
         break;
...
void nanny_choose_weaponstyle( DESCRIPTOR_DATA *d, const char *argument )
{
   CHAR_DATA *ch = d->character;
         ...
         switch( argument[0] )
         {
            default:
               if( class_level( ch, CLASS_FIGHTER ) )
                  ch_printf( ch, "Invalid choice. Choose 1-6.\r\n" );
               else if( class_level( ch, CLASS_RANGER ) )
                  ch_printf( ch, "Invalid choice. Choose 1-4.\r\n" );
               return;

            case '1':
               if( ch->pcdata->learned[gsn_archery_style] > 1 )
               {
                  write_to_buffer( d, "You already know archery style.", 0 );
                  return;
               }
               else
               {
                  write_to_buffer( d, "You now gain +2 to your ranged weapon attack rolls.\r\n", 0 );
                  ch->pcdata->learned[gsn_archery_style] = 100;
                  d->connected = CON_PLAYING;
                  return;
               }
               break;
            ...
            case '2':
               if( ch->pcdata->learned[gsn_defense_style] > 1 )
               {
                  write_to_buffer( d, "You already know defense style.", 0 );
                  return;
               }
               else
               {
                  write_to_buffer( d, "You now gain a +1 bonus to your AC when wearing armor.\r\n", 0 );
                  ch->pcdata->learned[gsn_defense_style] = 100;
                  d->connected = CON_PLAYING;
                  return;
               }
               break;
            ...
            case '3':
               if( ch->pcdata->learned[gsn_dueling_style] > 1 )
               {
                  write_to_buffer( d, "You already know dueling style.", 0 );
                  return;
               }
               else
               {
                  write_to_buffer( d, "You now gain +2 to damage rolls when using a single weapon.\r\n", 0 );
                  ch->pcdata->learned[gsn_dueling_style] = 100;
                  d->connected = CON_PLAYING;
                  return;
               }
               break;
            ...
            case '4':
               if( ch->pcdata->learned[gsn_two_weapon_style] > 1 )
               {
                  write_to_buffer( d, "You already know two-weapon style.", 0 );
                  return;
               }
               else
               {
                  write_to_buffer( d, "You now add your ability modifier to the damage of your offhand attack.\r\n", 0 );
                  ch->pcdata->learned[gsn_two_weapon_style] = 100;
                  d->connected = CON_PLAYING;
                  return;
               }
            ...
            if( class_level( ch, CLASS_FIGHTER ) )
            {
               if( ch->pcdata->learned[gsn_great_weapon_style] > 1 )
               {
                  write_to_buffer( d, "You already know great weapon style.", 0 );
                  return;
               }
               else
               {
                  write_to_buffer( d, "You now reroll damage dice if you roll a 1 or 2 while wielding a two-handed or versatile weapon.\r\n", 0 );
                  ch->pcdata->learned[gsn_great_weapon_style] = 100;
                  d->connected = CON_PLAYING;
                  return;
               }
            }
            ...
            if( class_level( ch, CLASS_FIGHTER ) )
            {
               if( ch->pcdata->learned[gsn_protection_style] > 1 )
               {
                  write_to_buffer( d, "You already know protection style.", 0 );
                  return;
               }
               else
               {
                  write_to_buffer( d, "You can now use your reaction to impose a disadvantage when using a shield.\r\n", 0 );
                  ch->pcdata->learned[gsn_protection_style] = 100;
                  d->connected = CON_PLAYING;
                  return;
               }
            }
...
void nanny_get_new_class( DESCRIPTOR_DATA * d, const char *argument )
{
   ...
   {
            case '1':
               do_help( ch, "newclass_barbarian" );
               ch->Class = CLASS_BARBARIAN;
               break;

            case '2':
               do_help( ch, "newclass_bard" );
               ch->Class = CLASS_BARD;
               break;  

            case '3':
               do_help( ch, "newclass_cleric" );
               ch->Class = CLASS_CLERIC;
               break;   

            case '4':
               do_help( ch, "newclass_druid" );
               ch->Class = CLASS_DRUID;
               break;  

            case '5':
               do_help( ch, "newclass_fighter" );
               ch->Class = CLASS_FIGHTER;
               break;   

            case '6':
               do_help( ch, "newclass_monk" );
               ch->Class = CLASS_MONK;
               break;   

            case '7':
               do_help( ch, "newclass_paladin" );
               ch->Class = CLASS_PALADIN;
               break;   

            case '8':
               do_help( ch, "newclass_ranger" );
               ch->Class = CLASS_RANGER;
               break;    

            case '9':
               do_help( ch, "newclass_rogue" );
               ch->Class = CLASS_ROGUE;
               break;      

            case 'A':
            case 'a':
               do_help( ch, "newclass_sorcerer" );
               ch->Class = CLASS_SORCERER;
                break;      

            case 'B':
            case 'b':
               do_help( ch, "newclass_warlock" );
               ch->Class = CLASS_WARLOCK;
               break;      

            case 'C':
            case 'c':
               do_help( ch, "newclass_wizard" );
               ch->Class = CLASS_WIZARD;
               break; 

            default:
               ch_printf( ch, "&wThe hooded figure looks at you and says, &C\"&cI'm not sure I understand...  what is your profession?&C\" [1-9, A-C]: \r\n" );
               return;               
   }
   ...
   ch_printf( ch, "&wThe hooded figure looks at you and says, &C\"&cWill this be your profession?&C\" [Y/N]: \r\n" );
   d->connected = CON_CONFIRM_NEW_CLASS;
}
   
void nanny_confirm_new_class( DESCRIPTOR_DATA *d, const char *argument )
{
   CHAR_DATA *ch;
   char arg[MAX_STRING_LENGTH];
   ...
   switch( arg[0] )
   {
      ...
      case 'y':
         ...
         multiclass->classname = ch->Class;
         multiclass->level = 1;
         LINK( multiclass, ch->first_multiclass, ch->last_multiclass, next, prev );
         write_to_buffer( d, "Your class has been set.\r\n", 0);
         name_stamp_stats( ch );
         break;
      ...
      case 'n':
         write_to_buffer( d, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\r\n", 0 );
         write_to_buffer( d, "1) Barbarian - A fierce warrior of primitive background who can enter a battle rage.\r\n", 0 );
         write_to_buffer( d, "2) Bard - An inspiring musician whose power echoes the music of creation.\r\n", 0 );
         write_to_buffer( d, "3) Cleric - A priestly champion who wields divine magic in service of a higher power.\r\n", 0 );
         write_to_buffer( d, "4) Druid - A priest of the old faith, wielding the powers of nature and adopting animal forms.\r\n", 0 );
         write_to_buffer( d, "5) Fighter - A master of martial combat, skilled with a variety of weapons and armor.\r\n", 0 );
         write_to_buffer( d, "6) Monk - A master of martial arts, harnessing the power of the body in pursuit of physical and spiritual perfection.\r\n", 0 );
         write_to_buffer( d, "7) Paladin - A holy warrior bound to a sacred oath.\r\n", 0 );
         write_to_buffer( d, "8) Ranger - A warrior who combats threats on the edges of civilization.\r\n", 0 );
         write_to_buffer( d, "9) Rogue - A scoundrel who uses stealth and trickery to overcome obstacles and enemies.\r\n", 0 );
         write_to_buffer( d, "A) Sorcerer - A spellcaster who draws on inherent magic from a gift or bloodline.\r\n", 0 );
         write_to_buffer( d, "B) Warlock - A wielder of magic that is derived from a bargain with an extraplanar entity.\r\n", 0 );
         write_to_buffer( d, "C) Wizard - A scholarly magic-user capable of manipulating the structures of reality.\r\n", 0 );
         ch_printf( ch, "\r\n&cThe hooded figure gestures to you, &C\"&WWhat is your choice, &w%s?&C\" &W(1-9, A-C):\r\n", race_table[ch->race]->race_name );
         d->connected = CON_GET_NEW_CLASS;
         break;
            
      default:
         ch_printf( ch, "\r\n&cThe hooded figure gestures to you, &C\"&WI didn't hear you correctly... is this your choice, &w%s?&C\" &W(Y/N):\r\n", class_table[ch->Class]->who_name );
         return;
      }

   write_to_buffer( d, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\r\n", 0 );
   write_to_buffer( d, " Here, you can assign the following attribute scores to the abilities of your choice.\r\n", 0 );
 
   ch_printf( ch, "&w1) Strength: &G%s&W, measures natural athleticism and bodily power. Important for &wBarbarian, Fighter, and Paladin\r\n", attribtext( ch->perm_str ));
   ch_printf( ch, "&w2) Intelligence: &G%s&W, measures mental acuity, information recall, and analytical skills. Important for &wWizard\r\n", attribtext( ch->perm_int ));
   ch_printf( ch, "&w3) Wisdom: &G%s&W, measures awareness, intuition, and insight. Important for &wCleric, Druid\r\n", attribtext( ch->perm_wis ));
   ch_printf( ch, "&w4) Dexterity: &G%s&W, measures physical agility, reflexes, balance, and poise. Important for &wMonk, Ranger, and Rogue\r\n", attribtext( ch->perm_dex ));
   ch_printf( ch, "&w5) Constitution: &G%s&W, measures health, stamina, and vital force. Important for &weveryone\r\n", attribtext( ch->perm_con ));
   ch_printf( ch, "&w6) Charisma: &G%s&W, measures confidence, eloquence, and leadership. Important for &wBard, Sorcerer, and Warlock\r\n", attribtext( ch->perm_cha ));

   ch_printf( ch, "\r\n&cThe hooded figure says, &C\"&WA bold choice, &w%s&W. What will be your best attribute?&C\" &W(1-6):\r\n", class_table[ch->Class]->who_name );
   d->connected = CON_ROLL_STATS_1;
}

void nanny_roll_stats_one( DESCRIPTOR_DATA *d, const char *argument )
{
   CHAR_DATA *ch;
   char arg[MAX_STRING_LENGTH];
   ch = d->character;
   ...
   {
      case '1':
         write_to_buffer( d, "Your best ability is strength. What is your next best ability?\r\n", 0 );
         ch->perm_str = 15;
         break;

      case '2':
         write_to_buffer( d, "Your best ability is intelligence. What is your next best ability?\r\n", 0 );
         ch->perm_int = 15;
         break;

      case '3':
         write_to_buffer( d, "Your best ability is wisdom. What is your next best ability?\r\n", 0 );
         ch->perm_wis = 15;
         break;
      case '4':
         write_to_buffer( d, "Your best ability is dexterity. What is your next best ability?\r\n", 0 );
         ch->perm_dex = 15;
         break;
            
      case '5':
         write_to_buffer( d, "Your best ability is constitution. What is your next best ability?\r\n", 0 );
         ch->perm_con = 15;
         break;
      case '6':
         write_to_buffer( d, "Your best ability is charisma. What is your next best ability?\r\n", 0 );
         ch->perm_cha = 15;
         break;
   ...
   d->connected = CON_ROLL_STATS_2;
}

void nanny_roll_stats_two( DESCRIPTOR_DATA *d, const char *argument )
{
   char arg[MAX_STRING_LENGTH];
   CHAR_DATA *ch;
   ...
   {
      case '1':
         write_to_buffer( d, "Your next best ability is strength. What is your next best ability?\r\n", 0 );
         ch->perm_str = 14;
         break;

      case '2':
         write_to_buffer( d, "Your next best ability is intelligence. What is your next best ability?\r\n", 0 );
         ch->perm_int = 14;
         break;

      case '3':
         write_to_buffer( d, "Your next best ability is wisdom. What is your next best ability?\r\n", 0 );
         ch->perm_wis = 14;
         break;
      
      case '4':
         write_to_buffer( d, "Your next best ability is dexterity. What is your next best ability?\r\n", 0 );
         ch->perm_dex = 14;
         break;
            
      case '5':
         write_to_buffer( d, "Your next best ability is constitution. What is your next best ability?\r\n", 0 );
         ch->perm_con = 14;
         break;
      
      case '6':
         write_to_buffer( d, "Your next best ability is charisma. What is your next best ability?\r\n", 0 );
         ch->perm_cha = 14;
         break;
   ...
   d->connected = CON_ROLL_STATS_3;
}

void nanny_roll_stats_three( DESCRIPTOR_DATA *d, const char *argument )
{
   char arg[MAX_STRING_LENGTH];
   CHAR_DATA *ch;
   ...
   d->connected = CON_ROLL_STATS_4;
}

void nanny_roll_stats_four( DESCRIPTOR_DATA *d, const char *argument )
{
   char arg[MAX_STRING_LENGTH];
   CHAR_DATA *ch;
   ...
   d->connected = CON_ROLL_STATS_5;
}

void nanny_roll_stats_five( DESCRIPTOR_DATA *d, const char *argument )
{
   char arg[MAX_STRING_LENGTH];
   CHAR_DATA *ch;
   ...
   {
      case '1':
         write_to_buffer( d, "Your next best ability is strength. What is your next worst ability?\r\n", 0 );
         ch->perm_str = 10;
         break;

      case '2':
         write_to_buffer( d, "Your next best ability is intelligence. What is your worst ability?\r\n", 0 );
         ch->perm_int = 10;
         break;

      case '3':
         write_to_buffer( d, "Your next best ability is wisdom. What is your worst ability?\r\n", 0 );
         ch->perm_wis = 10;
         break;
      
      case '4':
         write_to_buffer( d, "Your next best ability is dexterity. What is your worst ability?\r\n", 0 );
         ch->perm_dex = 10;
         break;
            
      case '5':
         write_to_buffer( d, "Your next best ability is constitution. What is your worst ability?\r\n", 0 );
         ch->perm_con = 10;
         break;
      
      case '6':
         write_to_buffer( d, "Your next best ability is charisma. What is your worst ability?\r\n", 0 );
         ch->perm_cha = 10;
         break;
   ...
   d->connected = CON_ROLL_STATS_6;
}
void nanny_roll_stats_six( DESCRIPTOR_DATA *d, const char *argument )
{
   char arg[MAX_STRING_LENGTH];
   CHAR_DATA *ch;
   ...
   {
      case '1':
         write_to_buffer( d, "Your worst ability is strength.\r\n", 0 );
         ch->perm_str = 8;
         break;

      case '2':
         write_to_buffer( d, "Your worst ability is intelligence.\r\n", 0 );
         ch->perm_int = 8;
         break;

      case '3':
         write_to_buffer( d, "Your worst ability is wisdom.\r\n", 0 );
         ch->perm_wis = 8;
         break;
      
      case '4':
         write_to_buffer( d, "Your worst ability is dexterity.\r\n", 0 );
         ch->perm_dex = 8;
         break;
            
      case '5':
         write_to_buffer( d, "Your worst ability is constitution.\r\n", 0 );
         ch->perm_con = 8;
         break;
      
      case '6':
         write_to_buffer( d, "Your worst ability is charisma.\r\n", 0);
         ch->perm_cha = 8;
         break;
   ...
   ch_printf( ch, "&w5) Constitution: &G%s&W, measures health, stamina, and vital force. Important for &weveryone\r\n", attribtext( ch->perm_con ) );
   ch_printf( ch, "&w6) Charisma: &G%s&W, measures confidence, eloquence, and leadership. Important for &wBard, Sorcerer, and Warlock\r\n", attribtext( ch->perm_cha ));
   ch_printf( ch, "&cThe hooded figure scribbles into his book, &C\"&WWill you keep these abilities, &w%s?&C\"&W (Y/N):\n\r", ch->name );            
   d->connected = CON_CONFIRM_NEW_STATS;
}

void nanny_confirm_new_stats( DESCRIPTOR_DATA *d, const char *argument )
{
   CHAR_DATA *ch;
   char arg[MAX_STRING_LENGTH];
   char log_buf[MAX_STRING_LENGTH];
   ...
   switch( arg[0] )
   {
      case 'y':
      case 'Y':
         pager_printf( ch, "The hooded figure nods in acknowledgment, \"You are now ready to enter the world, %s...\"\r\n", ch->name );
      ...
      case 'N':
         ...
         ch_printf( ch, "&w1) Strength: %s, measures natural athleticism and bodily power. Important for &WBarbarian, Fighter, and Paladin\r\n", attribtext( ch->perm_str ));
         ch_printf( ch, "&w2) Intelligence: %s, Measures mental acuity, information recall, and analytical skills. Important for &WWizard\r\n", attribtext( ch->perm_int ));
         ch_printf( ch, "&w3) Wisdom: %s, measures awareness, intuition, and insight. Important for &WCleric, Druid\r\n", attribtext( ch->perm_wis ));
         ch_printf( ch, "&w4) Dexterity: %s, measures physical agility, reflexes, balance, and poise. Important for &WMonk, Ranger, and Rogue\r\n", attribtext( ch->perm_dex ));
         ch_printf( ch, "&w5) Constitution: %s, measures health, stamina, and vital force. Important for &Weveryone\r\n", attribtext( ch->perm_con ));
         ch_printf( ch, "&w6) Charisma: %s, measures confidence, eloquence, and leadership. Important for &WBard, Sorcerer, and Warlock\r\n", attribtext( ch->perm_cha ));

         ch_printf( ch, "\r\n&cThe hooded figure says, &C\"&WA bold choice, &w%s. &WWhat will be your best attribute?&C\" &W(1-6):\r\n", class_table[ch->Class]->who_name );
         d->connected = CON_ROLL_STATS_1;
         return;

      default:
         ch_printf( ch, "&cThe hooded figure peers at you, &C\"&WI'm not sure I understand. Are these your attributes?&C\"&W (Y/N):\r\n" );
         return;  
      }
}
...
void nanny_get_new_race( DESCRIPTOR_DATA * d, const char *argument )
{
   ...
   {
            case '1':
               do_help( ch, "newrace_human" );
               ch->race = RACE_HUMAN;
               break;

            case '2':
               do_help( ch, "newrace_elf" );
               ch->race = RACE_HIGH_ELF;
               break;

            case '3':
               do_help( ch, "newrace_dwarf" );
               ch->race = RACE_HILL_DWARF;
               break;

            case '4':
               do_help( ch, "newrace_halfling" );
               ch->race = RACE_LIGHTFOOT_HALFLING;
               break;

            case '5':
               do_help( ch, "newrace_halforc" );
               ch->race = RACE_HALF_ORC;
               break;

            case '6':
               do_help( ch, "newrace_halfelf" );
               ch->race = RACE_HALF_ELF;
               break;

            case '7':
               do_help( ch, "newrace_gnome" );
               ch->race = RACE_FOREST_GNOME;
               break;

            default:
               ch_printf( ch, "&cThe hooded figure looks at you confused, &C\"&WThat's not an available choice, &w%s.&C\" &W(1-7):\r\n", ch->name );
               return;
   }
   ch_printf( ch, "&cThe hooded figure looks at you and says, &C\"&WIs this your heritage?&C\"&W (Y/N):\r\n" );
...
void nanny_confirm_new_race( DESCRIPTOR_DATA *d, const char *argument )
{
   CHAR_DATA *ch;
   char arg[MAX_STRING_LENGTH];
   ...
   switch( arg[0] )
   {
      ...
      case 'n':
         write_to_buffer( d, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\r\n", 0 );
         write_to_buffer( d, "1) Human - Humans are the most adaptable and ambitious people among the common races.\r\n", 0 );
         write_to_buffer( d, "2) Elf - Elves are a magical people of otherworldly grace, living in the world but not entirely part of it.\r\n", 0 );
         write_to_buffer( d, "3) Dwarf - Bold and hardy, dwarves are known as skilled warriors, miners, and workers of stone and metal.\r\n", 0 );
         write_to_buffer( d, "4) Halfling - The diminutive halflings survive in a world full of larger creatures by avoiding notice or avoiding offense.\r\n", 0 );
         write_to_buffer( d, "5) Half-Orc - Half-orcs' grayish pigmentation, jutting jaws, prominent teeth, and towering builds make their heritage plain to see.\r\n", 0 );
         write_to_buffer( d, "6) Half-Elf - Half-elves combine what some say are the best qualities of their elf and human parents.\r\n", 0 );
         write_to_buffer( d, "7) Gnome - A gnome's energy and enthusiasm for living shines through every inch of their tiny body.\r\n", 0 );
         ch_printf( ch, "&cThe hooded figure nods in acknowledgment, &C\"&WAnd what is your heritage, &w%s&W?&C\"&W (1-7):\r\n", ch->name );
   ...
   ch_printf( ch, "\r\nThe hooded figure writes into his book, \"Welcome, little %s. What profession will you follow?\"\r\n", race_table[ch->race]->race_name );
   write_to_buffer( d, "You may pick from the following list:\r\n", 0 );
   write_to_buffer( d, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\r\n", 0 );
   write_to_buffer( d, "1) Barbarian - A fierce warrior of primitive background who can enter a battle rage.\r\n", 0 );
   write_to_buffer( d, "2) Bard - An inspiring musician whose power echoes the music of creation.\r\n", 0 );
   write_to_buffer( d, "3) Cleric - A priestly champion who wields divine magic in service of a higher power.\r\n", 0 );            
   ...
   write_to_buffer( d, "B) Warlock - A wielder of magic that is derived from a bargain with an extraplanar entity.\r\n", 0 );
   write_to_buffer( d, "C) Wizard - A scholarly magic-user capable of manipulating the structures of reality.\r\n", 0 );
   ch_printf( ch, "\r\n&cThe hooded figure gestures to you, &C\"&WAnd what profession do you pursue, &w%s?&C\" &W(1-12):\r\n", race_table[ch->race]->race_name );
   d->connected = CON_GET_NEW_CLASS;
}
...
void nanny_read_motd( DESCRIPTOR_DATA * d, const char *argument )
{
   ...
   if( ch->level == 0 )
   {
      ...
      ch->saving_str = race_table[ch->race]->saving_str;
      ch->saving_dex = race_table[ch->race]->saving_dex;
      ch->saving_con = race_table[ch->race]->saving_con;
      ch->saving_int = race_table[ch->race]->saving_int;
      ch->saving_wis = race_table[ch->race]->saving_wis;
      ch->saving_cha = race_table[ch->race]->saving_cha;
      ...
      ch->max_move = race_table[ch->race]->speed;
      ch->max_mana = 100;
...
void nanny_advance_level( DESCRIPTOR_DATA *d, const char *argument )
{
   CHAR_DATA *ch = d->character;
   ...
   switch( argument[0] )
   {
      default:
         if( ch->level == 8 || ch->level == 16 || ch->level == 24 || ch->level == 28 || ch->level == 32 || ch->level == 40 )
         {
            if( ch->statpoints < 1 )
            {
               ...
               write_to_buffer( d, "Please choose another stat to increase.\r\n", 0 );
         }
         write_to_buffer( d, "Please choose 1-7.\r\n", 0 );
         return;
      
      case '1':
         if( ch->Class == CLASS_FIGHTER )
         {
            ...
            else if( ch->level == 8 || ch->level == 16 || ch->level == 24 || ch->level == 28 || ch->level == 32 || ch->level == 40 )
            {
               if( ch->statpoints < 1 )
               {
                  d->connected = CON_PLAYING;
               ...
               if( ch->perm_str >= 21 )
                  write_to_buffer( d, "You cannot raise that stat any further than %d.\r\n", ch->perm_str );
               else
               {
                  ch->statpoints -= 1;
                  ch->perm_str += 1;
                  ch_printf( ch, "You've raised your strength to %d! %d points remaining.\r\n", ch->perm_str, ch->statpoints );

               }
               if( ch->statpoints < 1 )
                  d->connected = CON_PLAYING;
               else
                  d->connected = CON_ADVANCE_LEVEL;
            }
         }
         break;
         ...
         if( ch->Class == CLASS_FIGHTER )
         {
            ...
            else if( ch->level == 8 || ch->level == 16 || ch->level == 24 || ch->level == 28 || ch->level == 32 || ch->level == 40 )
            {
               ...
               else
               {
                  ch->statpoints -= 1;
                  ch->perm_dex += 1;
                  ch_printf( ch, "You've raised your dexterity to %d! %d points remaining.\r\n", ch->perm_dex, ch->statpoints );
               }
               if( ch->statpoints < 1 )
                  d->connected = CON_PLAYING;
               else
         ...
         if( ch->Class == CLASS_FIGHTER )
         {
            ...
            else if( ch->level == 8 || ch->level == 16 || ch->level == 24 || ch->level == 28 || ch->level == 32 || ch->level == 40 )
            {
               ...
               else
               {
                  ch->statpoints -= 1;
                  ch->perm_int += 1;
                  ch_printf( ch, "You've raised your intelligence to %d! %d points remaining.\r\n", ch->perm_int, ch->statpoints );
               }
               if( ch->statpoints < 1 )
                  d->connected = CON_PLAYING;
               else
         ...
         if( ch->Class == CLASS_FIGHTER )
         {
            ...
            else if( ch->level == 8 || ch->level == 16 || ch->level == 24 || ch->level == 28 || ch->level == 32 || ch->level == 40 )
            {
               ...
               else
               {
                  ch->statpoints -= 1;
                  ch->perm_wis += 1;
                  ch_printf( ch, "You've raised your wisdom to %d! %d points remaining.\r\n", ch->perm_wis, ch->statpoints );
               }
               if( ch->statpoints < 1 )
                  d->connected = CON_PLAYING;
               else
      ...
      case '5':
         if( ch->Class == CLASS_FIGHTER )
         {
            if( ch->level == 2 || ch->level == 20 )
            {
               write_to_buffer( d, "You now can use your reaction to impose a disadvantage when using a shield.\r\n", 0 );
               ch->pcdata->learned[gsn_protection_style] = 100;
               d->connected = CON_PLAYING;
               break;
            }
            else if( ch->level == 8 || ch->level == 16 || ch->level == 24 || ch->level == 28 || ch->level == 32 || ch->level == 40 )
            {
               ...
               else
               {
                  ch->statpoints -= 1;
                  ch->perm_con += 1;
                  ch_printf( ch, "You've raised your constitution to %d! %d points remaining.\r\n", ch->perm_con, ch->statpoints );
               }
               if( ch->statpoints < 1 )
                  d->connected = CON_PLAYING;
               else
         ...
         if( ch->Class == CLASS_FIGHTER )
         {
            ...
            else if( ch->level == 8 || ch->level == 16 || ch->level == 24 || ch->level == 28 || ch->level == 32 || ch->level == 40 )
            {
               ...
               else
               {
                  ch->statpoints -= 1;
                  ch->perm_cha += 1;
                  ch_printf( ch, "You've raised your charisma to %d! %d points remaining.\r\n", ch->perm_cha, ch->statpoints );
               }
               if( ch->statpoints < 1 )
                  d->connected = CON_PLAYING;
               else
...
void nanny( DESCRIPTOR_DATA * d, char *argument )
{
   ...
   switch ( d->connected )
   {
      ...
      case CON_ADVANCE_LEVEL:
         nanny_advance_level( d, argument );
         break;

      case CON_RAISE_STAT:
         nanny_raise_stats( d, argument );
         break;

      case CON_CHOOSE_WEAPONSTYLE_FIGHTER:
      case CON_CHOOSE_WEAPONSTYLE_RANGER:
         nanny_choose_weaponstyle( d, argument );
         break;

      case CON_ROLL_STATS_1:
         nanny_roll_stats_one( d, argument );
         break;

      case CON_ROLL_STATS_2:
         nanny_roll_stats_two( d, argument );
         break;

      case CON_ROLL_STATS_3:
         nanny_roll_stats_three( d, argument );
         break;

      case CON_ROLL_STATS_4:
         nanny_roll_stats_four( d, argument );
         break;

      case CON_ROLL_STATS_5:
         nanny_roll_stats_five( d, argument );
         break;

      case CON_ROLL_STATS_6:
         nanny_roll_stats_six( d, argument );
         break;

      case CON_CONFIRM_NEW_STATS:
         nanny_confirm_new_stats( d, argument );
         break;

      case CON_CHOOSE_FAVORED_ENEMY:
         nanny_choose_favored_enemy( d, argument );
         break;
   }
   return;
}
...
char *default_fprompt( CHAR_DATA * ch )
{
   ...
   mudstrlcpy( buf, "&w--<[ &GHealth: &w%h&G/&w%H ", 120 );
   if( IS_VAMPIRE( ch ) )
      mudstrlcat( buf, "&R%bbp", 120 );
   else
      mudstrlcat( buf, " &PMana: &w%m&P/&w%M ", 120 );
   mudstrlcat( buf, " &RStamina: &w%v&R/&w%V ", 120 );
   mudstrlcat( buf, " &CBreath: &w30&C/&w30 ]>--", 120 );
   /*
...
void display_prompt( DESCRIPTOR_DATA * d )
{
   ...
   const char *prompt;
   const char *helpstart = "&R---&r<<&R-&W@     To begin the game please type &RHELP START&W     &W@&R-&r>>&R---&w";
   char buf[MAX_STRING_LENGTH];
   char *pbuf = buf;
   unsigned int pstat;
   int percent;
