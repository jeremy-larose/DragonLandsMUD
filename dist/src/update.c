...
void new_spell_mem_update args( ( CHAR_DATA *ch ) );
void cooldown_update args( ( CHAR_DATA *ch ) );
/* From interp.c */
...
void advance_level( CHAR_DATA * ch )
{
   ...
   int add_hp, add_mana, add_move, add_prac, x, y;
   ...
   add_prac = wis_app[get_curr_wis( ch )].bonus;
   ...
   for ( x = 0; x < MAX_PER_SLOT; x++ )
      ...
      {
         ch->pcdata->slots[x][y].free = 1;
/*         ch_printf( ch, "You gain additional spell slots! Slot[%dx][%dy]:%dfree\r\n", x, y, ch->pcdata->slots[x][y].free ); */
      }
      ...
      switch ( iCond )
      {
         case COND_FULL:
            if( ch->level < LEVEL_AVATAR && !IS_VAMPIRE( ch ) )
            {
               ...
               retcode = damage( ch, ch, 1, TYPE_UNDEFINED, FALSE );
            }
            break;
         ...
         case COND_THIRST:
            if( ch->level < LEVEL_AVATAR && !IS_VAMPIRE( ch ) )
            {
               ...
               retcode = damage( ch, ch, 2, TYPE_UNDEFINED, FALSE );
            }
            break;
         ...
         case COND_BLOODTHIRST:
            if( ch->level < LEVEL_AVATAR )
            {
               ...
               retcode = damage( ch, ch, ch->max_hit / 20, TYPE_UNDEFINED, FALSE );
            }
            break;
         case COND_DRUNK:
            if( condition != 0 )
...
void mobile_update( void )
{
   ...
   if ( xIS_SET(ch->act, ACT_NIGHT ) && time_info.sunlight == SUN_LIGHT )
   {
      ...
      /* Begin Spec_Fun Section - Seikyr 12.19 *
      if( ch->spec_fun && !str_cmp( ch->spec_funname, "wolf" ) && number_bits( 2 ) == 0 )
      {
         switch( number_range( 1, 4 ) )
         {
         case 1:
            act( AT_WHITE, "$n looks up toward the sky and lets out a forlorn howl.", ch, NULL, NULL, TO_ROOM );
            break;
         case 2:
            act( AT_WHITE, "$n scratches behind $s ear and whimpers.", ch, NULL, NULL, TO_ROOM );
            break;
         case 3:
            act( AT_WHITE, "$n drags $s butt along the ground in a circle. $e might have worms...", ch, NULL, NULL, TO_ROOM );
            break;
         case 4:
            CHAR_DATA *victim;
            CHAR_DATA *v_next;
            ...
            for( victim = ch->in_room->first_person; victim; victim = v_next )
            {
               ...
               if( victim->spec_fun && !str_cmp( victim->spec_funname, "wolf" ) )
               {
                  log_printf( "Executing follow script! %s -> %s", victim->name, ch->name );
...
//      if( !xIS_SET( ch->act, ACT_RUNNING ) && ch->spec_fun && !IS_AFFECTED( ch, AFF_POSSESS ) )
      if( !xIS_SET( ch->act, ACT_RUNNING ) && !IS_AFFECTED( ch, AFF_POSSESS ) )
      {
         //if( ( *ch->spec_fun ) ( ch ) )
         //if( ch->spec_fun )
         //   continue;
   ...
   for( ch = last_char; ch; ch = trvch_wnext( lc ) )
   {
      ...
      if( !IS_NPC( ch ) && ch->level < LEVEL_IMMORTAL )
      {
	   ...
	   if ( ch->in_room )
	   {
         ...
         if( ch->position == POS_MEMORIZE ) /* Spell memorization 1/17 JL */
            new_spell_mem_update(ch);
...
//         }
      ...
      if( !char_died( ch ) )
      {
         ...
         if( IS_AFFECTED( ch, AFF_POISON ) )
         {
            ...
            damage( ch, ch, 6, gsn_poison, FALSE );
         }
         else if( ch->position == POS_INCAP )
            damage( ch, ch, 1, TYPE_UNDEFINED, FALSE );
         else if( ch->position == POS_MORTAL )
            damage( ch, ch, 4, TYPE_UNDEFINED, FALSE );
         if( char_died( ch ) )
            continue;
             ...
             || ( ch->in_room && ch->in_room->sector_type == SECT_OCEANFLOOR ) )
         {
            if( !IS_AFFECTED( ch, AFF_AQUA_BREATH ) )
            {
               if( ch->level < LEVEL_IMMORTAL )
               {
                  ...
                  damage( ch, ch, dam, TYPE_UNDEFINED, FALSE );
               }
            }
         }
...
void auction_update (void)
{
	...
	case 2 : /* going twice */
		...
		sprintf (buf, "%s: going %s for ", auction->item->short_descr,
			...
			else sprintf( buf2, "Error in update_auction report to Seikyr!");
	...
	case 3 : /* SOLD! */
		...
		>     can_carry_w( auction->seller ) )
		{
		    ...
		    obj_to_char (auction->item,auction->seller);
		tax = (int)get_value(auction->item->gold_cost, auction->item->silver_cost, auction->item->copper_cost)/20;
		/*tax = (int)cost * 0.05;*/
		boost_economy( auction->seller->in_room->area, tax );
...
/*
 * Go through PC cooldowns to restore counters
 */
void cooldown_update( CHAR_DATA *ch )
{
   ...
   for( cooldown = ch->first_cooldown; cooldown; cooldown = cooldown->next )
   {
      ...
      if( cooldown->wait > 0 )
      {
/*         ch_printf( ch, "Decreasing cooldown for %s. %d remaining. (%d/%d available)\r\n", skill_table[cooldown->sn]->name, cooldown->wait, cooldown->curr_available, cooldown->max_available ); */
         --cooldown->wait;
      }
      
      if( cooldown->wait == 0 && cooldown->curr_available < cooldown->max_available )
      {
         cooldown->curr_available = cooldown->max_available;
         ch_printf( ch, "You are now able to use %s again. (%d/%d uses remaining.)\r\n", skill_table[cooldown->sn]->name, cooldown->curr_available, cooldown->max_available );
         cooldown->wait = -1;
      }
   }
   return;
...
void new_spell_mem_update( CHAR_DATA * ch )
{
   int message = 0;
   ...
   for( spell = ch->first_prep; spell; spell = spell->next )
   {
      if( spell->mem_wait > 0 )
      {
         ...
         {
            ch_printf( ch, "Argh! %s has been lost!\r\n", skill_table[spell->sn]->name );
         ...
         if( spell->mem_wait == 0 )
         {
            spell->fully_prepped = TRUE;
            spell->mem_wait = -1;
            message = number_range( 1, 10 );
            if( message <= 4 )
            {
               ch_printf( ch, "Your blood burns as you finish memorizing %s.\n\r",
               skill_table[spell->sn]->name );
	    }
            else if( message <= 7 )
            {
               ch_printf( ch, "You feel that familiar tingle as you finish studying %s.\n\r",
               skill_table[spell->sn]->name );
            }
            else
            {
               ch_printf( ch, "A jolt travels your spine as you study %s.\n\r",
               skill_table[spell->sn]->name );
            }
            
            message = number_range( 1, 10 );
            if (message <= 3)
               act( AT_ACTION, "$n closes a spellbook with a smile.", ch, NULL, NULL, TO_ROOM);
            else if (message <= 5)
               act( AT_ACTION, "$n smiles broadly as $e puts away a spellbook.",ch, NULL, NULL, TO_ROOM);
            else if (message <= 8)
               act( AT_ACTION, "$n grins and packs up a spellbook.", ch, NULL, NULL, TO_ROOM);
            else
               act( AT_ACTION, "$n puts away a spellbook with a sigh of relief.",ch,NULL,NULL,TO_ROOM);
	 }
	 else  /* show that spells are still being memorized */
         {
            still_memming = TRUE;
            ch_printf( ch, "You turn a page in your spellbook, preparing %s. (%d pages remaining.)\r\n",
                     skill_table[spell->sn]->name, spell->mem_wait );
         }
      }
   }
   
   if ( !still_memming )  /* finished memorizing */
   {
      send_to_char( "Finished with your studies, you put your spellbook away and rest.\r\n", ch );
  ...
  for (x = 0; x < MAX_CIRCLE_LEVEL; x++)
	...
	if (ch->pcdata->circles[x][y].ticks == 0)
	{
	  ...
	  if (message <= 3)
        ...
        {
            still_memming = TRUE;
            ch_printf( ch, "You continue your meditation. (%d ticks remaining.)\r\n", ch->pcdata->circles[x][y].ticks );
        }
      }
    }
