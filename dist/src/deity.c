   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'E':
            ...
            KEY( "Element", deity->element, fread_bitvector( fp ) );
         ...
         case 'S':
            ...
            KEY( "Suscept", deity->suscept, fread_bitvector( fp ) );
      ...
      for( vch = first_char; vch; vch = vch->next )
      {
         ...
         if( !IS_NPC( vch ) )
         {
            if( vch->pcdata->deity == deity )
            {
               ...
               xREMOVE_BITS( vch->resistant, vch->pcdata->deity->element );
               xREMOVE_BITS( vch->susceptible, vch->pcdata->deity->suscept );
   ...
   if( !str_cmp( arg2, "suscept" ) )
   {
      ...
      while( argument[0] != '\0' )
      {
         ...
         else
         {
            ...
            {
               xTOGGLE_BIT( deity->suscept, value );
               fMatch = TRUE;
            }
         }
      }
   ...
   if( !str_cmp( arg2, "element" ) )
   {
      ...
      while( argument[0] != '\0' )
      {
         ...
         else
         {
            ...
            {
               xTOGGLE_BIT( deity->element, value );
               fMatch = TRUE;
            }
         }
      }
...
void do_showdeity( CHAR_DATA* ch, const char* argument)
{
   ...
   ch_printf( ch, "Suscept: %s\r\n", ext_flag_string( &deity->suscept, ris_flags ) );
   ch_printf( ch, "Element: %s\r\n", ext_flag_string( &deity->element, ris_flags ) );
...
void do_devote( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !str_cmp( arg, "none" ) )
   {
      ...
      xREMOVE_BITS( ch->resistant, ch->pcdata->deity->element );
      xREMOVE_BITS( ch->susceptible, ch->pcdata->deity->suscept );
